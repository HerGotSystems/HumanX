...
Last updated: 2026-06-06 after D-45 GitHub Actions Node 24 wrapper override.
...
**D-34 updated the workflow to Node 24** — `node-version: '20'` → `node-version: '24'`. D-44 audited a persistent annotation: `actions/checkout@v4` and `actions/setup-node@v4` JS wrapper bundles are compiled for Node 20 regardless of `node-version`. **D-45 adds `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24: true`** as a top-level workflow env to force those wrappers to run under Node 24. Annotation expected to clear on the next Read Smoke run after D-45 push.
...
4. **D-45 — ✅ COMPLETE (not yet pushed)** — `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24: true` added to workflow. CI-only, direct main.
5. **Run HumanX Read Smoke after D-45 push** — trigger manually on `main` to confirm annotation is gone and all endpoints still pass.
6. **D-26 manual test plan** — `docs/D26_MANUAL_LIVE_UI_TEST_PLAN.md`. When ready to open a browser session. Requires explicit per-session approval for any **[WRITE]** steps.
7. **No live write smoke** without explicit per-session approval.
8. **No further migrations** without explicit per-session approval and PRAGMA confirmation.
