...
Last updated: 2026-06-06 after D-36 stale check-count references fixed in usage docs.
...
The D-23 planning chain and all implementation batches through D-34 are complete. The canonical RunPack provenance system (D-24B + D-28 + D-29) is fully operational. The D-24 moderation chain (D-24A → D-24G) is fully operational. D-32 added the read smoke CI workflow; D-33 confirmed the first successful run; D-34 updated the workflow to Node 24. Static checks hold at 100/24/39.

1. **Run HumanX Read Smoke once after D-34 pushes to main** — GitHub Actions → HumanX Read Smoke → Run workflow → main. Confirm Node 24 produces the same green result as D-33 and that the deprecation annotation is gone. No new docs batch needed unless it fails.
