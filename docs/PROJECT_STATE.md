...
Last updated: 2026-06-06 after D-28 Worker-side RunPack provenance (PR #94 merged).
...
| `hardening-smoke-test.mjs` | `99 passed, 0 failed` |
...
1. **Frontend provenance de-duplication** — follow-up frontend-only batch (direct to main): update `generateRunPack()` in `public/app-v10.js` to skip re-stamping `packet_id`, `runpack_version`, `generated_at`, and `source_snapshot_hash` when the Worker response already contains them. Until this lands, the frontend's locally-generated `packet_id` overwrites the server-canonical one; advisory features still work correctly.
2. **Run D-26 manual test plan** — `docs/D26_MANUAL_LIVE_UI_TEST_PLAN.md` is ready. Execute when the user is ready to open a browser session against `humanx.rinkimirikata.com`. Use `HX_TEST_D26_` prefix for all test claims. Requires explicit per-session approval for any **[WRITE]** steps.
