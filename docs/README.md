...
| `hardening-smoke-test.mjs` | `1006 passed, 0 failed` |
...
### `D149B_OWNER_TOKEN_TELEMETRY_SAMPLING_REPORT_CHECKPOINT.md` ⭐ CURRENT — TELEMETRY SAMPLING REPORT IMPROVED; NOT YET DEPLOYED — READY FOR NEXT FEATURE
D-149A's review found a sample of 1 (`route_counts: { getMe: 1 }`) and no way to tell, from the response alone, which of the eight owner-sensitive routes had ever produced telemetry versus which hadn't. D-149B widens `ownerTokenTelemetryDebug()`'s response: `total_count` and a divide-by-zero-safe `valid_ratio`; `route_status_counts` giving every one of the eight known routes (`getMe`/`myHumanX`/`archiveMyHumanXItem`/`exportMyHumanX`/`saveProfileSettings`/`saveBeliefSnapshot`/`listBeliefSnapshots`/`promoteBeliefSnapshot`) all six status buckets, zero-defaulted; `observed_routes`/`unobserved_owner_routes` derived from that fixed route list rather than just whatever appears in query results; and explicit `sample_window`/`all_time` fields since the table has no time-range filter. Still admin-gated, still advisory-only, no enforcement, no migration, no `wrangler.toml`/frontend change. Re-confirmed (smoke-tested) the whole feature area never references a raw token/secret/admin-token/full-uid/headers/body/ip. **Live verification still pending** — ships code only, not yet deployed or checked, same discipline as D-148E→F. Baseline: 1006/24/57 (1 expected parameterised-route warning), up from 1000/24/57. Recommended next: deploy, then live-verify the new fields, then a successor telemetry review can finally answer the route-coverage question D-149A couldn't.
