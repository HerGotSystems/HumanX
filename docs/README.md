...
| `hardening-smoke-test.mjs` | `925 passed, 0 failed` |
...
### `D144C_NOINDEX_ROBOTS_CHECKPOINT.md` ⭐ CURRENT — NOINDEX/ROBOTS POLICY DEPLOYED + OWNER SMOKE PASS — READY FOR NEXT FEATURE
D-144A audit (public profile discoverability / robots / sitemap) → D-144B share-only indexing policy (`public/robots.txt` disallowing only `/u/`; `renderPublicProfileShell()` injects `<meta name="robots" content="noindex">` unconditionally into every `/u/:slug` response, and `<link rel="canonical" href=".../u/:slug">` only when a public profile resolves, never for the generic/private/not-found shell, never using the `#/u/` hash route). Owner confirmed live: `/u/calenhir` still returns 200 with noindex present, canonical present for the resolved profile, all five D-143 OG/Twitter tags still present, `/robots.txt` returns `User-agent: *` / `Disallow: /u/`, no sitemap.xml, no frontend behavior change — public profiles remain shareable by direct link, just not search-indexed. No migration, no new endpoint, no `wrangler.toml` change. Known limitation carried forward: profiles are intentionally not indexed in v1 (no `profile_indexable` opt-in flag yet), no sitemap, no `lastmod`/`profile_updated_at` tracking, no canonical on the generic private/not-found shell, no OG image yet, Cloudflare bot protection may still block some preview/crawler fetchers. Baseline: 925/24/56 (1 expected parameterised-route warning). Recommended next: D-145A — public profile trust / owner identity / signed owner header audit.
