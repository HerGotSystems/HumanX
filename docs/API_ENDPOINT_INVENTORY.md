...
| GET | `/api/version` | Returns static deployment provenance: `app`, `checkpoint`, `commit`, `baseline`, `updated_at`, advisory `note` (D-150A) | Public (no auth required) | None | No DB access. Values come from `src/deploy-meta.js` — a static module committed with each deploy. Never contains secrets, tokens, user ids, or admin fields. Update `deploy-meta.js` on every manual deploy so live `/api/version` reflects what production is actually running |
| GET | `/api/debug` | Returns row counts for all tables and the 5 most recent claims | **Admin only** (D-153B) | All tables (COUNT only), `claims` | Requires admin token (`x-humanx-admin`). Unauthenticated calls return 403. Returns live row counts for all tables and the 5 most recent claims (including items in non-public `review_state`) — admin-only debug view. Previous inventory entry incorrectly said "relies on obscurity only"; D-153A audit confirmed `requireAdmin` is in place |
