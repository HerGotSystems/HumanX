...
Last updated: 2026-06-06 after D-44 D-43 validation and Actions Node warning audit.
...
**D-34 updated the workflow to Node 24** — `node-version: '20'` → `node-version: '24'`. However, as audited in D-44, a Node 20 annotation persists because `actions/checkout@v4` and `actions/setup-node@v4` ship their own Node 20 wrapper bundles independently of the `node-version` setting. The annotation is non-blocking. D-45 will add `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` to silence it.
...
4. **D-45 — Optional, low-urgency** — add `FORCE_JAVASCRIPT_ACTIONS_TO_NODE24=true` env to `.github/workflows/read-smoke.yml`. Suppresses the Node 20 wrapper annotation. CI-only change; same policy as D-34 (direct main allowed). Not urgent until GitHub deadline (June 16, 2026 forced cutover; September 16 Node 20 removal). Full context in `docs/D44_D43_VALIDATION_AND_ACTIONS_NODE_WARNING.md`.
5. **D-26 manual test plan** — `docs/D26_MANUAL_LIVE_UI_TEST_PLAN.md`. When ready to open a browser session. Requires explicit per-session approval for any **[WRITE]** steps.
