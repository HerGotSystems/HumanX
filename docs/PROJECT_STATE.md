...
Last updated: 2026-06-06 after D-41 evidence review migration proposal.
...
1. **D-42 — Backend evidence moderation (branch + PR)** — Worker/module changes: `insertEvidence` passes `review_state='review'`; `listEvidenceVault` adds evidence-level filter; `claimDetail`+`getClaim` evidence queries filter by review_state; `reportTarget` adds evidence branch; `reviewQueue` adds evidence items; `reviewDecision` adds evidence branch. 5 new hardening smoke checks (103→108). Prerequisite: production migration 0007 approved and applied; `PRAGMA table_info(evidence)` confirmed.
