...
| `hardening-smoke-test.mjs` | `907 passed, 0 failed` |
...
### `D143C_PUBLIC_PROFILE_OG_ROUTE_CHECKPOINT.md` ⭐ CURRENT — PUBLIC PROFILE OG ROUTE DEPLOYED + OWNER SMOKE PASS — READY FOR NEXT FEATURE
D-143A audit (share-card / OpenGraph / SEO) → D-143B server-rendered OG meta tags for `GET /u/:slug` (`loadPublicProfileSummary()`, `escHtml()`, `renderPublicProfileShell()`, route matched before the static-asset fallback, `getPublicProfile()` refactored onto the shared summary helper with its response shape unchanged) → D-143B hotfix (root cause was **not** relative asset paths — `public/index.html` already used root-relative `/styles.css`/`/app-v10.js` — the real bug was `renderPublicProfileShell()` requesting `/index.html` from `env.ASSETS`, which Cloudflare's default `html_handling` redirects to `/`, producing an empty body; fixed by fetching `/` instead). Owner confirmed live: `https://humanx.rinkimirikata.com/u/calenhir` returns 200 with `<title>Calenhir on HumanX</title>` plus `og:title`/`og:description`/`og:type`/`og:url`/`twitter:card`, none of email/user_id/raw_json/stress_points_json/dimensions_json/contradictions_json/sharedSnapshot/dominantPattern appear in the source, and the old `#/u/calenhir` hash route still works. No global SPA fallback, no `wrangler.toml` `not_found_handling` change, no OG image endpoint, no mutating endpoint, no migration. Known limitation carried forward: no OG image/share-card image yet, no sitemap/robots/canonical-link pass yet, OG description uses bio only (not counts or shared-snapshot data), Cloudflare bot protection may block some external fetchers (observed during owner-smoke tooling, not an app bug). Baseline: 907/24/56 (1 expected parameterised-route warning). Recommended next: D-144A — public profile discoverability / robots / sitemap audit.
