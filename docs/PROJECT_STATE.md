...
Last updated: 2026-06-06 after D-27 RunPack provenance Phase 2 worker plan.
...
2. **RunPack provenance Phase 2 implementation** — plan complete in `docs/D27_RUNPACK_PROVENANCE_PHASE2_WORKER_PLAN.md`. Requires branch `feature/d27-runpack-provenance-worker` + PR. Key changes: `createAipPacket` generates server-canonical `packet_id` and calls new `workerSnapshotHash`; `claimDetail` queries add `id`/`created_at` to evidence/pressure/test SELECTs; `buildRunPack` accepts provenance param; Worker returns `runpack_version:'1.2'`. No D1 migration required.
