...
| `hardening-smoke-test.mjs` | `1000 passed, 0 failed` |
...
### `D148E_OWNER_TELEMETRY_DEBUG_RESPONSE_NORMALIZATION_CHECKPOINT.md` ⭐ CURRENT — DEBUG RESPONSE SHAPE NORMALIZED; NOT YET DEPLOYED — READY FOR NEXT FEATURE
D-148D's live check found `valid_count: null` from a console extraction even though the endpoint returned 200 — traced to the response shape itself: the old `{ byStatus, byRoute, recent }` shape only included a status key if at least one row with that status existed, so there was no predictable place to read "the valid count" from. D-148E rewrites `ownerTokenTelemetryDebug()` to return an explicit shape: `status_counts` always includes all six buckets (`secret_missing`/`missing`/`invalid`/`expired`/`uid_mismatch`/`valid`), zero-defaulted; a new top-level `valid_count` exactly mirrors `status_counts.valid` on every return path; `recent` rows are built via an explicit field-by-field allowlist (`route`/`status`/`uid_suffix`/`request_family_hash`/`created_at`, no `SELECT *`-style spread); a new `query_error` field surfaces a sanitized D1 error message (never request data) if the table is missing or a query fails, instead of silently looking identical to "zero rows so far". Still `requireAdmin()`-gated, still advisory-only, no enforcement, no migration, no `wrangler.toml` change, no frontend change. **Live verification still pending** — this checkpoint ships code only, not yet deployed or live-checked, per the same discipline as D-146C/D-147C/D-148D. Baseline: 1000/24/57 (1 expected parameterised-route warning), up from 993/24/57. Recommended next: deploy, then live-verify `valid_count` is now directly readable (not `null`).
