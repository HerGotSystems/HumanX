...
Last updated: 2026-06-06 after D-47 evidence moderation manual test plan.
...
7. **Run D-47 manually** — only when user explicitly wants a live-write browser session. Requires per-session approval. Use `HX_TEST_D47_` prefix. Plan at `docs/D47_EVIDENCE_MODERATION_MANUAL_TEST_PLAN.md`.
8. **D-48 — After D-47 execution** — record test results, pass/fail, evidence IDs, cleanup status.
9. **Actions v5 upgrade (optional)** — upgrade to `actions/checkout@v5` and `actions/setup-node@v5` when available with native Node 24 support. CI-only, direct main.
10. **D-26 manual test plan** — `docs/D26_MANUAL_LIVE_UI_TEST_PLAN.md`. General UI test plan still available.
11. **No live write smoke** without explicit per-session approval.
12. **No further migrations** without explicit per-session approval and PRAGMA confirmation.
