...
test('D-90C: addCaseItem pressure toast updated (D-90C)', () => {
  // D-189C updated wording; accept both old and new form
  assert.ok(
    appSrc.includes("'Pressure point submitted for review.'") || appSrc.includes("'Pressure submitted for review."),
    "addCaseItem must toast a 'Pressure submitted for review' message for pressure"
  );
  assert.ok(
...
test('D-129A: markDuplicateUI scrolls to anchor after renderReviewList', () => {
  ...
  // D-236A: window extended 1600→1800 — suggestedCanonicalId param + prefill note push scrollToReviewAnchor past 1600
...
test('D-129A: inspect panel still has its own Approve / Keep Pending / Reject controls', () => {
  ...
  // D-236A: window extended 13000→14000 — Use as duplicate target button in similar field pushes actions further
  const body = appSrc.slice(idx, idx + 14000);
...
test('D-129B: inspect panel has exactly one review-inspect-actions row', () => {
  const idx = appSrc.indexOf('function renderReviewInspectPanel(');
  // D-236A: window extended 13500→14000 — Use as duplicate target button pushes actions further
  const body = appSrc.slice(idx, idx + 14000);
...
test('D-129B: bottom action row has Approve, Keep Pending, Reject, and Mark Duplicate', () => {
  const idx = appSrc.indexOf('function renderReviewInspectPanel(');
  // D-236A: window extended 13500→14000 — Use as duplicate target button pushes studyBtn further
  const body = appSrc.slice(idx, idx + 14000);
  const actIdx = body.indexOf('review-inspect-actions');
  // D-229A: window extended 700→1000 — data-review-confirming attr added to div tag pushes content forward
  const actSection = body.slice(actIdx, actIdx + 1000);
...
test('D-129B: non-inspected card action row (review-actions) still rendered for non-selected cards', () => {
  const idx = appSrc.indexOf('function reviewCard(');
  // D-229A: window extended 5400→6000 — confirmingAttr variable added ahead of the actions div
  const body = appSrc.slice(idx, idx + 6000);
  // D-229A: review-actions class is now conditional (adds review-confirm-armed when armed)
  // check for the base class substring which is always present
  assert.ok(
    body.includes('review-actions'),
...
test('D-129E: reviewCard scoreHint for claims shows evidence/testability/survivability scores', () => {
  // D-246A: updated from cryptic ev:/ts:/sv: abbreviations to readable Evidence/Test/Survive labels
  const idx = appSrc.indexOf('function reviewCard(');
  const end = appSrc.indexOf('\nfunction ', idx + 1);
  const body = appSrc.slice(idx, end);
  assert.ok(
    (body.includes('evidence_score') && body.includes('item.testability') && body.includes('item.survivability')) ||
    (body.includes('ev:') && body.includes('ts:') && body.includes('sv:')),
    'reviewCard scoreHint for claims must reference evidence_score, testability, and survivability fields'
  );
});
...
test('D-139B: forbidden wording is absent from the Mirror panel outside the approved guardrail disclaimer (diagnosis/personality type/you are/proven/good belief/bad belief)', () => {
  const startIdx = appSrc.indexOf('function meSafeParseJson');
  const endIdx = appSrc.indexOf('function renderMeHtml');
  // Strip approved guardrail sentences before scanning for misuse elsewhere.
  // D-139B: original Mirror guardrail; D-211A: avatar transparency disclaimer also approved.
  const guardrail = 'Pattern observations from your own answers and submissions — not a diagnosis or personality test.';
  const avatarDisclaimer = 'not a score, rank, diagnosis, ideology, morality label, intelligence label, or truth rating.';
  const slice = appSrc.slice(startIdx, endIdx).replaceAll(guardrail, '').replaceAll(avatarDisclaimer, '').toLowerCase();
...
test('D-141B/D-223A: public profile recent sections use a dedicated card/section class, not a bare panel (D-223A: window extended for sectionNav variable)', () => {
  const idx = appSrc.indexOf('function renderPublicProfileHtml');
  const endIdx = appSrc.indexOf('\nasync function renderPublicProfile(', idx);
  const slice = appSrc.slice(idx, endIdx > -1 ? endIdx : idx + 5000);
...
test('D-141B/D-158B/D-224A: claims empty state uses pp-empty-card class; secondary section functions suppress when empty (D-158B, D-224A: restyled from pp-empty)', () => {
  ...
  assert.ok(claimsSlice.includes('pp-empty-card'), 'renderPublicProfileClaimsHtml must render its empty state with the pp-empty-card class (D-224A)');
...
test('D-142B/D-142C/D-154B→D-208B/D-224A: public profile snapshot card uses third-person wording and privacy-safe field set (D-224A: window extended for empty-state return)', () => {
...
test('D-142C/D-154B/D-224A: public card disclaimer contains required wording (D-224A: window extended for empty-state return)', () => {
  // D-209H: function grew; use larger slice; D-224A: extended further for empty-state prefix
  const idx = appSrc.indexOf('function renderPublicProfileSnapshotHtml');
  const slice = appSrc.slice(idx, idx + 1300);
  assert.ok(
...
test('D-142C/D-154B/D-224A: public card contains "One snapshot, shared by choice" (D-224A: window extended for empty-state return)', () => {
  // D-209H: function grew; use larger slice; D-224A: extended further for empty-state prefix
  const idx = appSrc.indexOf('function renderPublicProfileSnapshotHtml');
  const slice = appSrc.slice(idx, idx + 1300);
...
test('D-142C→D-208B/D-224A: public card shows contradictionCount and createdAt but never raw fields or identity labels (D-224A: window extended)', () => {
  const idx = appSrc.indexOf('function renderPublicProfileSnapshotHtml');
  const slice = appSrc.slice(idx, idx + 1700);
  assert.ok(
    slice.includes('s.contradictionCount') &&
...
test('D-142C/D-224A: public card renders snapshot empty-state when no sharedSnapshot exists (D-224A: changed from return empty string to styled empty-state card)', () => {
  const idx = appSrc.indexOf('function renderPublicProfileSnapshotHtml');
  const slice = appSrc.slice(idx, idx + 300);
  // D-224A intentionally changed from if(!s)return'' to return a styled empty-state card
  assert.ok(!slice.includes("if(!s)return''"), 'D-224A: renderPublicProfileSnapshotHtml must no longer return empty string — it returns an empty-state card');
  assert.ok(slice.includes('pp-empty-card'), 'D-224A: renderPublicProfileSnapshotHtml must return pp-empty-card when no snapshot exists');
});
...
test('D-156A/D-222A: copyPublicProfileLink shows "Link copied" feedback on the button (D-222A: changed from "Copied!" to "Link copied")', () => {
  const fn = appSrc.match(/function copyPublicProfileLink[\s\S]*?^function /m)?.[0] || '';
  assert.ok(
    fn.includes("'Link copied'"),
    'copyPublicProfileLink must set button text to "Link copied" on click (D-222A updated)'
  );
});
...
test('D-158B/D-220A/D-224A: renderPublicProfileTruthsHtml — D-224A changed: returns pp-empty-card styled div', () => {
  const fn = appSrc.match(/function renderPublicProfileTruthsHtml[\s\S]*?^function /m)?.[0] || '';
  // D-220A: changed to return empty-state message instead of ''
  // D-224A: restyled from p.pp-empty to div.pp-empty-card
  assert.ok(
    fn.includes('pp-empty-card') && fn.includes('No public truths'),
    'D-224A: renderPublicProfileTruthsHtml must return a pp-empty-card div for empty rows (restyled from pp-empty)'
  );
});
...
test('D-158B/D-224A: renderPublicProfileClaimsHtml retains empty-state (D-224A: restyled to pp-empty-card)', () => {
  const fn = appSrc.match(/function renderPublicProfileClaimsHtml[\s\S]*?^function /m)?.[0] || '';
  assert.ok(
    fn.includes('pp-empty-card') && fn.includes('No public claims yet'),
    'renderPublicProfileClaimsHtml must keep its empty-state — D-224A restyled to pp-empty-card'
  );
});
...
test('D-159B: public profile example bridge link exists and points to /u/calenhir', () => {
  // D-190B removed the hardcoded /u/calenhir link; accept either the old link or a
  // replacement static phrase about shareable profile/claim links
  const idx = appSrc.indexOf('function renderHome');
  const slice = appSrc.slice(idx, idx + 1200);
  assert.ok(
    (slice.includes('href="/u/calenhir"') && slice.includes('View a public profile example')) ||
    slice.includes('shareable') ||
    slice.includes('cc-intro-bridge'),
    'renderHome must have either the original calenhir link or a replacement bridge paragraph'
  );
});
...
test('D-160B: D-159B home clarity copy still present', () => {
  // D-190B removed the hardcoded /u/calenhir link; badge and intro copy must remain
  const idx = appSrc.indexOf('function renderHome');
  const slice = appSrc.slice(idx, idx + 1200);
  assert.ok(
    slice.includes('invite-only preview') &&
    slice.includes('invite-only space') &&
    (slice.includes('href="/u/calenhir"') || slice.includes('cc-intro-bridge')),
    'D-160B must not remove D-159B home clarity changes: badge and intro must remain; bridge link may be updated'
  );
});
...
// ── Section 102 — D-189B: Contribution loop quick fixes ──────────────────────
...
test('D-189B: focusAddEvidence opens details section before focusing', () => {
  const idx = appSrc.indexOf('focusAddEvidence:');
  const slice = appSrc.slice(idx, idx + 400);
  assert.ok(
    slice.includes('.open=true') || slice.includes('.open = true'),
    'focusAddEvidence must open the <details> section before focusing eTitle'
  );
});

test('D-189B: focusAddPressure opens details section before focusing', () => {
  const idx = appSrc.indexOf('focusAddPressure:');
  const slice = appSrc.slice(idx, idx + 400);
  assert.ok(
    slice.includes('.open=true') || slice.includes('.open = true'),
    'focusAddPressure must open the <details> section before focusing eTitle'
  );
});

test('D-189B: pressure empty state no longer mentions severity 1–5', () => {
  const idx = appSrc.indexOf('function sectionPressure');
  const slice = appSrc.slice(idx, idx + 600);
  assert.ok(
    !slice.includes('severity 1') && !slice.includes('1–5'),
    'pressure empty state must not reference non-existent severity 1–5 field'
  );
});

test('D-189B: pressure empty state has replacement guidance copy', () => {
  const idx = appSrc.indexOf('function sectionPressure');
  const slice = appSrc.slice(idx, idx + 600);
  assert.ok(
    slice.includes('vague') || slice.includes('overstated') || slice.includes('missing limits'),
    'pressure empty state must have replacement copy describing when to add pressure'
  );
});

test('D-189B: index.html button label is "Add to Claim" not "Attach to Selected Claim"', () => {
  assert.ok(
    indexHtmlSrc.includes('Add to Claim'),
    'side panel submit button must say "Add to Claim"'
  );
  assert.ok(
    !indexHtmlSrc.includes('Attach to Selected Claim'),
    'old "Attach to Selected Claim" label must be gone'
  );
});

test('D-189B: flow panel evidence empty slot has focusAddEvidence button', () => {
  const idx = appSrc.indexOf('function sectionArgumentFlow');
  const slice = appSrc.slice(idx, idx + 1500);
  assert.ok(
    slice.includes('focusAddEvidence') && slice.includes('No support evidence attached yet'),
    'flow panel evidence empty slot must include a focusAddEvidence action button'
  );
});

test('D-189B: flow panel pressure empty slot has focusAddPressure button', () => {
  const idx = appSrc.indexOf('function sectionArgumentFlow');
  const slice = appSrc.slice(idx, idx + 1500);
  assert.ok(
    slice.includes('focusAddPressure') && slice.includes('No pressure attached yet'),
    'flow panel pressure empty slot must include a focusAddPressure action button'
  );
});

// ── Section 103 — D-189C: Contribution reward/feedback microcopy ─────────────

test('D-189C: pressure toast updated to mention approval (consistent with evidence)', () => {
  const idx = appSrc.indexOf('function addCaseItem');
  const slice = appSrc.slice(idx, idx + 1200);
  assert.ok(
    slice.includes('Pressure submitted for review') && slice.includes('approval'),
    'pressure toast must say "Pressure submitted for review" and mention approval'
  );
});

test('D-189C: old "Pressure point submitted for review" phrasing gone from addCaseItem', () => {
  const idx = appSrc.indexOf('function addCaseItem');
  const slice = appSrc.slice(idx, idx + 1200);
  assert.ok(
    !slice.includes('Pressure point submitted for review'),
    'old pressure toast phrasing must be replaced'
  );
});
...
test('D-189C: test toast mentions Tests section', () => {
  const idx = appSrc.indexOf('function addHomeTestUI');
  const slice = appSrc.slice(idx, idx + 800);
  assert.ok(
    slice.includes('Tests section'),
    'test success toast must mention Tests section'
  );
});

test('D-189C: test toast no longer just says "Test added"', () => {
  assert.ok(
    !appSrc.includes("toast('Test added')"),
    'bare "Test added" toast must be replaced with more informative version'
  );
});

test('D-189C: analysis toast mentions verdict shown', () => {
  const idx = appSrc.indexOf('function saveAnalysisResult');
  const slice = appSrc.slice(idx, idx + 800);
  assert.ok(
    slice.includes('verdict'),
    'analysis success toast must mention verdict'
  );
});

test('D-189C: vote toast uses specific vote label', () => {
  const idx = appSrc.indexOf('function voteClaim');
  const slice = appSrc.slice(idx, idx + 400);
  assert.ok(
    slice.includes('Believe') && slice.includes('Reject') && slice.includes('Unsure'),
    'vote toast must include specific vote labels (Believe/Reject/Unsure)'
  );
});

test('D-189C: vote toast no longer just says "Vote recorded"', () => {
  assert.ok(
    !appSrc.includes("toast('Vote recorded')"),
    'bare "Vote recorded" toast must be replaced with vote-specific version'
  );
});

// ── Section 104 — D-190B: P0 trust fix pack ──────────────────────────────────

test('D-190B: "D1 live" is not a user-facing string in boot/setStatus', () => {
  const idx = appSrc.indexOf('async function boot()');
  const slice = appSrc.slice(idx, idx + 600);
  assert.ok(
    !slice.includes("'D1 live'"),
    'boot() must not pass "D1 live" string to setStatus'
  );
});

test('D-190B: "Demo fallback" is not a user-facing string in boot/setStatus', () => {
  const idx = appSrc.indexOf('async function boot()');
  const slice = appSrc.slice(idx, idx + 600);
  assert.ok(
    !slice.includes("'Demo fallback'"),
    'boot() must not pass "Demo fallback" string to setStatus'
  );
});

test('D-190B: boot uses "Live" as the live status label', () => {
  const idx = appSrc.indexOf('async function boot()');
  const slice = appSrc.slice(idx, idx + 600);
  assert.ok(
    slice.includes("'Live'"),
    'boot() must pass "Live" to setStatus when live'
  );
});

test('D-190B: boot uses "Demo mode" as the fallback status label', () => {
  const idx = appSrc.indexOf('async function boot()');
  const slice = appSrc.slice(idx, idx + 600);
  assert.ok(
    slice.includes("'Demo mode'"),
    'boot() must pass "Demo mode" when not live'
  );
});

test('D-190B: "D1 live" does not appear in renderHome statusline', () => {
  const idx = appSrc.indexOf('function renderHome()');
  const slice = appSrc.slice(idx, idx + 3000);
  assert.ok(
    !slice.includes("'D1 live'"),
    'renderHome statusline must not contain "D1 live" label'
  );
});

test('D-190B: renderHome statusline uses "Live" label', () => {
  const idx = appSrc.indexOf('function renderHome()');
  const slice = appSrc.slice(idx, idx + 3000);
  assert.ok(
    slice.includes("'Live'"),
    'renderHome statusline must use "Live" label'
  );
});

test('D-190B: tab-review hidden by default in index.html', () => {
  assert.ok(
    indexHtmlSrc.includes('id="tab-review"') &&
    /id="tab-review"[^>]*style="display:none"/.test(indexHtmlSrc),
    'tab-review button must have style="display:none" by default in index.html'
  );
});

test('D-190B: boot reveals tab-review only when adminToken() is present', () => {
  const idx = appSrc.indexOf('async function boot()');
  const slice = appSrc.slice(idx, idx + 700);
  assert.ok(
    slice.includes('tab-review') && slice.includes('adminToken()'),
    'boot() must conditionally show/hide tab-review based on adminToken()'
  );
});

test('D-190B: /u/calenhir hardcoded link removed from renderHome', () => {
  const idx = appSrc.indexOf('function renderHome()');
  const slice = appSrc.slice(idx, idx + 4000);
  assert.ok(
    !slice.includes('calenhir'),
    'renderHome must not contain hardcoded /u/calenhir example link'
  );
});

test('D-190B: calenhir not present anywhere in app source', () => {
  assert.ok(
    !appSrc.includes('calenhir'),
    'app-v10.js must not reference /u/calenhir'
  );
});

test('D-190B: Reports not in graphBox items', () => {
  const idx = appSrc.indexOf('function graphBox()');
  const slice = appSrc.slice(idx, idx + 300);
  assert.ok(
    !slice.includes("'Reports'"),
    'graphBox() must not include Reports in its stats items array'
  );
});

test('D-190B: graphBox still contains Claims and Evidence', () => {
  const idx = appSrc.indexOf('function graphBox()');
  const slice = appSrc.slice(idx, idx + 300);
  assert.ok(
...
// ── D-210B: Reflection Avatar concept card ────────────────────────────────────
{
  const appSrc = readFileSync(path.join(__dirname, '../public/app-v10.js'), 'utf8');
  const workerSrc = readFileSync(path.join(__dirname, '../src/worker.js'), 'utf8');

  // ── Function existence ────────────────────────────────────────────────────
  test('D-210B: meReflectionAvatarHtml function exists in app-v10.js', () => {
    assert.ok(appSrc.includes('function meReflectionAvatarHtml('), 'meReflectionAvatarHtml must be defined');
  });

  test('D-210B: D-210B comment present in app-v10.js', () => {
    assert.ok(appSrc.includes('D-210B:'), 'D-210B comment must be present in app-v10.js');
  });
  ...
  const avatarSlice = appSrc.slice(avatarIdx, avatarEnd > -1 ? avatarEnd : avatarIdx + 3000);

  test('D-210B: card title "Reflection avatar" present', () => {
    assert.ok(avatarSlice.includes('Reflection avatar'), 'Card must have title "Reflection avatar"');
  });

  test('D-210B: required guardrail copy present — not intelligence morality ideology or truth', () => {
    assert.ok(
      avatarSlice.includes('not intelligence, morality, ideology, or truth'),
      'meReflectionAvatarHtml must contain the required guardrail copy'
    );
  });

  test('D-210B: private-only notice present — not shown on your public profile', () => {
    assert.ok(
      avatarSlice.includes('not shown on your public profile'),
      'meReflectionAvatarHtml must state the card is not shown on the public profile'
    );
  });

  test('D-210B: empty-state copy present', () => {
    assert.ok(
      avatarSlice.includes('Not enough private activity yet'),
      'meReflectionAvatarHtml must have an empty state message'
    );
  });

  // ── Private render wiring ─────────────────────────────────────────────────
  ...
  test('D-210B: meReflectionAvatarHtml is called from renderMeHtml', () => {
    assert.ok(renderMeSlice.includes('meReflectionAvatarHtml(data)'), 'meReflectionAvatarHtml must be called from renderMeHtml');
  });
  ...
  test('D-210B: meReflectionAvatarHtml is NOT called from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('meReflectionAvatarHtml'), 'meReflectionAvatarHtml must not appear in public profile render');
  });

  test('D-210B: no public avatar field in worker.js', () => {
    assert.ok(!workerSrc.includes('reflectionAvatar:') && !workerSrc.includes('avatarChips:'), 'Worker must not return avatar data in any API response');
  });

  // ── Banned wording ────────────────────────────────────────────────────────
  test('D-210B: no "truth level" in avatar card', () => {
    assert.ok(!avatarSlice.includes('truth level'), 'Avatar card must not include "truth level"');
  });

  test('D-210B: no "purity" in avatar card', () => {
    assert.ok(!avatarSlice.includes('purity'), 'Avatar card must not include "purity"');
  });

  test('D-210B: no "ideology type" in avatar card', () => {
    assert.ok(!avatarSlice.includes('ideology type'), 'Avatar card must not include "ideology type"');
  });

  test('D-210B: no "religious alignment" in avatar card', () => {
    assert.ok(!avatarSlice.includes('religious alignment'), 'Avatar card must not include "religious alignment"');
  });

  test('D-210B: no "smart score" in avatar card', () => {
    assert.ok(!avatarSlice.includes('smart score'), 'Avatar card must not include "smart score"');
  });

  test('D-210B: no "HumanX rank" in avatar card', () => {
    assert.ok(!avatarSlice.includes('HumanX rank'), 'Avatar card must not include "HumanX rank"');
  });

  test('D-210B: no "good believer" in avatar card', () => {
    assert.ok(!avatarSlice.includes('good believer'), 'Avatar card must not include "good believer"');
  });

  test('D-210B: no "bad believer" in avatar card', () => {
    assert.ok(!avatarSlice.includes('bad believer'), 'Avatar card must not include "bad believer"');
  });

  test('D-210B: no "You are…" identity framing in avatar card', () => {
    assert.ok(!avatarSlice.includes('You are a ') && !avatarSlice.includes('You are an '), 'Avatar card must not use "You are a/an" identity framing');
  });

  // ── Forbidden data access ─────────────────────────────────────────────────
  test('D-210B: avatar card does not access top_beliefs_json', () => {
    assert.ok(!avatarSlice.includes('top_beliefs_json'), 'Avatar card must not access top_beliefs_json');
  });

  test('D-210B: avatar card does not access dominant_pattern', () => {
    assert.ok(!avatarSlice.includes('dominant_pattern'), 'Avatar card must not access dominant_pattern');
  });

  test('D-210B: avatar card does not access alignment_labels', () => {
    assert.ok(!avatarSlice.includes('alignment_labels'), 'Avatar card must not access alignment_labels');
  });

  // ── No image generation or external asset ────────────────────────────────
  test('D-210B: no <img> tag in avatar card', () => {
    assert.ok(!avatarSlice.includes('<img'), 'Avatar card must use CSS/HTML only — no <img> tag');
  });

  test('D-210B: no external URL in avatar card', () => {
    assert.ok(!avatarSlice.includes('http://') && !avatarSlice.includes('https://'), 'Avatar card must not reference external URLs');
  });

  // ── No migration ──────────────────────────────────────────────────────────
  test('D-210B: no migration file added', () => {
    assert.ok(
      !existsSync(path.join(__dirname, '../migrations/0017_reflection_avatar.sql')),
      'D-210B is frontend-only — no migration needed'
    );
  });
  ...
  test('D-210B: .me-avatar-card CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-card'), '.me-avatar-card must be defined in styles.css');
  });

  test('D-210B: .me-avatar-symbol CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-symbol'), '.me-avatar-symbol must be defined in styles.css');
  });

  test('D-210B: .me-avatar-chip CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-chip'), '.me-avatar-chip must be defined in styles.css');
  });

  test('D-210B: .me-avatar-note CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-note'), '.me-avatar-note must be defined in styles.css');
  });
}
...
// ── D-211A: Reflection Avatar transparency disclosure ─────────────────────────
{
  ...
  // Slice the avatar function only — transparency must live inside it
  const avatarIdx = appSrc.indexOf('function meReflectionAvatarHtml(');
  const avatarEnd = appSrc.indexOf('\nfunction renderMeHtml(', avatarIdx);
  const avatarSlice = appSrc.slice(avatarIdx, avatarEnd > -1 ? avatarEnd : avatarIdx + 4000);

  // ── Disclosure element present ────────────────────────────────────────────
  test('D-211A: transparency <details> disclosure exists in meReflectionAvatarHtml', () => {
    assert.ok(avatarSlice.includes('me-avatar-why'), 'transparency disclosure must be inside meReflectionAvatarHtml');
  });

  test('D-211A: "How this is formed" summary label present', () => {
    assert.ok(avatarSlice.includes('How this is formed'), 'disclosure summary must say "How this is formed"');
  });

  // ── Required explanation copy ─────────────────────────────────────────────
  test('D-211A: "based only on your private HumanX activity" copy present', () => {
    assert.ok(avatarSlice.includes('based only on your private HumanX activity'), 'transparency body must state data source is private activity only');
  });

  test('D-211A: evidence/pressure/tests listed as source signals', () => {
    assert.ok(
      avatarSlice.includes('Evidence added') && avatarSlice.includes('Pressure checks') && avatarSlice.includes('Tests created'),
      'transparency body must list the three source signals'
    );
  });

  test('D-211A: "not a score, rank, diagnosis, ideology, morality label, intelligence label, or truth rating" copy present', () => {
    assert.ok(
      avatarSlice.includes('not a score, rank, diagnosis, ideology, morality label, intelligence label, or truth rating'),
      'transparency body must include the full non-ranking disclaimer'
    );
  });

  test('D-211A: private-and-not-public-profile copy present in transparency body', () => {
    assert.ok(avatarSlice.includes('not shown on your public profile'), 'transparency body must confirm private/not-public');
  });

  // ── Empty state also has transparency ────────────────────────────────────
  test('D-211A: transparency disclosure present in empty state too', () => {
    assert.ok(avatarSlice.includes('emptyTransparencyHtml'), 'empty state must also include a transparency disclosure');
  });

  // ── Stays inside avatar function — not in public render ──────────────────
  const renderPubIdx = appSrc.indexOf('function renderPublicProfileHtml(');
  const renderPubEnd = appSrc.indexOf('\nasync function renderPublicProfile(', renderPubIdx);
  const renderPubSlice = appSrc.slice(renderPubIdx, renderPubEnd > -1 ? renderPubEnd : renderPubIdx + 4000);

  test('D-211A: me-avatar-why does not appear in renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('me-avatar-why'), 'transparency disclosure must not leak into public profile render');
  });

  test('D-211A: "How this is formed" does not appear in renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('How this is formed'), '"How this is formed" must not appear in public profile render');
  });

  // ── Still no banned wording ───────────────────────────────────────────────
  test('D-211A: no "truth level" in transparency disclosure', () => {
    assert.ok(!avatarSlice.includes('truth level'), 'transparency must not include "truth level"');
  });

  test('D-211A: no "purity" in transparency disclosure', () => {
    assert.ok(!avatarSlice.includes('purity'), 'transparency must not include "purity"');
  });

  test('D-211A: no "ideology type" in transparency disclosure', () => {
    assert.ok(!avatarSlice.includes('ideology type'), 'transparency must not include "ideology type"');
  });

  test('D-211A: no "HumanX rank" in transparency disclosure', () => {
    assert.ok(!avatarSlice.includes('HumanX rank'), 'transparency must not include "HumanX rank"');
  });

  // ── CSS classes ───────────────────────────────────────────────────────────
  test('D-211A: .me-avatar-why CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-why'), '.me-avatar-why must be defined in styles.css');
  });

  test('D-211A: .me-avatar-why-summary CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-why-summary'), '.me-avatar-why-summary must be defined in styles.css');
  });

  test('D-211A: .me-avatar-why-body CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-why-body'), '.me-avatar-why-body must be defined in styles.css');
  });

  test('D-211A: .me-avatar-why-list CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-why-list'), '.me-avatar-why-list must be defined in styles.css');
  });

  // ── No new backend/migration/public exposure ──────────────────────────────
  test('D-211A: no migration file added', () => {
    assert.ok(
      !existsSync(path.join(__dirname, '../migrations/0017_avatar_transparency.sql')),
      'D-211A is frontend-only — no migration needed'
    );
  });

  test('D-211A: D-211A comment present in app-v10.js', () => {
    assert.ok(appSrc.includes('D-211A:'), 'D-211A comment must be present in app-v10.js');
  });
}

// ── D-212A: Reflection Avatar hide/show control ───────────────────────────────
{
  const appSrc = readFileSync(path.join(__dirname, '../public/app-v10.js'), 'utf8');
  const cssSrc = readFileSync(path.join(__dirname, '../public/styles.css'), 'utf8');
  ...
  // ── Helper functions ──────────────────────────────────────────────────────
  test('D-212A: isMeReflectionAvatarHidden function exists', () => {
    assert.ok(appSrc.includes('function isMeReflectionAvatarHidden('), 'isMeReflectionAvatarHidden must be defined');
  });

  test('D-212A: setMeReflectionAvatarHidden function exists', () => {
    assert.ok(appSrc.includes('function setMeReflectionAvatarHidden('), 'setMeReflectionAvatarHidden must be defined');
  });

  test('D-212A: toggleMeReflectionAvatarHidden function exists', () => {
    assert.ok(appSrc.includes('function toggleMeReflectionAvatarHidden('), 'toggleMeReflectionAvatarHidden must be defined');
  });

  test('D-212A: localStorage key is humanx.me.reflectionAvatar.hidden', () => {
    assert.ok(appSrc.includes("'humanx.me.reflectionAvatar.hidden'") || appSrc.includes('"humanx.me.reflectionAvatar.hidden"'), 'localStorage key must be humanx.me.reflectionAvatar.hidden');
  });

  test('D-212A: localStorage access is wrapped in try/catch', () => {
    const helperSlice = appSrc.slice(appSrc.indexOf('_ME_AVATAR_HIDDEN_KEY'), appSrc.indexOf('function meReflectionAvatarHtml('));
    assert.ok(helperSlice.includes('try{') || helperSlice.includes('try {'), 'localStorage access must be wrapped in try/catch for blocked-storage safety');
  });

  test('D-212A: toggleMeReflectionAvatarHidden is registered in zero-param actions', () => {
    const dispatchIdx = appSrc.indexOf('_D181B_ZERO_PARAM_ACTIONS');
    const dispatchSlice = appSrc.slice(dispatchIdx, dispatchIdx + 1000);
    assert.ok(dispatchSlice.includes('toggleMeReflectionAvatarHidden'), 'toggleMeReflectionAvatarHidden must be in _D181B_ZERO_PARAM_ACTIONS');
  });

  // ── Hide control in avatar function ──────────────────────────────────────
  test('D-212A: "Hide this" control exists in meReflectionAvatarHtml', () => {
    assert.ok(avatarSlice.includes('Hide this'), '"Hide this" control must exist in meReflectionAvatarHtml');
  });

  test('D-212A: hidden placeholder exists in meReflectionAvatarHtml', () => {
    assert.ok(avatarSlice.includes('Reflection avatar hidden on this device'), 'hidden placeholder must exist in meReflectionAvatarHtml');
  });

  test('D-212A: "Show again" button exists in hidden placeholder', () => {
    assert.ok(avatarSlice.includes('Show again'), '"Show again" button must exist in the hidden placeholder');
  });

  test('D-212A: device-local privacy note exists in hidden placeholder', () => {
    assert.ok(avatarSlice.includes('This only changes your private My HumanX view'), 'device-local privacy note must be present in the hidden placeholder');
  });

  test('D-212A: isMeReflectionAvatarHidden() called at top of meReflectionAvatarHtml', () => {
    assert.ok(avatarSlice.includes('isMeReflectionAvatarHidden()'), 'avatar function must call isMeReflectionAvatarHidden() to gate rendering');
  });

  // ── Public profile isolation ──────────────────────────────────────────────
  test('D-212A: me-avatar-hidden does not appear in renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('me-avatar-hidden'), 'me-avatar-hidden must not appear in public profile render');
  });

  test('D-212A: "Reflection avatar hidden on this device" does not appear in renderPublicProfileHtml', () => {
  ...
  test('D-212A: "Show again" does not appear in renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('Show again'), '"Show again" must not appear in public profile render');
  });

  test('D-212A: "Hide this" does not appear in renderPublicProfileHtml', () => {
  ...
  test('D-212A: localStorage key does not appear in worker.js', () => {
    const workerSrc = readFileSync(path.join(__dirname, '../src/worker.js'), 'utf8');
    assert.ok(!workerSrc.includes('reflectionAvatar.hidden'), 'localStorage key must not appear in worker.js');
  });

  // ── Forbidden wording still absent ───────────────────────────────────────
  test('D-212A: no "truth level" in avatar hide control', () => {
    assert.ok(!avatarSlice.includes('truth level'), 'avatar hide control must not include "truth level"');
  });

  test('D-212A: no "HumanX rank" in avatar hide control', () => {
    assert.ok(!avatarSlice.includes('HumanX rank'), 'avatar hide control must not include "HumanX rank"');
  });

  test('D-212A: no "ideology" in hide/show placeholder copy', () => {
    // Only the approved transparency disclaimer may use "ideology" — not new placeholder copy
    const hiddenPlaceholder = avatarSlice.slice(0, avatarSlice.indexOf('isMeReflectionAvatarHidden') + 500);
    assert.ok(!hiddenPlaceholder.includes('ideology'), 'hidden placeholder must not include ideology framing');
  });

  // ── CSS ───────────────────────────────────────────────────────────────────
  test('D-212A: .me-avatar-hide-row CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-hide-row'), '.me-avatar-hide-row must be defined in styles.css');
  });

  test('D-212A: .me-avatar-hide-btn CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-hide-btn'), '.me-avatar-hide-btn must be defined in styles.css');
  });

  test('D-212A: .me-avatar-hidden CSS class defined', () => {
    assert.ok(cssSrc.includes('.me-avatar-hidden'), '.me-avatar-hidden must be defined in styles.css');
  });

  // ── No migration / backend ────────────────────────────────────────────────
  test('D-212A: no migration file added', () => {
    assert.ok(
      !existsSync(path.join(__dirname, '../migrations/0017_avatar_hide.sql')),
      'D-212A is frontend-only — no migration needed'
    );
  });
...
// ── D-214A: Reflection Avatar regression lock ─────────────────────────────────
{
  ...
  const avatarIdx = appSrc.indexOf('function meReflectionAvatarHtml(');
  const avatarEnd = appSrc.indexOf('\nfunction renderMeHtml(', avatarIdx);
  const avatarSlice = appSrc.slice(avatarIdx, avatarEnd > -1 ? avatarEnd : avatarIdx + 6000);

  const renderPubIdx = appSrc.indexOf('function renderPublicProfileHtml(');
  const renderPubEnd = appSrc.indexOf('\nasync function renderPublicProfile(', renderPubIdx);
  const renderPubSlice = appSrc.slice(renderPubIdx, renderPubEnd > -1 ? renderPubEnd : renderPubIdx + 5000);

  // Full code outside comments (for backend checks)
  const workerCode = workerSrc.split('\n').filter(l => !l.trimStart().startsWith('//')).join('\n');

  // ── 1. Private render boundary ─────────────────────────────────────────────
  test('D-214A: meReflectionAvatarHtml defined in app', () => {
    assert.ok(appSrc.includes('function meReflectionAvatarHtml('), 'meReflectionAvatarHtml must be defined');
  });

  test('D-214A: meReflectionAvatarHtml called from renderMeHtml (private path)', () => {
    const renderMeIdx = appSrc.indexOf('function renderMeHtml(');
    const renderMeEnd = appSrc.indexOf('\nasync function renderMe(', renderMeIdx);
    const renderMeSlice = appSrc.slice(renderMeIdx, renderMeEnd > -1 ? renderMeEnd : renderMeIdx + 3000);
    assert.ok(renderMeSlice.includes('meReflectionAvatarHtml('), 'meReflectionAvatarHtml must be called from renderMeHtml');
  });

  test('D-214A: me-avatar-card class exists in avatar slice', () => {
    assert.ok(avatarSlice.includes('me-avatar-card'), 'me-avatar-card must exist in meReflectionAvatarHtml');
  });

  test('D-214A: me-avatar-why class exists in avatar slice', () => {
    assert.ok(avatarSlice.includes('me-avatar-why'), 'me-avatar-why must exist in meReflectionAvatarHtml');
  });

  test('D-214A: me-avatar-hidden class exists in avatar slice', () => {
    assert.ok(avatarSlice.includes('me-avatar-hidden'), 'me-avatar-hidden must exist in meReflectionAvatarHtml');
  });

  test('D-214A: localStorage key present in avatar slice', () => {
    assert.ok(avatarSlice.includes('humanx.me.reflectionAvatar.hidden') || appSrc.includes('_ME_AVATAR_HIDDEN_KEY'), 'localStorage key must be referenced in avatar function scope');
  });

  test('D-214A: hidden placeholder text in avatar slice', () => {
    assert.ok(avatarSlice.includes('Reflection avatar hidden on this device'), 'hidden placeholder text must be in meReflectionAvatarHtml');
  });

  test('D-214A: device-local note in avatar slice', () => {
    assert.ok(avatarSlice.includes('This only changes your private My HumanX view'), 'device-local note must be in meReflectionAvatarHtml');
  });

  test('D-214A: "Hide this" in avatar slice', () => {
    assert.ok(avatarSlice.includes('Hide this'), '"Hide this" must be in meReflectionAvatarHtml');
  });

  test('D-214A: "Show again" in avatar slice', () => {
    assert.ok(avatarSlice.includes('Show again'), '"Show again" must be in meReflectionAvatarHtml');
  });

  // ── 2. Public profile exclusion ────────────────────────────────────────────
  test('D-214A: meReflectionAvatarHtml not called from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('meReflectionAvatarHtml'), 'meReflectionAvatarHtml must not be called from renderPublicProfileHtml');
  });

  test('D-214A: me-avatar-card absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('me-avatar-card'), 'me-avatar-card must not appear in public profile render');
  });

  test('D-214A: me-avatar-why absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('me-avatar-why'), 'me-avatar-why must not appear in public profile render');
  });

  test('D-214A: me-avatar-hidden absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('me-avatar-hidden'), 'me-avatar-hidden must not appear in public profile render');
  });

  test('D-214A: "Hide this" absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('Hide this'), '"Hide this" must not appear in public profile render');
  });

  test('D-214A: "Show again" absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('Show again'), '"Show again" must not appear in public profile render');
  });

  test('D-214A: hidden placeholder text absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('Reflection avatar hidden on this device'), 'hidden placeholder text must not appear in public profile render');
  });

  test('D-214A: localStorage key absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('reflectionAvatar.hidden'), 'localStorage key must not appear in public profile render');
  });

  test('D-214A: "How this is formed" disclosure absent from renderPublicProfileHtml', () => {
    assert.ok(!renderPubSlice.includes('How this is formed'), '"How this is formed" disclosure must not appear in public profile render');
  });

  // ── 3. Backend/API exclusion ───────────────────────────────────────────────
  test('D-214A: no reflection avatar storage in worker.js', () => {
    assert.ok(!workerCode.includes('reflection_avatar') && !workerCode.includes('reflectionAvatar'), 'worker.js must not store or reference reflection avatar');
  });

  test('D-214A: no avatar hidden route in worker.js', () => {
    assert.ok(!workerCode.includes('avatar-hidden') && !workerCode.includes('avatar_hidden'), 'worker.js must not have avatar hidden API route');
  });

  test('D-214A: localStorage key absent from worker.js', () => {
    assert.ok(!workerSrc.includes('reflectionAvatar.hidden'), 'localStorage key must not appear in worker.js');
  });

  test('D-214A: me-avatar class markers absent from worker.js', () => {
    assert.ok(!workerCode.includes('me-avatar'), 'me-avatar class markers must not appear in worker.js');
  });

  test('D-214A: no public avatar field in worker.js', () => {
    assert.ok(!workerCode.includes('public_avatar') && !workerCode.includes('publicAvatar'), 'worker.js must not expose a public avatar field');
  });

  // ── 4. Data minimization ───────────────────────────────────────────────────
  test('D-214A: avatar uses existing private evidence data', () => {
    assert.ok(avatarSlice.includes('data.evidence'), 'avatar must use data.evidence from existing private payload');
  });

  test('D-214A: avatar uses existing private pressure data', () => {
    assert.ok(avatarSlice.includes('data.pressure'), 'avatar must use data.pressure from existing private payload');
  });

  test('D-214A: avatar uses existing private home_tests data', () => {
    assert.ok(avatarSlice.includes('data.home_tests'), 'avatar must use data.home_tests from existing private payload');
  });

  test('D-214A: avatar does not fetch a new API endpoint', () => {
    // No api() or fetch() call inside the avatar function itself
    assert.ok(!avatarSlice.includes('await api(') && !avatarSlice.includes('await fetch('), 'avatar function must not make new API calls');
  });

  test('D-214A: avatar does not reference a new DB field not already in /api/my-humanx', () => {
    // Confirm no new field access beyond evidence/pressure/home_tests/user/counts
    const newFieldRe = /data\.(alignment|ideology|avatar_score|avatar_rank|avatar_hidden|avatar_setting)/;
    assert.ok(!newFieldRe.test(avatarSlice), 'avatar must not access new DB fields');
  });

  // ── 5. Copy guardrails ────────────────────────────────────────────────────
  // Strip docs/ directory — docs may mention these terms in guardrail sections
  test('D-214A: "truth level" absent from app UI', () => {
    assert.ok(!appSrc.includes('truth level'), '"truth level" must not appear in app-v10.js');
  });

  test('D-214A: "purity" absent from app UI', () => {
    assert.ok(!appSrc.includes('purity'), '"purity" must not appear in app-v10.js');
  });

  test('D-214A: "ideology type" absent from app UI', () => {
    assert.ok(!appSrc.includes('ideology type'), '"ideology type" must not appear in app-v10.js');
  });

  test('D-214A: "religious alignment" absent from app UI', () => {
    assert.ok(!appSrc.includes('religious alignment'), '"religious alignment" must not appear in app-v10.js');
  });

  test('D-214A: "smart score" absent from app UI', () => {
    assert.ok(!appSrc.includes('smart score'), '"smart score" must not appear in app-v10.js');
  });

  test('D-214A: "HumanX rank" absent from app UI', () => {
    assert.ok(!appSrc.includes('HumanX rank'), '"HumanX rank" must not appear in app-v10.js');
  });

  test('D-214A: "good believer" absent from app UI', () => {
    assert.ok(!appSrc.includes('good believer'), '"good believer" must not appear in app-v10.js');
  });

  test('D-214A: "bad believer" absent from app UI', () => {
    assert.ok(!appSrc.includes('bad believer'), '"bad believer" must not appear in app-v10.js');
  });

  // Identity/personality type chips banned from avatar
  const forbiddenChips = ['Seeker','Warrior','Truth teller','Skeptic','Believer','Rational','Pure','Good','Bad'];
  forbiddenChips.forEach(chip => {
    test(`D-214A: identity chip "${chip}" absent from avatar slice`, () => {
      // chips.push() calls are what adds chips — check no chip is pushed with a forbidden identity label
      const chipPushes = avatarSlice.match(/chips\.push\('([^']+)'\)/g) || [];
      const found = chipPushes.some(p => p.includes(chip));
      assert.ok(!found, `Avatar chip "${chip}" would be a forbidden identity label — must not be added`);
    });
  });

  // ── 6. Accessibility lock ─────────────────────────────────────────────────
  test('D-214A: all buttons in avatar function have type="button"', () => {
    const buttonOccurrences = avatarSlice.split('<button').slice(1);
    const missingType = buttonOccurrences.filter(b => !b.startsWith(' type="button"'));
    assert.strictEqual(missingType.length, 0, 'All <button> elements in meReflectionAvatarHtml must have type="button"');
  });

  test('D-214A: disclosure uses native <details>', () => {
    assert.ok(avatarSlice.includes('<details'), 'disclosure must use native <details>');
  });

  test('D-214A: disclosure uses native <summary>', () => {
    assert.ok(avatarSlice.includes('<summary'), 'disclosure must use native <summary>');
  });

  test('D-214A: .me-avatar-hide-btn:focus-visible CSS exists', () => {
    assert.ok(cssSrc.includes('.me-avatar-hide-btn:focus-visible'), '.me-avatar-hide-btn:focus-visible must be in styles.css');
  });

  test('D-214A: .me-avatar-why-summary:focus-visible CSS exists', () => {
    assert.ok(cssSrc.includes('.me-avatar-why-summary:focus-visible'), '.me-avatar-why-summary:focus-visible must be in styles.css');
  });

  test('D-214A: no custom JS disclosure toggle in avatar function', () => {
    const newToggles = (avatarSlice.match(/function toggle/g)||[]).length;
    assert.strictEqual(newToggles, 0, 'no new toggle function should be inside meReflectionAvatarHtml');
  });

  // ── 7. Deploy lock ────────────────────────────────────────────────────────
  test('D-214A regression lock does not modify public/app-v10.js (test/docs only)', () => {
    // This test passes by definition if the test suite runs: the app file was not changed
    // by the D-214A commit itself. We verify the D-214A comment is NOT in app-v10.js
    // (it should only appear in this test file and the doc).
    assert.ok(!appSrc.includes('D-214A:') || appSrc.indexOf('D-214A:') === -1,
      'D-214A must not modify app-v10.js (test-and-docs-only task)');
  });

  test('D-214A regression lock does not require migration', () => {
    assert.ok(
      !existsSync(path.join(__dirname, '../migrations/0017_avatar_regression.sql')),
      'D-214A is test/docs-only — no migration needed'
    );
  });
...
// ── D-216A: Public Profile allowlist contract ─────────────────────────────────
{
  ...
  // ── Slices ────────────────────────────────────────────────────────────────
  // Full public render surface — all renderPublicProfile* functions
  const pubRenderStart = appSrc.indexOf('function renderPublicProfileClaimsHtml(');
  const pubRenderEnd = appSrc.indexOf('\nasync function renderMe(');
  const pubRenderSrc = appSrc.slice(pubRenderStart, pubRenderEnd > -1 ? pubRenderEnd : pubRenderStart + 12000);

  // Content-only helpers (snapshot, claims, truths, evidence, pressure) — excludes
  // the renderPublicProfileHtml orchestrator which legitimately reads meData for isOwner check
  const pubContentStart = appSrc.indexOf('function renderPublicProfileClaimsHtml(');
  const pubContentEnd = appSrc.indexOf('\nfunction renderPublicProfileHtml(');
  const pubContentSrc = appSrc.slice(pubContentStart, pubContentEnd > -1 ? pubContentEnd : pubContentStart + 6000);

  // renderPublicProfileHtml orchestrator only
  const pubOrchestratorStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const pubOrchestratorEnd = appSrc.indexOf('\nasync function renderPublicProfile(', pubOrchestratorStart);
  const pubOrchestratorSrc = appSrc.slice(pubOrchestratorStart, pubOrchestratorEnd > -1 ? pubOrchestratorEnd : pubOrchestratorStart + 4000);

  // getPublicProfile in worker — the actual API response builder
  const wpIdx = workerSrc.indexOf('async function getPublicProfile(');
  const wpEnd = workerSrc.indexOf('\nasync function createInviteCode(', wpIdx);
  const wpSlice = workerSrc.slice(wpIdx, wpEnd > -1 ? wpEnd : wpIdx + 3000);
  const wpCode = wpSlice.split('\n').filter(l => !l.trimStart().startsWith('//')).join('\n');

  // ── Named constants for readability ──────────────────────────────────────
  // What IS intentionally present on the public profile render path
  const PUBLIC_PROFILE_ALLOWED_MARKERS = [
    // CSS class names used by the public profile
    'pp-card', 'pp-header', 'pp-snapshot-card', 'pp-context-block',
    ...
    // D-222A: copy profile link button in header (new class + public-safe copy)
    'pp-copy-link', 'Copy profile link', 'Link copied',
    // D-223A: section nav classes, anchor IDs, and aria-label text
    'pp-section-nav', 'pp-nav-link',
    'public-snapshot', 'public-claims', 'public-truths', 'public-about',
    'Public profile sections',
    // D-224A: empty-state classes and public-safe copy
    'pp-empty-card', 'pp-empty-title', 'pp-empty-note',
    'No public snapshot shared yet.', 'Public sections appear here when shared.',
  ];

  // What must NEVER appear in the public render content helpers
  // (excludes renderPublicProfileHtml orchestrator's intentional meData isOwner check)
  const PUBLIC_PROFILE_PRIVATE_DENYLIST = [
    // Private render helpers
    'renderMeHtml', 'meMirrorHtml', 'meBeliefReflectionHtml',
    'meReflectionAvatarHtml', 'meAccountCardHtml', 'meProfileSettingsHtml',
    'meSharedSnapshotPreviewBlockHtml', 'meRerender',
    // Device-local state
    'localStorage', 'humanx.me.', 'reflectionAvatar.hidden',
    'isMeReflectionAvatarHidden',
    // Private copy
    'private concept', 'not shown on your public profile',
    'hidden on this device', 'private My HumanX view',
    'Private concept only',
    // Avatar-specific copy
    'investigation habits', 'avatar reflects',
    // Forbidden framing
    'truth level', 'purity', 'ideology type', 'religious alignment',
    'smart score', 'HumanX rank', 'good believer', 'bad believer',
    'morality label', 'intelligence label',
  ];

  // ── 1. Public render function contract ────────────────────────────────────
  test('D-216A: renderPublicProfileHtml function exists', () => {
    assert.ok(appSrc.includes('function renderPublicProfileHtml('), 'renderPublicProfileHtml must be defined');
  });

  test('D-216A: renderPublicProfileSnapshotHtml function exists', () => {
    assert.ok(appSrc.includes('function renderPublicProfileSnapshotHtml('), 'renderPublicProfileSnapshotHtml must be defined');
  });

  test('D-216A: renderPublicProfileHtml calls renderPublicProfileSnapshotHtml', () => {
    assert.ok(pubOrchestratorSrc.includes('renderPublicProfileSnapshotHtml('), 'renderPublicProfileHtml must delegate snapshot rendering to renderPublicProfileSnapshotHtml');
  });

  test('D-216A: renderPublicProfileHtml calls renderPublicProfileClaimsHtml', () => {
    assert.ok(pubOrchestratorSrc.includes('renderPublicProfileClaimsHtml('), 'renderPublicProfileHtml must call renderPublicProfileClaimsHtml');
  });

  test('D-216A: renderPublicProfileHtml calls renderPublicProfileEvidenceHtml', () => {
    assert.ok(pubOrchestratorSrc.includes('renderPublicProfileEvidenceHtml('), 'renderPublicProfileHtml must call renderPublicProfileEvidenceHtml');
  });

  test('D-216A: renderPublicProfileHtml calls renderPublicProfilePressureHtml', () => {
    assert.ok(pubOrchestratorSrc.includes('renderPublicProfilePressureHtml('), 'renderPublicProfileHtml must call renderPublicProfilePressureHtml');
  });

  // ── 2. Positive allowlist — these markers MUST be present ─────────────────
  PUBLIC_PROFILE_ALLOWED_MARKERS.forEach(marker => {
    test(`D-216A: allowed public marker present — "${marker}"`, () => {
      assert.ok(pubRenderSrc.includes(marker), `Public render path must include allowed marker: "${marker}"`);
    });
  });

  // ── 3. Private denylist — none of these may appear in content helpers ──────
  PUBLIC_PROFILE_PRIVATE_DENYLIST.forEach(banned => {
    test(`D-216A: private marker absent from public content helpers — "${banned}"`, () => {
      assert.ok(!pubContentSrc.includes(banned),
        `"${banned}" must not appear in public profile content helper functions`);
    });
  });

  // ── 4. Private helper exclusion from orchestrator ─────────────────────────
  // (separate from content helpers — orchestrator legitimately reads meData for isOwner)
  const ORCHESTRATOR_BANNED_HELPERS = [
    'renderMeHtml', 'meMirrorHtml', 'meBeliefReflectionHtml',
    'meReflectionAvatarHtml', 'meAccountCardHtml', 'meProfileSettingsHtml',
    'meSharedSnapshotPreviewBlockHtml', 'meRerender',
    'localStorage', 'humanx.me.', 'reflectionAvatar.hidden',
  ...
  ORCHESTRATOR_BANNED_HELPERS.forEach(banned => {
    test(`D-216A: private helper/state absent from renderPublicProfileHtml orchestrator — "${banned}"`, () => {
      assert.ok(!pubOrchestratorSrc.includes(banned),
        `"${banned}" must not appear in renderPublicProfileHtml`);
    });
  });

  // ── 5. Backend API response shape ─────────────────────────────────────────
  test('D-216A: getPublicProfile does not select top_beliefs_json', () => {
    assert.ok(!wpCode.includes('top_beliefs_json'), 'top_beliefs_json must not be selected in getPublicProfile');
  });

  test('D-216A: getPublicProfile does not select dominant_pattern', () => {
    assert.ok(!wpCode.includes('dominant_pattern'), 'dominant_pattern must not be selected in getPublicProfile');
  });

  test('D-216A: getPublicProfile does not expose reflection_avatar field', () => {
    assert.ok(!wpCode.includes('reflection_avatar'), 'reflection_avatar must not appear in getPublicProfile');
  });

  test('D-216A: getPublicProfile does not expose avatar_hidden field', () => {
    assert.ok(!wpCode.includes('avatar_hidden'), 'avatar_hidden must not appear in getPublicProfile response');
  });

  test('D-216A: getPublicProfile does not expose alignment_labels field', () => {
    assert.ok(!wpCode.includes('alignment_labels'), 'alignment_labels must not appear in getPublicProfile response');
  });

  test('D-216A: getPublicProfile returns slug in public profile shape', () => {
    assert.ok(wpSlice.includes('slug:'), 'getPublicProfile must return slug in public profile shape');
  });

  test('D-216A: getPublicProfile returns displayName in public profile shape', () => {
    assert.ok(wpSlice.includes('displayName:'), 'getPublicProfile must return displayName in public profile shape');
  });

  test('D-216A: getPublicProfile returns sharedSnapshot via buildPublicSharedSnapshot', () => {
    assert.ok(wpSlice.includes('buildPublicSharedSnapshot('), 'getPublicProfile must use buildPublicSharedSnapshot for snapshot shaping');
  });

  test('D-216A: getPublicProfile does not return raw home_tests', () => {
    assert.ok(!wpSlice.includes('home_tests'), 'getPublicProfile must not return home_tests (private activity)');
  });

  test('D-216A: getPublicProfile does not return raw belief_snapshots array', () => {
    // Should use the single gated sharedSnapshot, not the full array
    assert.ok(!wpCode.includes('belief_snapshots:'), 'getPublicProfile must not return raw belief_snapshots array');
  });

  // ── 6. Deploy integrity ───────────────────────────────────────────────────
  test('D-216A allowlist contract does not modify public/app-v10.js', () => {
    assert.ok(!appSrc.includes('D-216A:'), 'D-216A must not modify app-v10.js');
  });

  test('D-216A allowlist contract does not modify worker.js', () => {
    assert.ok(!workerSrc.includes('D-216A'), 'D-216A must not modify worker.js');
  });

  test('D-216A allowlist contract does not require migration', () => {
    assert.ok(
...
// ── D-218A: Worker route warning audit maintainability ────────────────────────
{
  const checkerSrc = readFileSync(path.join(__dirname, '../scripts/worker-route-static-check.mjs'), 'utf8');
  const readmeSrc = readFileSync(path.join(__dirname, '../docs/README.md'), 'utf8');
  const appSrc = readFileSync(path.join(__dirname, '../public/app-v10.js'), 'utf8');
  const workerSrc = readFileSync(path.join(__dirname, '../src/worker.js'), 'utf8');

  test('D-218A: KNOWN_PARAM_ROUTES constant defined in worker-route-static-check.mjs', () => {
    assert.ok(checkerSrc.includes('KNOWN_PARAM_ROUTES'), 'KNOWN_PARAM_ROUTES must be defined in worker-route-static-check.mjs');
  });

  test('D-218A: /api/u/:slug listed in KNOWN_PARAM_ROUTES', () => {
    assert.ok(checkerSrc.includes("'/api/u/:slug'"), '/api/u/:slug must be in KNOWN_PARAM_ROUTES');
  });

  test('D-218A: /api/claims/:id listed in KNOWN_PARAM_ROUTES', () => {
    assert.ok(checkerSrc.includes("'/api/claims/:id'"), '/api/claims/:id must be in KNOWN_PARAM_ROUTES');
  });

  test('D-218A: checker distinguishes known from NEW unknown parameterised routes', () => {
    assert.ok(checkerSrc.includes('NEW parameterised route not in KNOWN_PARAM_ROUTES'), 'checker must emit different warning for unknown parameterised routes');
  });

  test('D-218A: D-218A documented limitation referenced in checker', () => {
    assert.ok(checkerSrc.includes('D-218A'), 'D-218A must be referenced in worker-route-static-check.mjs');
  });

  test('D-218A: README references D218A warning audit doc', () => {
    assert.ok(readmeSrc.includes('D218A'), 'README must reference D-218A worker route warning audit');
  });

  test('D-218A: D-218A does not modify public/app-v10.js', () => {
    assert.ok(!appSrc.includes('D-218A'), 'D-218A must not modify app-v10.js');
  });

  test('D-218A: D-218A does not modify src/worker.js', () => {
    assert.ok(!workerSrc.includes('D-218A'), 'D-218A must not modify worker.js');
  });
...
{
  const d221PubStart = appSrc.indexOf('function renderPublicProfileClaimsHtml(');
  const d221PubEnd = appSrc.indexOf('\nasync function renderPublicProfile(');
  const d221PubSrc = appSrc.slice(d221PubStart, d221PubEnd > -1 ? d221PubEnd : d221PubStart + 8000);
  const d221OrchStart = appSrc.indexOf('\nfunction renderPublicProfileHtml(');
  const d221OrchSrc = appSrc.slice(d221OrchStart, d221PubEnd > -1 ? d221PubEnd : d221OrchStart + 4000);

  test('D-221A: public profile context block uses native <details>/<summary> (keyboard/screen-reader accessible)', () => {
    assert.ok(d221OrchSrc.includes('<details'), 'renderPublicProfileHtml must use <details> for context block');
    assert.ok(d221OrchSrc.includes('<summary'), 'renderPublicProfileHtml must use <summary> for context block toggle');
  });

  test('D-221A: public profile disclosure summary has focus-visible CSS', () => {
    assert.ok(cssSrc.includes('.pp-vocab-summary:focus-visible'), 'CSS must define .pp-vocab-summary:focus-visible');
  });

  test('D-221A: public claim action row remains present and keyboard reachable', () => {
    assert.ok(d221PubSrc.includes('pp-item-actions'), 'pp-item-actions must be present in public claim render');
    assert.ok(d221PubSrc.includes('class="btn-mini"'), 'btn-mini buttons must be inside pp-item-actions');
  });

  test('D-221A: public action button text is accessible and descriptive', () => {
    assert.ok(d221PubSrc.includes('View in HumanX'), 'claim action button must have descriptive text');
    assert.ok(d221PubSrc.includes('Copy link'), 'claim copy button must have descriptive text');
  });

  test('D-221A: pp-item-actions btn-mini has scoped focus-visible CSS (D-221A new)', () => {
    assert.ok(cssSrc.includes('.pp-item-actions .btn-mini:focus-visible'), 'CSS must define .pp-item-actions .btn-mini:focus-visible');
  });

  test('D-221A: pp-item-actions btn-mini mobile touch target min-height:44px added (D-221A new)', () => {
    assert.ok(cssSrc.includes('.pp-item-actions .btn-mini{min-height:44px'), 'mobile media query must set min-height:44px on .pp-item-actions .btn-mini');
  });

  test('D-221A: empty states do not imply private/hidden data (public-safe wording)', () => {
    assert.ok(d221PubSrc.includes('No public claims yet.'), 'claims empty state must not imply private data');
    assert.ok(d221PubSrc.includes('No public truths on this profile yet.'), 'truths empty state must not imply private data');
    assert.ok(!d221PubSrc.includes('"hidden"') && !d221PubSrc.includes('>hidden<'), 'empty states must not use hidden wording as content');
  });

  test('D-221A: no private My HumanX wording in public profile render path (confirms D-215A still active)', () => {
    assert.ok(!d221PubSrc.includes('My HumanX'), 'public render path must not reference My HumanX');
    assert.ok(!d221PubSrc.includes('localStorage'), 'public render path must not reference localStorage');
  });

  test('D-221A: no Reflection Avatar / hide-show in public render path (confirms D-214A still active)', () => {
    assert.ok(!d221PubSrc.includes('meReflectionAvatarHtml'), 'public render path must not reference Reflection Avatar html');
    assert.ok(!d221PubSrc.includes('isMeReflectionAvatarHidden'), 'public render path must not reference avatar hide state');
  });

  test('D-221A: no new private field reads in public render path', () => {
    assert.ok(!d221PubSrc.includes('.ideologyType'), 'D-221A must not add ideology field to public render');
    assert.ok(!d221PubSrc.includes('.alignmentLabel'), 'D-221A must not add alignment label to public render');
    assert.ok(!d221PubSrc.includes('top_beliefs_json'), 'D-221A must not expose top_beliefs_json in public render');
    assert.ok(!d221PubSrc.includes('alignment_labels'), 'D-221A must not reference alignment_labels in public render');
  });

  test('D-221A: deploy integrity — D-221A absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-221A'), 'D-221A must not modify worker.js');
  });

  test('D-221A: README references D221A', () => {
    assert.ok(readmeSrc.includes('D221A'), 'README must reference D-221A accessibility polish');
  });
}

// ── D-222A: Public Profile copy link affordance ───────────────────────────────
{
  const d222OrchStart = appSrc.indexOf('\nfunction renderPublicProfileHtml(');
  const d222OrchEnd = appSrc.indexOf('\nasync function renderPublicProfile(');
  const d222OrchSrc = appSrc.slice(d222OrchStart, d222OrchEnd > -1 ? d222OrchEnd : d222OrchStart + 4000);
  const d222FnSrc = appSrc.match(/function copyPublicProfileLink[\s\S]*?^function /m)?.[0] || '';

  test('D-222A: public profile header renders "Copy profile link" button', () => {
    assert.ok(d222OrchSrc.includes('pp-copy-link'), 'renderPublicProfileHtml must render pp-copy-link button in header');
    assert.ok(d222OrchSrc.includes('Copy profile link'), 'public profile header must contain "Copy profile link" text');
  });

  test('D-222A: copy profile link button has type="button"', () => {
    assert.ok(d222OrchSrc.includes('type="button"'), 'copy profile link button must have type="button" for accessibility');
  });

  test('D-222A: copyPublicProfileLink uses window.location.href (not slug-constructed URL)', () => {
    assert.ok(d222FnSrc.includes('window.location.href'), 'copyPublicProfileLink must use window.location.href');
  });

  test('D-222A: copyPublicProfileLink shows "Link copied" on success', () => {
    assert.ok(d222FnSrc.includes("'Link copied'"), 'copyPublicProfileLink must show "Link copied" on success');
  });

  test('D-222A: copyPublicProfileLink shows failure message when clipboard blocked', () => {
    assert.ok(d222FnSrc.includes('Copy failed'), 'copyPublicProfileLink must show failure text when clipboard is unavailable');
  });

  test('D-222A: copyPublicProfileLink does not use localStorage', () => {
    assert.ok(!d222FnSrc.includes('localStorage'), 'copyPublicProfileLink must not use localStorage');
  });

  test('D-222A: copyPublicProfileLink does not call backend', () => {
    assert.ok(!d222FnSrc.includes('fetch(') && !d222FnSrc.includes('await api('), 'copyPublicProfileLink must not call fetch or api()');
  });

  test('D-222A: copyPublicProfileLink has try/catch failure handling', () => {
    assert.ok(d222FnSrc.includes('catch'), 'copyPublicProfileLink must have catch/failure handling');
  });

  test('D-222A: D-216A allowlist updated with D-222A public classes and copy', () => {
    const smokeSelf = readFileSync(path.join(__dirname, 'hardening-smoke-test.mjs'), 'utf8');
    assert.ok(smokeSelf.includes("'pp-copy-link'"), 'PUBLIC_PROFILE_ALLOWED_MARKERS must include pp-copy-link (D-222A)');
    assert.ok(smokeSelf.includes("'Link copied'"), 'PUBLIC_PROFILE_ALLOWED_MARKERS must include "Link copied" (D-222A)');
  });

  test('D-222A: no private My HumanX wording in public profile orchestrator (confirms D-215A still active)', () => {
    assert.ok(!d222OrchSrc.includes('localStorage'), 'public profile orchestrator must not reference localStorage');
    assert.ok(!d222OrchSrc.includes('renderMeHtml'), 'public profile orchestrator must not reference renderMeHtml');
  });

  test('D-222A: pp-copy-link CSS class defined', () => {
    const cssSrc2 = readFileSync(path.join(__dirname, '../public/styles.css'), 'utf8');
    assert.ok(cssSrc2.includes('.pp-copy-link'), 'CSS must define .pp-copy-link for header copy button');
    assert.ok(cssSrc2.includes('.pp-copy-link:focus-visible'), 'CSS must define .pp-copy-link:focus-visible');
  });

  test('D-222A: deploy integrity — D-222A absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-222A'), 'D-222A must not modify worker.js');
  });

  test('D-222A: README references D222A', () => {
    assert.ok(readmeSrc.includes('D222A'), 'README must reference D-222A copy link affordance');
  });
}

// ── D-223A: Public Profile section navigation ─────────────────────────────────
{
  const d223OrchStart = appSrc.indexOf('\nfunction renderPublicProfileHtml(');
  const d223OrchEnd = appSrc.indexOf('\nasync function renderPublicProfile(');
  const d223OrchSrc = appSrc.slice(d223OrchStart, d223OrchEnd > -1 ? d223OrchEnd : d223OrchStart + 4000);
  const d223SnapStart = appSrc.indexOf('function renderPublicProfileSnapshotHtml(');
  const d223SnapEnd = appSrc.indexOf('\nfunction copyPublicProfileLink(');
  const d223SnapSrc = appSrc.slice(d223SnapStart, d223SnapEnd > -1 ? d223SnapEnd : d223SnapStart + 1000);

  test('D-223A: public profile renders <nav> with accessible label', () => {
    assert.ok(d223OrchSrc.includes('<nav class="pp-section-nav"'), 'renderPublicProfileHtml must render .pp-section-nav');
    assert.ok(d223OrchSrc.includes('aria-label="Public profile sections"'), 'section nav must have aria-label="Public profile sections"');
  });

  test('D-223A: section nav links point only to public section anchors', () => {
    assert.ok(d223OrchSrc.includes('href="#public-claims"'), 'section nav must link to #public-claims');
    assert.ok(d223OrchSrc.includes('href="#public-truths"'), 'section nav must link to #public-truths');
    assert.ok(d223OrchSrc.includes('href="#public-about"'), 'section nav must link to #public-about');
  });

  test('D-223A/D-224A: snapshot nav link always present in section nav (D-224A: removed conditional — snapshot always has anchor target)', () => {
    assert.ok(d223OrchSrc.includes('href="#public-snapshot"'), 'snapshot nav link must link to #public-snapshot');
    assert.ok(!d223OrchSrc.includes("sn?'<a href=\"#public-snapshot\""), 'D-224A: snapshot nav link must not be conditional on sn — always rendered');
  });

  test('D-223A: id="public-claims" present on claims section', () => {
    assert.ok(d223OrchSrc.includes('id="public-claims"'), 'claims section must have id="public-claims"');
  });

  test('D-223A: id="public-truths" present on truths section', () => {
    assert.ok(d223OrchSrc.includes('id="public-truths"'), 'truths section must have id="public-truths"');
  });

  test('D-223A: id="public-about" present on context block', () => {
    assert.ok(d223OrchSrc.includes('id="public-about"'), 'context block must have id="public-about"');
  });

  test('D-223A: id="public-snapshot" present on snapshot render function', () => {
    assert.ok(d223SnapSrc.includes('id="public-snapshot"'), 'renderPublicProfileSnapshotHtml must set id="public-snapshot" on its root div');
  });

  test('D-223A: section nav has focus-visible CSS (.pp-nav-link:focus-visible)', () => {
    assert.ok(cssSrc.includes('.pp-nav-link:focus-visible'), 'CSS must define .pp-nav-link:focus-visible');
  });

  test('D-223A: section nav CSS wraps on mobile', () => {
    assert.ok(cssSrc.includes('pp-section-nav') && cssSrc.includes('flex-wrap:wrap'), 'pp-section-nav must use flex-wrap:wrap for mobile wrapping');
  });

  test('D-223A: section nav uses pure HTML anchors (no JS handlers on nav)', () => {
    const navMatch = d223OrchSrc.match(/sectionNav=`[\s\S]*?`/)?.[0] || '';
    assert.ok(!navMatch.includes('onclick') && !navMatch.includes('addEventListener') && !navMatch.includes('localStorage'), 'sectionNav must not contain JS handlers or localStorage');
  });

  test('D-223A: no private My HumanX wording in public profile orchestrator', () => {
    assert.ok(!d223OrchSrc.includes('localStorage'), 'public profile orchestrator must not reference localStorage');
    assert.ok(!d223OrchSrc.includes('renderMeHtml'), 'public profile orchestrator must not reference renderMeHtml');
  });

  test('D-223A: deploy integrity — D-223A absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-223A'), 'D-223A must not modify worker.js');
  });

  test('D-223A: README references D223A', () => {
    assert.ok(readmeSrc.includes('D223A'), 'README must reference D-223A section navigation');
  });
}

// ── D-224A: Public Profile empty-state polish ─────────────────────────────────
{
  const d224SnapStart = appSrc.indexOf('function renderPublicProfileSnapshotHtml(');
  const d224SnapEnd = appSrc.indexOf('\nfunction copyPublicProfileLink(');
  const d224SnapSrc = appSrc.slice(d224SnapStart, d224SnapEnd > -1 ? d224SnapEnd : d224SnapStart + 1200);

  const d224ClaimsStart = appSrc.indexOf('function renderPublicProfileClaimsHtml');
  const d224ClaimsSrc = appSrc.slice(d224ClaimsStart, d224ClaimsStart + 250);

  const d224TruthsStart = appSrc.indexOf('function renderPublicProfileTruthsHtml');
  const d224TruthsSrc = appSrc.slice(d224TruthsStart, d224TruthsStart + 200);

  const d224OrchStart = appSrc.indexOf('\nfunction renderPublicProfileHtml(');
  const d224OrchEnd = appSrc.indexOf('\nasync function renderPublicProfile(');
  const d224OrchSrc = appSrc.slice(d224OrchStart, d224OrchEnd > -1 ? d224OrchEnd : d224OrchStart + 5000);

  test('D-224A: snapshot empty-state renders id="public-snapshot" when no snapshot', () => {
    assert.ok(d224SnapSrc.includes('id="public-snapshot"'), 'renderPublicProfileSnapshotHtml must always emit id="public-snapshot" (both empty and real states)');
    const emptyReturn = d224SnapSrc.match(/if\(!s\)return'([^']+)'/)?.[1] || '';
    assert.ok(emptyReturn.includes('id="public-snapshot"'), 'renderPublicProfileSnapshotHtml empty-state return must include id="public-snapshot"');
  });

  test('D-224A: snapshot empty-state uses pp-empty-card class', () => {
    const emptyReturn = d224SnapSrc.match(/if\(!s\)return'([^']+)'/)?.[1] || '';
    assert.ok(emptyReturn.includes('pp-empty-card'), 'snapshot empty-state must use pp-empty-card class');
  });

  test('D-224A: snapshot empty-state copy is public-safe — "No public snapshot shared yet."', () => {
    const emptyReturn = d224SnapSrc.match(/if\(!s\)return'([^']+)'/)?.[1] || '';
    assert.ok(emptyReturn.includes('No public snapshot shared yet.'), 'snapshot empty state must say "No public snapshot shared yet."');
    assert.ok(!emptyReturn.includes('private') && !emptyReturn.includes('hidden'), 'snapshot empty state must not imply private or hidden data');
  });

  test('D-224A: snapshot empty-state secondary note uses pp-empty-note and public-safe copy', () => {
    const emptyReturn = d224SnapSrc.match(/if\(!s\)return'([^']+)'/)?.[1] || '';
    assert.ok(emptyReturn.includes('pp-empty-note'), 'snapshot empty-state must include pp-empty-note element');
    assert.ok(emptyReturn.includes('Public sections appear here when shared.'), 'snapshot empty-state note must say "Public sections appear here when shared."');
  });

  test('D-224A: claims empty-state uses pp-empty-card class', () => {
    assert.ok(d224ClaimsSrc.includes('pp-empty-card'), 'renderPublicProfileClaimsHtml empty state must use pp-empty-card class');
  });

  test('D-224A: claims empty-state copy is public-safe — "No public claims yet."', () => {
    assert.ok(d224ClaimsSrc.includes('No public claims yet.'), 'claims empty state must say "No public claims yet."');
    assert.ok(!d224ClaimsSrc.includes('private') && !d224ClaimsSrc.includes('hidden'), 'claims empty state must not imply private or hidden data');
  });

  test('D-224A: truths empty-state uses pp-empty-card class', () => {
    assert.ok(d224TruthsSrc.includes('pp-empty-card'), 'renderPublicProfileTruthsHtml empty state must use pp-empty-card class');
  });

  test('D-224A: truths empty-state copy is public-safe — "No public truths on this profile yet."', () => {
    assert.ok(d224TruthsSrc.includes('No public truths on this profile yet.'), 'truths empty state must say "No public truths on this profile yet."');
    assert.ok(!d224TruthsSrc.includes('private') && !d224TruthsSrc.includes('hidden'), 'truths empty state must not imply private or hidden data');
  });

  test('D-224A: empty states do not contain forbidden wording', () => {
    const allEmpty = d224SnapSrc + d224ClaimsSrc + d224TruthsSrc;
    const forbidden = ['truth level', 'purity', 'ideology', 'smart score', 'HumanX rank', 'good believer', 'bad believer', 'diagnosis', 'localStorage', 'My HumanX'];
    for (const word of forbidden) {
      assert.ok(!allEmpty.slice(0, 600).includes(word), `empty states must not contain forbidden wording: "${word}"`);
    }
  });

  test('D-224A: snapshot section nav link is always present (not conditional on sn)', () => {
    assert.ok(d224OrchSrc.includes('href="#public-snapshot"'), 'section nav must always link to #public-snapshot');
    assert.ok(!d224OrchSrc.includes("sn?'<a href=\"#public-snapshot\""), 'D-224A: snapshot nav link must not be conditional on sn');
  });

  test('D-224A: pp-empty-card CSS defined', () => {
    assert.ok(cssSrc.includes('.pp-empty-card'), 'CSS must define .pp-empty-card');
    assert.ok(cssSrc.includes('text-align:center'), 'pp-empty-card must include text-align:center for calm presentation');
  });

  test('D-224A: pp-empty-title and pp-empty-note CSS defined', () => {
    assert.ok(cssSrc.includes('.pp-empty-title'), 'CSS must define .pp-empty-title');
    assert.ok(cssSrc.includes('.pp-empty-note'), 'CSS must define .pp-empty-note');
  });

  test('D-224A: deploy integrity — D-224A absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-224A'), 'D-224A must not modify worker.js');
  });
}

// ── D-225A: Public Profile polish regression lock (D-220 → D-224 arc) ──────────
{
  // Shared slices for arc lock tests
  const d225PubStart = appSrc.indexOf('function renderPublicProfileClaimsHtml(');
  const d225PubEnd = appSrc.indexOf('\nasync function renderPublicProfile(');
  const d225PubSrc = appSrc.slice(d225PubStart, d225PubEnd > -1 ? d225PubEnd : d225PubStart + 12000);

  const d225OrchStart = appSrc.indexOf('\nfunction renderPublicProfileHtml(');
  const d225OrchSrc = appSrc.slice(d225OrchStart, d225PubEnd > -1 ? d225PubEnd : d225OrchStart + 5000);

  const d225CopyFnSrc = appSrc.match(/function copyPublicProfileLink[\s\S]*?^function /m)?.[0] || '';

  const d225SnapStart = appSrc.indexOf('function renderPublicProfileSnapshotHtml(');
  const d225SnapEnd = appSrc.indexOf('\nfunction copyPublicProfileLink(');
  const d225SnapSrc = appSrc.slice(d225SnapStart, d225SnapEnd > -1 ? d225SnapEnd : d225SnapStart + 1500);

  test('D-225A lock: D-220→D-224 page structure order — counts, sectionNav, snapshot, claims all in correct order in return template', () => {
    const returnStart = d225OrchSrc.indexOf('return`');
    const tpl = d225OrchSrc.slice(returnStart);
    const countsPos = tpl.indexOf('${countsCard}');
    const navPos = tpl.indexOf('${sectionNav}');
    const snapPos = tpl.indexOf('renderPublicProfileSnapshotHtml(');
    const claimsPos = tpl.indexOf('id="public-claims"');
    assert.ok(countsPos > -1 && navPos > -1 && snapPos > -1 && claimsPos > -1, 'return template must contain countsCard, sectionNav, snapshot, and claims');
    assert.ok(countsPos < navPos, 'countsCard must precede sectionNav in return template');
    assert.ok(navPos < snapPos, 'sectionNav must precede renderPublicProfileSnapshotHtml in return template');
    assert.ok(snapPos < claimsPos, 'snapshot must precede claims section in return template');
  });

  test('D-225A lock: all D-220→D-224 CSS classes present in styles.css', () => {
    // D-220A classes
    assert.ok(cssSrc.includes('.pp-vocab-details'), 'D-220A: .pp-vocab-details must still be in CSS');
    assert.ok(cssSrc.includes('.pp-item-actions'), 'D-220A: .pp-item-actions must still be in CSS');
    // D-221A additions
    assert.ok(cssSrc.includes('.pp-item-actions .btn-mini:focus-visible'), 'D-221A: .pp-item-actions .btn-mini:focus-visible must still be in CSS');
    // D-222A additions
    assert.ok(cssSrc.includes('.pp-copy-link'), 'D-222A: .pp-copy-link must still be in CSS');
    // D-223A additions
    assert.ok(cssSrc.includes('.pp-section-nav'), 'D-223A: .pp-section-nav must still be in CSS');
    assert.ok(cssSrc.includes('.pp-nav-link'), 'D-223A: .pp-nav-link must still be in CSS');
    // D-224A additions
    assert.ok(cssSrc.includes('.pp-empty-card'), 'D-224A: .pp-empty-card must still be in CSS');
    assert.ok(cssSrc.includes('.pp-empty-title'), 'D-224A: .pp-empty-title must still be in CSS');
    assert.ok(cssSrc.includes('.pp-empty-note'), 'D-224A: .pp-empty-note must still be in CSS');
  });

  test('D-225A lock: D-222 copy-link helper integrity — window.location.href, Link copied, failure copy, no localStorage, no fetch', () => {
    assert.ok(d225CopyFnSrc.includes('window.location.href'), 'D-222A lock: copyPublicProfileLink must use window.location.href');
    assert.ok(d225CopyFnSrc.includes("'Link copied'"), 'D-222A lock: success text must be "Link copied"');
    assert.ok(d225CopyFnSrc.includes('Copy failed'), 'D-222A lock: failure copy must still be present');
    assert.ok(d225CopyFnSrc.includes('use browser address bar'), 'D-222A lock: failure copy must say "use browser address bar"');
    assert.ok(!d225CopyFnSrc.includes('localStorage'), 'D-222A lock: copyPublicProfileLink must not use localStorage');
    assert.ok(!d225CopyFnSrc.includes('fetch(') && !d225CopyFnSrc.includes('await api('), 'D-222A lock: copyPublicProfileLink must not call fetch or api()');
  });

  test('D-225A lock: D-223 section nav — all four public anchors always present, no JS', () => {
    const navMatch = d225OrchSrc.match(/sectionNav=`[\s\S]*?`/)?.[0] || '';
    assert.ok(navMatch.includes('href="#public-snapshot"'), 'D-223/D-224 lock: Snapshot nav link must always be present');
    assert.ok(navMatch.includes('href="#public-claims"'), 'D-223 lock: Claims nav link must be present');
    assert.ok(navMatch.includes('href="#public-truths"'), 'D-223 lock: Truths nav link must be present');
    assert.ok(navMatch.includes('href="#public-about"'), 'D-223 lock: About nav link must be present');
    assert.ok(!navMatch.includes('onclick') && !navMatch.includes('addEventListener'), 'D-223 lock: sectionNav must use pure HTML anchors only');
  });

  test('D-225A lock: D-224 empty-state — snapshot id always emitted, claims/truths use pp-empty-card', () => {
    // Snapshot: id always in function (both empty and real return paths)
    assert.ok(d225SnapSrc.includes('id="public-snapshot"'), 'D-224 lock: id="public-snapshot" must always be emitted by renderPublicProfileSnapshotHtml');
    // Claims and truths: pp-empty-card class
    const claimsEmptySlice = appSrc.slice(appSrc.indexOf('function renderPublicProfileClaimsHtml'), appSrc.indexOf('function renderPublicProfileClaimsHtml') + 250);
    assert.ok(claimsEmptySlice.includes('pp-empty-card'), 'D-224 lock: claims empty-state must use pp-empty-card');
    const truthsEmptySlice = appSrc.slice(appSrc.indexOf('function renderPublicProfileTruthsHtml'), appSrc.indexOf('function renderPublicProfileTruthsHtml') + 200);
    assert.ok(truthsEmptySlice.includes('pp-empty-card'), 'D-224 lock: truths empty-state must use pp-empty-card');
  });

  test('D-225A lock: D-220→D-224 public allowlist contains all arc markers', () => {
    const smokeSelf = readFileSync(path.join(__dirname, 'hardening-smoke-test.mjs'), 'utf8');
    // D-220A markers
    assert.ok(smokeSelf.includes("'pp-item-actions'"), 'D-225A lock: allowlist must include pp-item-actions');
    assert.ok(smokeSelf.includes("'pp-vocab-details'"), 'D-225A lock: allowlist must include pp-vocab-details');
    // D-222A markers
    assert.ok(smokeSelf.includes("'pp-copy-link'"), 'D-225A lock: allowlist must include pp-copy-link');
    assert.ok(smokeSelf.includes("'Copy profile link'"), 'D-225A lock: allowlist must include "Copy profile link"');
    assert.ok(smokeSelf.includes("'Link copied'"), 'D-225A lock: allowlist must include "Link copied"');
    // D-223A markers
    assert.ok(smokeSelf.includes("'pp-section-nav'"), 'D-225A lock: allowlist must include pp-section-nav');
    assert.ok(smokeSelf.includes("'pp-nav-link'"), 'D-225A lock: allowlist must include pp-nav-link');
    assert.ok(smokeSelf.includes("'public-snapshot'"), 'D-225A lock: allowlist must include public-snapshot');
    assert.ok(smokeSelf.includes("'public-claims'"), 'D-225A lock: allowlist must include public-claims');
    assert.ok(smokeSelf.includes("'public-truths'"), 'D-225A lock: allowlist must include public-truths');
    assert.ok(smokeSelf.includes("'public-about'"), 'D-225A lock: allowlist must include public-about');
    // D-224A markers
    assert.ok(smokeSelf.includes("'pp-empty-card'"), 'D-225A lock: allowlist must include pp-empty-card');
    assert.ok(smokeSelf.includes("'pp-empty-title'"), 'D-225A lock: allowlist must include pp-empty-title');
    assert.ok(smokeSelf.includes("'pp-empty-note'"), 'D-225A lock: allowlist must include pp-empty-note');
  });

  test('D-225A lock: privacy boundary — no private markers in entire public render path (D-220→D-224 arc)', () => {
    const privateMarkers = [
      'meReflectionAvatarHtml', 'isMeReflectionAvatarHidden', 'meRerender',
      'humanx.me.reflectionAvatar.hidden', 'renderMeHtml', 'meMirrorHtml',
      'meSharedSnapshotPreviewBlockHtml', 'meAccountCardHtml',
    ];
    for (const marker of privateMarkers) {
      assert.ok(!d225PubSrc.includes(marker), `D-225A lock: public render path must not contain private marker "${marker}"`);
    }
    assert.ok(!d225PubSrc.includes('localStorage'), 'D-225A lock: public render path must not reference localStorage');
  });

  test('D-225A lock: forbidden public wording absent from all public render functions', () => {
    const forbidden = [
      'truth level', 'purity', 'ideology type', 'religious alignment',
      'smart score', 'HumanX rank', 'good believer', 'bad believer',
    ];
    for (const phrase of forbidden) {
      assert.ok(!d225PubSrc.includes(phrase), `D-225A lock: forbidden wording must be absent from public render path: "${phrase}"`);
    }
  });

  test('D-225A lock: D-221 accessibility — focus-visible rings and mobile touch targets both present for public action buttons', () => {
    assert.ok(cssSrc.includes('.pp-item-actions .btn-mini:focus-visible'), 'D-221A lock: focus-visible ring must still be defined for public claim action buttons');
    assert.ok(cssSrc.includes('.pp-item-actions .btn-mini{min-height:44px'), 'D-221A lock: mobile touch target must still be defined for public claim action buttons');
    assert.ok(cssSrc.includes('.pp-copy-link:focus-visible'), 'D-222A lock: focus-visible ring must still be defined for pp-copy-link');
    assert.ok(cssSrc.includes('.pp-nav-link:focus-visible'), 'D-223A lock: focus-visible ring must still be defined for pp-nav-link');
  });

  test('D-225A lock: D-225A itself does not modify app-v10.js, styles.css, or worker.js', () => {
    assert.ok(!workerSrc.includes('D-225A'), 'D-225A must not appear in worker.js — it is tests+docs only');
    // Verify the lock is docs+test only by checking for D-225A in the smoke test itself (expected)
    // and NOT in worker or app source
    const appSrcCheck = readFileSync(path.join(__dirname, '../public/app-v10.js'), 'utf8');
    assert.ok(!appSrcCheck.includes('D-225A'), 'D-225A must not appear in app-v10.js — it is tests+docs only');
  });

  test('D-225A lock: README references all D-220→D-224 arc docs and D-225A', () => {
    assert.ok(readmeSrc.includes('D220A'), 'D-225A lock: README must reference D-220A visual polish');
    assert.ok(readmeSrc.includes('D221A'), 'D-225A lock: README must reference D-221A accessibility');
    assert.ok(readmeSrc.includes('D222A'), 'D-225A lock: README must reference D-222A copy link');
    assert.ok(readmeSrc.includes('D223A'), 'D-225A lock: README must reference D-223A section nav');
    assert.ok(readmeSrc.includes('D224A'), 'D-225A lock: README must reference D-224A empty states');
    assert.ok(readmeSrc.includes('D225A'), 'D-225A lock: README must reference D-225A regression lock');
  });

  test('D-225A lock: D-220 context block — <details>/<summary> disclosure and counts-before-snapshot order still intact', () => {
    assert.ok(d225OrchSrc.includes('<details'), 'D-220A lock: context block must still use <details> element');
    assert.ok(d225OrchSrc.includes('About this profile page'), 'D-220A lock: "About this profile page" summary must still be present');
    assert.ok(d225PubSrc.includes('pp-item-actions'), 'D-220A lock: pp-item-actions must still be present in claims render');
    const returnStart = d225OrchSrc.indexOf('return`');
    const tpl = d225OrchSrc.slice(returnStart);
    assert.ok(tpl.indexOf('${countsCard}') < tpl.indexOf('renderPublicProfileSnapshotHtml('), 'D-220A lock: counts must still precede snapshot in return template');
  });

  test('D-225A lock: deploy integrity — no D-220→D-224 arc change touches worker.js or migrations', () => {
    const arcTags = ['D-220A', 'D-221A', 'D-222A', 'D-223A', 'D-224A', 'D-225A'];
    for (const tag of arcTags) {
      assert.ok(!workerSrc.includes(tag), `D-225A deploy lock: ${tag} must not appear in worker.js`);
    }
  });

  test('D-225A lock: empty-state copy — all three public-safe messages present in render path', () => {
    assert.ok(d225PubSrc.includes('No public snapshot shared yet.'), 'D-225A lock: snapshot empty-state copy must be present');
    assert.ok(d225PubSrc.includes('Public sections appear here when shared.'), 'D-225A lock: snapshot empty-state note must be present');
    assert.ok(d225PubSrc.includes('No public claims yet.'), 'D-225A lock: claims empty-state copy must be present');
    assert.ok(d225PubSrc.includes('No public truths on this profile yet.'), 'D-225A lock: truths empty-state copy must be present');
    // Confirm no private/hidden wording in any empty-state copy
    const emptyReturn = d225SnapSrc.match(/if\(!s\)return'([^']+)'/)?.[1] || '';
    assert.ok(!emptyReturn.includes('private') && !emptyReturn.includes('hidden'), 'D-225A lock: snapshot empty-state must not imply private/hidden data');
  });

  test('D-225A lock: copy-link button is present in public profile header for all profiles (owner and non-owner)', () => {
    assert.ok(d225OrchSrc.includes('pp-copy-link'), 'D-225A lock: pp-copy-link button must still be present in public profile header');
    assert.ok(d225OrchSrc.includes('Copy profile link'), 'D-225A lock: "Copy profile link" label must still be in header');
    assert.ok(d225OrchSrc.includes('data-action="copyPublicProfileLink"'), 'D-225A lock: header copy button must still wire to copyPublicProfileLink action');
  });
}
...
{
  const cardFnStart = appSrc.indexOf('function reviewCard(item)');
  const cardFnEnd = appSrc.indexOf('\nfunction ', cardFnStart + 1);
  const cardSrc = cardFnEnd > cardFnStart ? appSrc.slice(cardFnStart, cardFnEnd) : appSrc.slice(cardFnStart, cardFnStart + 4000);

  const inspectFnStart = appSrc.indexOf('function inspectReviewItem(id)');
  const inspectFnEnd = appSrc.indexOf('\nfunction ', inspectFnStart + 1);
  const inspectSrc = inspectFnEnd > inspectFnStart ? appSrc.slice(inspectFnStart, inspectFnEnd) : appSrc.slice(inspectFnStart, inspectFnStart + 500);

  const scrollFnStart = appSrc.indexOf('function scrollSelectedReviewCardIntoView()');
  const scrollFnEnd = appSrc.indexOf('\nfunction ', scrollFnStart + 1);
  const scrollSrc = scrollFnEnd > scrollFnStart ? appSrc.slice(scrollFnStart, scrollFnEnd) : appSrc.slice(scrollFnStart, scrollFnStart + 200);

  test('D-227B: reviewCard emits data-review-selected="true" when isSelected', () => {
    assert.ok(
      cardSrc.includes('data-review-selected="true"') && cardSrc.includes('isSelected'),
      'reviewCard must emit data-review-selected="true" on the article when the item is inspected'
    );
  });

  test('D-227B: reviewCard data-review-selected is conditional on isSelected', () => {
    assert.ok(
      cardSrc.includes("isSelected?' data-review-selected=\"true\"':''"),
      'reviewCard must only emit data-review-selected when isSelected is true'
    );
  });

  test('D-227B: scrollSelectedReviewCardIntoView function exists', () => {
    assert.ok(scrollFnStart !== -1, 'scrollSelectedReviewCardIntoView must be defined in app-v10.js');
  });

  test('D-227B: scrollSelectedReviewCardIntoView queries data-review-selected="true"', () => {
    assert.ok(
      scrollSrc.includes('[data-review-selected="true"]'),
      'scrollSelectedReviewCardIntoView must use document.querySelector(\'[data-review-selected="true"]\')'
    );
  });

  test('D-227B: scrollSelectedReviewCardIntoView uses optional chaining (null-safe)', () => {
    assert.ok(
      scrollSrc.includes('?.scrollIntoView'),
      'scrollSelectedReviewCardIntoView must use optional chaining so absent card is a no-op'
    );
  });

  test('D-227B: scrollSelectedReviewCardIntoView uses block nearest', () => {
    assert.ok(
      scrollSrc.includes("block:'nearest'"),
      'scrollSelectedReviewCardIntoView must use block:nearest to minimise viewport jump'
    );
  });

  test('D-227B: inspectReviewItem calls requestAnimationFrame(scrollSelectedReviewCardIntoView)', () => {
    assert.ok(
      inspectSrc.includes('requestAnimationFrame(scrollSelectedReviewCardIntoView)'),
      'inspectReviewItem must schedule scrollSelectedReviewCardIntoView via requestAnimationFrame after render'
    );
  });

  test('D-227B: inspectReviewItem RAF scroll is guarded — only fires when panel is open', () => {
    assert.ok(
      inspectSrc.includes('inspectedReviewItem') && inspectSrc.includes('requestAnimationFrame'),
      'inspectReviewItem must only call requestAnimationFrame when inspectedReviewItem is truthy'
    );
  });

  test('D-227B: inspectReviewItem still scrolls inspect panel into view (regression)', () => {
    assert.ok(
      inspectSrc.includes('.review-inspect-panel') && inspectSrc.includes('scrollIntoView'),
      'inspectReviewItem must still scroll .review-inspect-panel into view (D-95B regression)'
    );
  });

  test('D-227B: scrollSelectedReviewCardIntoView exposed on window', () => {
    assert.ok(
      appSrc.includes('window.scrollSelectedReviewCardIntoView=scrollSelectedReviewCardIntoView'),
      'scrollSelectedReviewCardIntoView must be exposed on window'
    );
  });

  test('D-227B: styles.css selected card has stronger ring (box-shadow with 55 opacity)', () => {
    assert.ok(
      cssSrc.includes('review-card-selected') && cssSrc.includes('#57b8ff55'),
      'styles.css .review-card-selected must include the 2px ring (#57b8ff55)'
    );
  });

  test('D-227B: styles.css selected card has background accent', () => {
    assert.ok(
      cssSrc.includes('review-card-selected') && cssSrc.includes('rgba(87,184,255,.08)'),
      'styles.css .review-card-selected must include a subtle blue background gradient'
    );
  });

  test('D-227B: no moderation action names changed — requestApproveReview still present', () => {
    assert.ok(appSrc.includes('function requestApproveReview(id)'), 'requestApproveReview must still be present');
  });

  test('D-227B: no moderation action names changed — requestRejectReview still present', () => {
    assert.ok(appSrc.includes('function requestRejectReview(id)'), 'requestRejectReview must still be present');
  });

  test('D-227B: no moderation action names changed — reviewDecisionUI still present', () => {
    assert.ok(appSrc.includes('function reviewDecisionUI('), 'reviewDecisionUI must still be present');
  });

  test('D-227B: no backend review routes changed — /api/review/decision still referenced', () => {
    assert.ok(appSrc.includes("'/api/review/decision'"), 'review decision route must be unchanged');
  });

  test('D-227B: public profile render path does not include data-review-selected', () => {
    const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
    const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
    const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 6000);
    assert.ok(
      !ppSrc.includes('data-review-selected') && !ppSrc.includes('review-card-selected'),
      'renderPublicProfileHtml must not contain any review selected-card markers'
    );
  });

  test('D-227B: deploy integrity — D-227B tag absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-227B'), 'D-227B changes are frontend/CSS only — worker.js must not reference this tag');
  });
}
...
{
  const helperStart = appSrc.indexOf('function withReviewScrollPreserved(fn)');
  const helperEnd = appSrc.indexOf('\nfunction ', helperStart + 1);
  const helperSrc = helperEnd > helperStart ? appSrc.slice(helperStart, helperEnd) : appSrc.slice(helperStart, helperStart + 300);

  const inspectFnStart = appSrc.indexOf('function inspectReviewItem(id)');
  const inspectFnEnd = appSrc.indexOf('\nfunction ', inspectFnStart + 1);
  const inspectSrc = inspectFnEnd > inspectFnStart ? appSrc.slice(inspectFnStart, inspectFnEnd) : appSrc.slice(inspectFnStart, inspectFnStart + 500);

  test('D-228A: withReviewScrollPreserved helper exists', () => {
    assert.ok(helperStart !== -1, 'withReviewScrollPreserved must be defined in app-v10.js');
  });

  test('D-228A: helper captures window.scrollY', () => {
    assert.ok(helperSrc.includes('window.scrollY'), 'withReviewScrollPreserved must capture window.scrollY before render');
  });

  test('D-228A: helper uses requestAnimationFrame', () => {
    assert.ok(helperSrc.includes('requestAnimationFrame'), 'withReviewScrollPreserved must restore scroll via requestAnimationFrame');
  });

  test('D-228A: helper calls window.scrollTo', () => {
    assert.ok(helperSrc.includes('window.scrollTo'), 'withReviewScrollPreserved must call window.scrollTo to restore position');
  });

  test('D-228A: helper is null-safe — guards window availability', () => {
    assert.ok(
      helperSrc.includes("typeof window!=='undefined'"),
      'withReviewScrollPreserved must guard against unavailable window (SSR/test safety)'
    );
  });

  test('D-228A: setReviewFilter uses withReviewScrollPreserved', () => {
    assert.ok(
      appSrc.includes('function setReviewFilter(f){reviewStateFilter=f||\'review\';withReviewScrollPreserved(renderReviewList)}'),
      'setReviewFilter must call withReviewScrollPreserved(renderReviewList)'
    );
  });

  test('D-228A: setReviewSort uses withReviewScrollPreserved', () => {
    assert.ok(
      appSrc.includes('function setReviewSort(s){reviewSortOrder=s||\'newest\';withReviewScrollPreserved(renderReviewList)}'),
      'setReviewSort must call withReviewScrollPreserved(renderReviewList)'
    );
  });

  test('D-228A: requestRejectReview uses withReviewScrollPreserved', () => {
    assert.ok(
      appSrc.includes('withReviewScrollPreserved(renderReviewList)}') && appSrc.includes('function requestRejectReview'),
      'requestRejectReview must call withReviewScrollPreserved'
    );
  });

  test('D-228A: requestApproveReview uses withReviewScrollPreserved', () => {
    const fn = appSrc.match(/function requestApproveReview\(id\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'requestApproveReview must call withReviewScrollPreserved');
  });

  test('D-228A: cancelRejectReview uses withReviewScrollPreserved', () => {
    const fn = appSrc.match(/function cancelRejectReview\(\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'cancelRejectReview must call withReviewScrollPreserved');
  });

  test('D-228A: cancelApproveReview uses withReviewScrollPreserved', () => {
    const fn = appSrc.match(/function cancelApproveReview\(\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'cancelApproveReview must call withReviewScrollPreserved');
  });

  test('D-228A: toggleReviewAudit uses withReviewScrollPreserved', () => {
    assert.ok(
      appSrc.includes('function toggleReviewAudit(){reviewAuditOpen=!reviewAuditOpen;withReviewScrollPreserved(renderReviewList)}'),
      'toggleReviewAudit must call withReviewScrollPreserved(renderReviewList)'
    );
  });

  test('D-228A: inspectReviewItem does NOT use withReviewScrollPreserved (D-227B scroll wins)', () => {
    assert.ok(
      !inspectSrc.includes('withReviewScrollPreserved'),
      'inspectReviewItem must not use scroll preservation — D-227B selected-card scroll takes precedence'
    );
  });

  test('D-228A: inspectReviewItem still calls scrollSelectedReviewCardIntoView (D-227B regression)', () => {
    assert.ok(
      inspectSrc.includes('scrollSelectedReviewCardIntoView'),
      'inspectReviewItem must still call scrollSelectedReviewCardIntoView (D-227B must not be broken)'
    );
  });

  test('D-228A: withReviewScrollPreserved exposed on window', () => {
    assert.ok(
      appSrc.includes('window.withReviewScrollPreserved=withReviewScrollPreserved'),
      'withReviewScrollPreserved must be exposed on window'
    );
  });

  test('D-228A: no moderation action names changed — reviewDecisionUI unchanged', () => {
    assert.ok(appSrc.includes('function reviewDecisionUI('), 'reviewDecisionUI must still be present');
  });

  test('D-228A: reviewDecisionUI does not use withReviewScrollPreserved (scrollToReviewAnchor wins)', () => {
    const fn = appSrc.match(/async function reviewDecisionUI\([\s\S]*?(?=\nasync function |\nfunction )/)?.[0] || '';
    assert.ok(!fn.includes('withReviewScrollPreserved'), 'reviewDecisionUI must not use scroll preservation — scrollToReviewAnchor handles post-decision positioning');
  });

  test('D-228A: public profile render path does not include withReviewScrollPreserved', () => {
    const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
    const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
    const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 6000);
    assert.ok(
      !ppSrc.includes('withReviewScrollPreserved'),
      'renderPublicProfileHtml must not reference review scroll helper'
    );
  });

  test('D-228A: deploy integrity — D-228A tag absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-228A'), 'D-228A changes are frontend only — worker.js must not reference this tag');
  });
}
...

{
  const cardFnStart = appSrc.indexOf('function reviewCard(item)');
  const cardFnEnd = appSrc.indexOf('\nfunction ', cardFnStart + 1);
  const cardSrc = cardFnEnd > cardFnStart ? appSrc.slice(cardFnStart, cardFnEnd) : appSrc.slice(cardFnStart, cardFnStart + 5000);

  const panelFnStart = appSrc.indexOf('function renderReviewInspectPanel(item)');
  const panelFnEnd = appSrc.indexOf('\nfunction ', panelFnStart + 1);
  const panelSrc = panelFnEnd > panelFnStart ? appSrc.slice(panelFnStart, panelFnEnd) : appSrc.slice(panelFnStart, panelFnStart + 6000);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 6000);

  test('D-229A: reviewCard emits data-review-confirming="reject" when reject is armed', () => {
    assert.ok(
      cardSrc.includes('data-review-confirming="reject"'),
      'reviewCard must emit data-review-confirming="reject" when isPendingReject'
    );
  });

  test('D-229A: reviewCard emits data-review-confirming="approve" when approve is armed', () => {
    assert.ok(
      cardSrc.includes('data-review-confirming="approve"'),
      'reviewCard must emit data-review-confirming="approve" when isPendingApprove'
    );
  });

  test('D-229A: reviewCard adds review-card-approve-pending class when approve is armed', () => {
    assert.ok(
      cardSrc.includes('review-card-approve-pending'),
      'reviewCard must add review-card-approve-pending class when isPendingApprove (mirrors review-card-reject-pending)'
    );
  });

  test('D-229A: reviewCard actions div gets review-confirm-armed class when armed', () => {
    assert.ok(
      cardSrc.includes('review-confirm-armed'),
      'reviewCard actions div must include review-confirm-armed class when isPendingReject or isPendingApprove'
    );
  });

  test('D-229A: reviewCard cancel buttons still present in armed state', () => {
    assert.ok(
      cardSrc.includes('cancelRejectReview') && cardSrc.includes('cancelApproveReview'),
      'reviewCard must still render cancel buttons in both reject and approve armed states'
    );
  });

  test('D-229A: renderReviewInspectPanel emits data-review-confirming on actions div', () => {
    assert.ok(
      panelSrc.includes('data-review-confirming="approve"') &&
      panelSrc.includes('data-review-confirming="reject"') &&
      panelSrc.includes('data-review-confirming="cleanup"'),
      'renderReviewInspectPanel actions div must emit data-review-confirming for all three armed states'
    );
  });

  test('D-229A: renderReviewInspectPanel actions div gets review-confirm-armed when armed', () => {
    assert.ok(
      panelSrc.includes('review-inspect-actions') && panelSrc.includes('review-confirm-armed'),
      'renderReviewInspectPanel inspect actions div must include review-confirm-armed when any action is armed'
    );
  });

  test('D-229A: cleanup section uses review-cleanup-confirm-msg (not reject class)', () => {
    assert.ok(
      panelSrc.includes('review-cleanup-confirm-msg'),
      'renderReviewInspectPanel cleanup section must use review-cleanup-confirm-msg (not reuse review-reject-confirm-msg)'
    );
  });

  test('D-229A: cleanup section uses btn-cleanup-confirm (not btn-reject-confirm)', () => {
    assert.ok(
      panelSrc.includes('btn-cleanup-confirm'),
      'renderReviewInspectPanel cleanup confirm must use btn-cleanup-confirm (not btn-reject-confirm)'
    );
  });

  test('D-229A: cleanup cancel uses btn-cleanup-cancel (not btn-reject-cancel)', () => {
    assert.ok(
      panelSrc.includes('btn-cleanup-cancel'),
      'renderReviewInspectPanel cleanup cancel must use btn-cleanup-cancel (not btn-reject-cancel)'
    );
  });

  test('D-229A: styles.css has review-card-approve-pending', () => {
    assert.ok(cssSrc.includes('review-card-approve-pending'), 'styles.css must define .review-card-approve-pending');
  });

  test('D-229A: styles.css has review-confirm-armed', () => {
    assert.ok(cssSrc.includes('review-confirm-armed'), 'styles.css must define .review-confirm-armed');
  });

  test('D-229A: styles.css has review-cleanup-confirm-msg', () => {
    assert.ok(cssSrc.includes('review-cleanup-confirm-msg'), 'styles.css must define .review-cleanup-confirm-msg');
  });

  test('D-229A: styles.css has btn-cleanup-confirm', () => {
    assert.ok(cssSrc.includes('btn-cleanup-confirm'), 'styles.css must define .btn-cleanup-confirm');
  });

  test('D-229A: moderation backend routes unchanged — /api/review/decision present', () => {
    assert.ok(appSrc.includes("'/api/review/decision'"), '/api/review/decision route must still be referenced');
  });

  test('D-229A: reviewDecisionUI still handles public/rejected/review decisions', () => {
    assert.ok(
      appSrc.includes("'public'") && appSrc.includes("'rejected'") && appSrc.includes("'review'"),
      'reviewDecisionUI must still support all three decision outcomes'
    );
  });

  test('D-229A: D-227B data-review-selected still emitted on selected card', () => {
    assert.ok(
      cardSrc.includes('data-review-selected="true"'),
      'reviewCard must still emit data-review-selected="true" (D-227B must not be broken)'
    );
  });

  test('D-229A: D-228A scroll preservation still used in requestRejectReview', () => {
    const fn = appSrc.match(/function requestRejectReview\(id\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'requestRejectReview must still use withReviewScrollPreserved (D-228A regression)');
  });

  test('D-229A: public profile render path does not include confirm-state classes', () => {
    assert.ok(
      !ppSrc.includes('review-confirm-armed') && !ppSrc.includes('data-review-confirming'),
      'renderPublicProfileHtml must not reference review confirm-state classes'
    );
  });

  test('D-229A: deploy integrity — D-229A tag absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-229A'), 'D-229A changes are frontend/CSS only — worker.js must not reference this tag');
  });
}
...
{
  const decisionFnStart = appSrc.indexOf('async function reviewDecisionUI(');
  const decisionFnEnd = appSrc.indexOf('\nasync function ', decisionFnStart + 1);
  const decisionSrc = decisionFnEnd > decisionFnStart ? appSrc.slice(decisionFnStart, decisionFnEnd) : appSrc.slice(decisionFnStart, decisionFnStart + 2000);

  const renderListStart = appSrc.indexOf('function renderReviewList()');
  const renderListEnd = appSrc.indexOf('\nfunction ', renderListStart + 1);
  const renderListSrc = renderListEnd > renderListStart ? appSrc.slice(renderListStart, renderListEnd) : appSrc.slice(renderListStart, renderListStart + 3000);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 6000);

  test('D-230A: reviewDecisionFeedback state variable declared', () => {
    assert.ok(appSrc.includes('let reviewDecisionFeedback'), 'reviewDecisionFeedback module-level state variable must be declared');
  });

  test('D-230A: clearReviewDecisionFeedback helper exists', () => {
    assert.ok(appSrc.includes('function clearReviewDecisionFeedback'), 'clearReviewDecisionFeedback function must exist');
  });

  test('D-230A: clearReviewDecisionFeedback exposed on window', () => {
    assert.ok(appSrc.includes('window.clearReviewDecisionFeedback=clearReviewDecisionFeedback'), 'clearReviewDecisionFeedback must be exposed on window for onclick');
  });

  test('D-230A: feedback banner uses role="status" for accessibility', () => {
    assert.ok(renderListSrc.includes('role="status"'), 'renderReviewList feedback banner must use role="status"');
  });

  test('D-230A: feedback banner uses aria-live="polite"', () => {
    assert.ok(renderListSrc.includes('aria-live="polite"'), 'renderReviewList feedback banner must use aria-live="polite"');
  });

  test('D-230A: feedback dismiss button uses type="button"', () => {
    assert.ok(renderListSrc.includes('type="button"'), 'feedback dismiss button must have type="button"');
  });

  test('D-230A: feedback dismiss button calls clearReviewDecisionFeedback', () => {
    assert.ok(renderListSrc.includes('clearReviewDecisionFeedback()'), 'dismiss button onclick must call clearReviewDecisionFeedback()');
  });

  test('D-230A: approve decision sets "Approved review item." feedback', () => {
    assert.ok(decisionSrc.includes("'Approved review item.'"), 'reviewDecisionUI must set "Approved review item." feedback for public decision');
  });

  test('D-230A: reject decision sets "Rejected review item." feedback', () => {
    assert.ok(decisionSrc.includes("'Rejected review item.'"), 'reviewDecisionUI must set "Rejected review item." feedback for rejected decision');
  });

  test('D-230A: keep/review decision sets "Kept review item." feedback', () => {
    assert.ok(decisionSrc.includes("'Kept review item.'"), 'reviewDecisionUI must set "Kept review item." feedback for review/keep decision');
  });

  test('D-230A: feedback rendered in renderReviewList (review-decision-feedback class)', () => {
    assert.ok(renderListSrc.includes('review-decision-feedback'), 'renderReviewList must include review-decision-feedback element when feedback state is set');
  });

  test('D-230A: styles.css has review-decision-feedback', () => {
    assert.ok(cssSrc.includes('review-decision-feedback'), 'styles.css must define .review-decision-feedback');
  });

  test('D-230A: styles.css has review-feedback-dismiss', () => {
    assert.ok(cssSrc.includes('review-feedback-dismiss'), 'styles.css must define .review-feedback-dismiss');
  });

  test('D-230A: feedback copy absent from public profile render path', () => {
    assert.ok(
      !ppSrc.includes('Approved review item') && !ppSrc.includes('Rejected review item') && !ppSrc.includes('Kept review item'),
      'renderPublicProfileHtml must not include review decision feedback copy'
    );
  });

  test('D-230A: reviewDecisionUI still calls /api/review/decision', () => {
    assert.ok(decisionSrc.includes("'/api/review/decision'"), 'reviewDecisionUI must still POST to /api/review/decision');
  });

  test('D-230A: D-227B data-review-selected still emitted on selected card', () => {
    const cardSrc = appSrc.slice(appSrc.indexOf('function reviewCard('), appSrc.indexOf('function reviewCard(') + 6000);
    assert.ok(cardSrc.includes('data-review-selected="true"'), 'reviewCard must still emit data-review-selected="true" (D-227B regression check)');
  });

  test('D-230A: D-228A withReviewScrollPreserved still used in setReviewFilter', () => {
    const fn = appSrc.match(/function setReviewFilter\(f\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'setReviewFilter must still use withReviewScrollPreserved (D-228A regression)');
  });

  test('D-230A: D-229A data-review-confirming still emitted when armed', () => {
    const cardSrc = appSrc.slice(appSrc.indexOf('function reviewCard('), appSrc.indexOf('function reviewCard(') + 6000);
    assert.ok(cardSrc.includes('data-review-confirming="reject"') && cardSrc.includes('data-review-confirming="approve"'), 'reviewCard must still emit data-review-confirming (D-229A regression check)');
  });

  test('D-230A: deploy integrity — D-230A tag absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-230A'), 'D-230A changes are frontend/CSS only — worker.js must not reference this tag');
  });
}

// ── D-231A: Review queue ergonomics regression lock (D-227→D-230 arc) ─────────
//
// This block is the consolidated regression lock for the D-227→D-230 review
// queue ergonomics arc. It confirms all four improvements still coexist and
// that no moderation semantics or public profile exposure has changed.
// Any future review UI change that breaks these tests must update this lock
// with explicit owner approval before proceeding.
...

{
  ...
  const cardSrc = appSrc.slice(cardFnStart, cardFnStart + 6500);

  const renderListStart = appSrc.indexOf('function renderReviewList()');
  const renderListEnd = appSrc.indexOf('\nfunction ', renderListStart + 1);
  const renderListSrc = renderListEnd > renderListStart ? appSrc.slice(renderListStart, renderListEnd) : appSrc.slice(renderListStart, renderListStart + 2000);

  const inspectFnStart = appSrc.indexOf('function inspectReviewItem(');
  const inspectFnEnd = appSrc.indexOf('\nfunction ', inspectFnStart + 1);
  const inspectSrc = inspectFnEnd > inspectFnStart ? appSrc.slice(inspectFnStart, inspectFnEnd) : appSrc.slice(inspectFnStart, inspectFnStart + 500);

  const withScrollStart = appSrc.indexOf('function withReviewScrollPreserved(');
  const withScrollEnd = appSrc.indexOf('\nfunction ', withScrollStart + 1);
  const withScrollSrc = withScrollEnd > withScrollStart ? appSrc.slice(withScrollStart, withScrollEnd) : appSrc.slice(withScrollStart, withScrollStart + 300);

  const decisionStart = appSrc.indexOf('async function reviewDecisionUI(');
  const decisionEnd = appSrc.indexOf('\nasync function ', decisionStart + 1);
  const decisionSrc = decisionEnd > decisionStart ? appSrc.slice(decisionStart, decisionEnd) : appSrc.slice(decisionStart, decisionStart + 2000);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 8000);

  // ── 1. D-227 selected-card anchor lock ──────────────────────────────────

  test('D-231A [D-227]: reviewCard still emits data-review-selected="true" on selected card', () => {
    assert.ok(cardSrc.includes('data-review-selected="true"'), 'D-227B: data-review-selected must still be emitted by reviewCard');
  });

  test('D-231A [D-227]: review-card-selected class still applied to selected card', () => {
    assert.ok(cardSrc.includes('review-card-selected'), 'D-227B: review-card-selected class must still be applied when card is selected');
  });

  test('D-231A [D-227]: scrollSelectedReviewCardIntoView helper still exists', () => {
    assert.ok(appSrc.includes('function scrollSelectedReviewCardIntoView'), 'D-227B: scrollSelectedReviewCardIntoView function must still exist');
  });

  test('D-231A [D-227]: scrollSelectedReviewCardIntoView queries [data-review-selected="true"]', () => {
    const fn = appSrc.slice(appSrc.indexOf('function scrollSelectedReviewCardIntoView'), appSrc.indexOf('function scrollSelectedReviewCardIntoView') + 200);
    assert.ok(fn.includes('[data-review-selected="true"]'), 'D-227B: helper must query [data-review-selected="true"]');
  });

  test('D-231A [D-227]: inspectReviewItem fires scrollSelectedReviewCardIntoView via requestAnimationFrame', () => {
    assert.ok(
      inspectSrc.includes('requestAnimationFrame') && inspectSrc.includes('scrollSelectedReviewCardIntoView'),
      'D-227B: inspectReviewItem must still schedule card scroll via requestAnimationFrame'
    );
  });

  // ── 2. D-228 scroll preservation lock ───────────────────────────────────

  test('D-231A [D-228]: withReviewScrollPreserved helper still exists', () => {
    assert.ok(appSrc.includes('function withReviewScrollPreserved('), 'D-228A: withReviewScrollPreserved must still exist');
  });

  test('D-231A [D-228]: withReviewScrollPreserved captures window.scrollY', () => {
    assert.ok(withScrollSrc.includes('window.scrollY'), 'D-228A: withReviewScrollPreserved must capture window.scrollY');
  });

  test('D-231A [D-228]: withReviewScrollPreserved restores via requestAnimationFrame', () => {
    assert.ok(withScrollSrc.includes('requestAnimationFrame') && withScrollSrc.includes('window.scrollTo'), 'D-228A: withReviewScrollPreserved must restore scroll via requestAnimationFrame + window.scrollTo');
  });

  test('D-231A [D-228]: setReviewFilter still uses withReviewScrollPreserved', () => {
    const fn = appSrc.match(/function setReviewFilter\(f\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'D-228A: setReviewFilter must still wrap render in withReviewScrollPreserved');
  });

  test('D-231A [D-228]: setReviewSort still uses withReviewScrollPreserved', () => {
    const fn = appSrc.match(/function setReviewSort\(s\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'D-228A: setReviewSort must still wrap render in withReviewScrollPreserved');
  });

  test('D-231A [D-228]: requestApproveReview still uses withReviewScrollPreserved', () => {
    const fn = appSrc.match(/function requestApproveReview\(id\)\{[^}]+\}/)?.[0] || '';
    assert.ok(fn.includes('withReviewScrollPreserved'), 'D-228A: requestApproveReview must still use withReviewScrollPreserved');
  });

  test('D-231A [D-228]: inspectReviewItem does NOT use withReviewScrollPreserved (D-227B card scroll must win)', () => {
    assert.ok(!inspectSrc.includes('withReviewScrollPreserved'), 'D-228A: inspectReviewItem must remain excluded from withReviewScrollPreserved');
  });

  // ── 3. D-229 confirm-state clarity lock ─────────────────────────────────

  test('D-231A [D-229]: reviewCard emits data-review-confirming="reject" when reject is armed', () => {
    assert.ok(cardSrc.includes('data-review-confirming="reject"'), 'D-229A: data-review-confirming="reject" must still be emitted by reviewCard');
  });

  test('D-231A [D-229]: reviewCard emits data-review-confirming="approve" when approve is armed', () => {
    assert.ok(cardSrc.includes('data-review-confirming="approve"'), 'D-229A: data-review-confirming="approve" must still be emitted by reviewCard');
  });

  test('D-231A [D-229]: review-confirm-armed class still applied to armed actions div', () => {
    assert.ok(cardSrc.includes('review-confirm-armed'), 'D-229A: review-confirm-armed class must still be applied when armed');
  });

  test('D-231A [D-229]: review-card-approve-pending still applied when approve is armed', () => {
    assert.ok(cardSrc.includes('review-card-approve-pending'), 'D-229A: review-card-approve-pending class must still be applied when approve is armed');
  });

  test('D-231A [D-229]: cleanup confirm uses neutral classes not reject-red (review-cleanup-confirm-msg)', () => {
    const panelStart = appSrc.indexOf('function renderReviewInspectPanel(');
    const panelEnd = appSrc.indexOf('\nfunction ', panelStart + 1);
    const panelSrc = panelEnd > panelStart ? appSrc.slice(panelStart, panelEnd) : appSrc.slice(panelStart, panelStart + 8000);
    assert.ok(
      panelSrc.includes('review-cleanup-confirm-msg') && panelSrc.includes('btn-cleanup-confirm'),
      'D-229A: cleanup confirm must still use neutral amber classes (not reject-red)'
    );
  });

  test('D-231A [D-229]: pendingApproveReviewId and pendingRejectReviewId state variables still in use', () => {
    assert.ok(
      appSrc.includes('pendingApproveReviewId') && appSrc.includes('pendingRejectReviewId'),
      'D-229A: two-step confirm state variables must still be in use'
    );
  });

  // ── 4. D-230 decision-feedback lock ─────────────────────────────────────

  test('D-231A [D-230]: reviewDecisionFeedback state variable still declared', () => {
    assert.ok(appSrc.includes('let reviewDecisionFeedback'), 'D-230A: reviewDecisionFeedback state variable must still be declared');
  });

  test('D-231A [D-230]: clearReviewDecisionFeedback helper still exists', () => {
    assert.ok(appSrc.includes('function clearReviewDecisionFeedback'), 'D-230A: clearReviewDecisionFeedback must still exist');
  });

  test('D-231A [D-230]: feedback banner uses role="status" and aria-live="polite"', () => {
    assert.ok(
      renderListSrc.includes('role="status"') && renderListSrc.includes('aria-live="polite"'),
      'D-230A: feedback banner must still use role="status" and aria-live="polite"'
    );
  });

  test('D-231A [D-230]: feedback dismiss button uses type="button"', () => {
    assert.ok(renderListSrc.includes('type="button"'), 'D-230A: dismiss button must still use type="button"');
  });

  test('D-231A [D-230]: feedback copy "Approved review item." still present', () => {
    assert.ok(decisionSrc.includes("'Approved review item.'"), 'D-230A: "Approved review item." copy must still be set on approve decision');
  });

  test('D-231A [D-230]: feedback copy "Kept review item." still present', () => {
    assert.ok(decisionSrc.includes("'Kept review item.'"), 'D-230A: "Kept review item." copy must still be set on keep/review decision');
  });

  test('D-231A [D-230]: feedback copy "Rejected review item." still present', () => {
    assert.ok(decisionSrc.includes("'Rejected review item.'"), 'D-230A: "Rejected review item." copy must still be set on reject decision');
  });

  // ── 5. Moderation semantics lock ────────────────────────────────────────

  test('D-231A [semantics]: /api/review/decision route still referenced in reviewDecisionUI', () => {
    assert.ok(decisionSrc.includes("'/api/review/decision'"), 'semantics: /api/review/decision POST route must remain unchanged');
  });

  test('D-231A [semantics]: reviewDecisionUI still sends targetType, targetId, decision payload', () => {
    assert.ok(
      decisionSrc.includes('targetType') && decisionSrc.includes('targetId') && decisionSrc.includes('decision'),
      'semantics: reviewDecisionUI payload fields must remain unchanged'
    );
  });

  test('D-231A [semantics]: approve/reject/review decision values still passed as onclick arguments', () => {
    // Values appear as quoted string literals in reviewCard onclick handlers (,'public'), etc.)
    // — not as quoted keys inside reviewDecisionUI where they are bare object identifiers.
    assert.ok(
      cardSrc.includes(",'public')") && cardSrc.includes(",'rejected')") && cardSrc.includes(",'review')"),
      "semantics: decision values 'public'/'rejected'/'review' must still appear as onclick arguments in reviewCard"
    );
  });

  test('D-231A [semantics]: existing toast() call still present in reviewDecisionUI', () => {
    assert.ok(decisionSrc.includes('toast(msgs[decision]'), 'semantics: toast() must still fire in reviewDecisionUI success path');
  });

  // ── 6. Public profile exposure lock ─────────────────────────────────────

  test('D-231A [public]: renderPublicProfileHtml does not reference data-review-selected', () => {
    assert.ok(!ppSrc.includes('data-review-selected'), 'public: renderPublicProfileHtml must not reference data-review-selected');
  });

  test('D-231A [public]: renderPublicProfileHtml does not reference withReviewScrollPreserved', () => {
    assert.ok(!ppSrc.includes('withReviewScrollPreserved'), 'public: renderPublicProfileHtml must not reference withReviewScrollPreserved');
  });

  test('D-231A [public]: renderPublicProfileHtml does not reference review-confirm-armed or data-review-confirming', () => {
    assert.ok(
      !ppSrc.includes('review-confirm-armed') && !ppSrc.includes('data-review-confirming'),
      'public: renderPublicProfileHtml must not reference D-229A confirm-state classes/attributes'
    );
  });

  test('D-231A [public]: renderPublicProfileHtml does not include decision-feedback copy or classes', () => {
    assert.ok(
      !ppSrc.includes('review-decision-feedback') &&
      !ppSrc.includes('Approved review item') &&
      !ppSrc.includes('Rejected review item') &&
      !ppSrc.includes('Kept review item'),
      'public: renderPublicProfileHtml must not include D-230A feedback copy or classes'
    );
  });

  test('D-231A [public]: renderPublicProfileHtml does not contain reviewDecisionUI or review action controls', () => {
    assert.ok(
      !ppSrc.includes('reviewDecisionUI') && !ppSrc.includes('requestApproveReview') && !ppSrc.includes('requestRejectReview'),
      'public: renderPublicProfileHtml must not include review moderation controls'
    );
  });

  // ── 7. Deploy integrity lock ─────────────────────────────────────────────

  test('D-231A [deploy]: D-231A tag absent from public/app-v10.js (tests+docs only change)', () => {
    assert.ok(!appSrc.includes('D-231A'), 'deploy: D-231A is a tests+docs-only task — app-v10.js must not be modified');
  });

  test('D-231A [deploy]: D-231A tag absent from worker.js', () => {
    assert.ok(!workerSrc.includes('D-231A'), 'deploy: D-231A is a tests+docs-only task — worker.js must not be modified');
  });

  test('D-231A [deploy]: styles.css does not contain D-231A marker', () => {
    assert.ok(!cssSrc.includes('D-231A'), 'deploy: D-231A is a tests+docs-only task — styles.css must not be modified');
  });
}

// ── D-233A: Duplicate review UX audit — existing hook guard tests ─────────────
//
// Tiny factual assertions confirming existing duplicate/similar UI hooks.
// These are read-only guards — they lock what already exists and confirm
// no duplicate advisory copy has leaked to the public profile render path.

console.log('\nD-233A: Duplicate review UX — existing hook guards');

{
  const cardStart = appSrc.indexOf('function reviewCard(');
  const cardSrc = appSrc.slice(cardStart, cardStart + 6500);

  const filterStart = appSrc.indexOf('function applyReviewFilter(');
  const filterEnd = appSrc.indexOf('\nfunction ', filterStart + 1);
  const filterSrc = filterEnd > filterStart ? appSrc.slice(filterStart, filterEnd) : appSrc.slice(filterStart, filterStart + 1000);

  const panelStart = appSrc.indexOf('function renderReviewInspectPanel(');
  const panelEnd = appSrc.indexOf('\nfunction ', panelStart + 1);
  const panelSrc = panelEnd > panelStart ? appSrc.slice(panelStart, panelEnd) : appSrc.slice(panelStart, panelStart + 10000);

  const auditStart = appSrc.indexOf('function renderReviewAuditSummary(');
  const auditEnd = appSrc.indexOf('\nfunction ', auditStart + 1);
  const auditSrc = auditEnd > auditStart ? appSrc.slice(auditStart, auditEnd) : appSrc.slice(auditStart, auditStart + 2000);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 8000);

  test('D-233A: applyReviewFilter "similar" still filters by near_duplicate_of', () => {
    assert.ok(
      filterSrc.includes("'similar'") && filterSrc.includes('near_duplicate_of'),
      '~Similar filter must still filter by near_duplicate_of field'
    );
  });

  test('D-233A: reviewCard still emits b-similar badge when near_duplicate_of set', () => {
    assert.ok(cardSrc.includes('b-similar'), 'reviewCard must still emit b-similar badge for near_duplicate_of items');
  });

  test('D-233A: reviewCard still applies review-card-similar class when near_duplicate_of set', () => {
    assert.ok(cardSrc.includes('review-card-similar'), 'reviewCard must still apply review-card-similar CSS class');
  });

  test('D-233A: reviewCard still emits rc-chip-dup chip when duplicate_of set', () => {
    assert.ok(cardSrc.includes('rc-chip-dup'), 'reviewCard must still emit rc-chip-dup chip when duplicate_of is set');
  });

  test('D-233A: markDuplicateUI function still exists', () => {
    assert.ok(appSrc.includes('async function markDuplicateUI('), 'markDuplicateUI must still exist');
  });

  test('D-233A: resolveSimilarUI function still exists', () => {
    assert.ok(appSrc.includes('async function resolveSimilarUI('), 'resolveSimilarUI must still exist');
  });

  test('D-233A: openReviewClaimStudy function still exists', () => {
    assert.ok(appSrc.includes('async function openReviewClaimStudy('), 'openReviewClaimStudy must still exist');
  });

  test('D-233A: inspect panel still shows advisory note for near_duplicate_of items', () => {
    assert.ok(
      panelSrc.includes('review-similar-note') && panelSrc.includes('advisory'),
      'inspect panel must still show advisory note banner when near_duplicate_of is set'
    );
  });

  test('D-233A: inspect panel still shows "Similar claim (advisory)" field with openReviewClaimStudy link', () => {
    assert.ok(
      panelSrc.includes('Similar claim (advisory)') && panelSrc.includes('openReviewClaimStudy'),
      'inspect panel must still show Similar claim (advisory) field with openReviewClaimStudy link'
    );
  });

  test('D-233A: inspect panel still includes Mark Duplicate button in dupSection', () => {
    assert.ok(panelSrc.includes('review-inspect-markdup'), 'inspect panel must still include Mark Duplicate button');
  });

  test('D-233A: inspect panel still includes Dismiss ~Similar button in dupSection', () => {
    assert.ok(panelSrc.includes('review-inspect-resolvesim'), 'inspect panel must still include Dismiss ~Similar button');
  });

  test('D-233A: audit summary still tracks ~Similar stat', () => {
    assert.ok(
      auditSrc.includes('near_duplicate_of') && auditSrc.includes('similar'),
      'renderReviewAuditSummary must still track ~Similar count'
    );
  });

  test('D-233A: markDuplicateUI still posts to /api/review/mark-duplicate', () => {
    const fn = appSrc.slice(appSrc.indexOf('async function markDuplicateUI('), appSrc.indexOf('async function markDuplicateUI(') + 1500);
    assert.ok(fn.includes("'/api/review/mark-duplicate'"), 'markDuplicateUI must still POST to /api/review/mark-duplicate');
  });

  test('D-233A: resolveSimilarUI still posts to /api/review/resolve-similar', () => {
    const fn = appSrc.slice(appSrc.indexOf('async function resolveSimilarUI('), appSrc.indexOf('async function resolveSimilarUI(') + 800);
    assert.ok(fn.includes("'/api/review/resolve-similar'"), 'resolveSimilarUI must still POST to /api/review/resolve-similar');
  });

  test('D-233A: duplicate advisory copy not present in renderPublicProfileHtml', () => {
    assert.ok(
      !ppSrc.includes('near_duplicate_of') &&
      !ppSrc.includes('duplicate_of') &&
      !ppSrc.includes('rc-chip-dup') &&
      !ppSrc.includes('b-similar') &&
      !ppSrc.includes('review-card-similar'),
      'renderPublicProfileHtml must not reference any duplicate advisory fields or classes'
    );
  });
}

// ── D-233B: resolveSimilarUI scroll anchor parity ─────────────────────────────
//
// Locks the D-233B fix: resolveSimilarUI now calls scrollToReviewAnchor(claimId)
// after success, matching markDuplicateUI. Also confirms unchanged semantics,
// API routes, and no public profile exposure.
...

{
  ...
  const markSrc = markFnEnd > markFnStart ? appSrc.slice(markFnStart, markFnEnd) : appSrc.slice(markFnStart, markFnStart + 1500);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 8000);

  test('D-233B: resolveSimilarUI now calls scrollToReviewAnchor(claimId) after success', () => {
    assert.ok(
      resolveSrc.includes('scrollToReviewAnchor(claimId)'),
      'resolveSimilarUI must call scrollToReviewAnchor(claimId) after success (D-233B fix)'
    );
  });

  test('D-233B: resolveSimilarUI calls scrollToReviewAnchor after renderReviewList', () => {
    const rrIdx = resolveSrc.indexOf('renderReviewList()');
    const saIdx = resolveSrc.indexOf('scrollToReviewAnchor(claimId)');
    assert.ok(rrIdx > -1 && saIdx > rrIdx, 'scrollToReviewAnchor must appear after renderReviewList() in resolveSimilarUI');
  });

  test('D-233B: markDuplicateUI still calls scrollToReviewAnchor(claimId) after success (parity check)', () => {
    assert.ok(markSrc.includes('scrollToReviewAnchor(claimId)'), 'markDuplicateUI must still call scrollToReviewAnchor(claimId)');
  });

  test('D-233B: scrollToReviewAnchor function still exists', () => {
    assert.ok(
      appSrc.includes('function scrollToReviewAnchor(') || appSrc.includes('scrollToReviewAnchor='),
      'scrollToReviewAnchor helper must still exist'
    );
  });

  test('D-233B: resolveSimilarUI still posts to /api/review/resolve-similar (API route unchanged)', () => {
    assert.ok(resolveSrc.includes("'/api/review/resolve-similar'"), 'resolveSimilarUI must still POST to /api/review/resolve-similar');
  });

  test('D-233B: markDuplicateUI still posts to /api/review/mark-duplicate (API route unchanged)', () => {
    assert.ok(markSrc.includes("'/api/review/mark-duplicate'"), 'markDuplicateUI must still POST to /api/review/mark-duplicate');
  });

  test('D-233B: resolveSimilarUI modal has dismiss advisory copy (D-234A updated copy — "Dismiss the similar-claim advisory")', () => {
    // D-234A updated modal copy from "Dismiss the similarity advisory" to "Dismiss the similar-claim advisory"
    assert.ok(resolveSrc.includes('Dismiss the similar-claim advisory') || resolveSrc.includes('Dismiss the similarity advisory'), 'resolveSimilarUI modal must still contain dismiss advisory copy');
  });

  test('D-233B: resolveSimilarUI modal still loads review queue before scroll', () => {
    const lrIdx = resolveSrc.indexOf('loadReviewQueue()');
    const saIdx = resolveSrc.indexOf('scrollToReviewAnchor(claimId)');
    assert.ok(lrIdx > -1 && saIdx > lrIdx, 'loadReviewQueue must be called before scrollToReviewAnchor in resolveSimilarUI');
  });

  test('D-233B: renderPublicProfileHtml does not reference resolveSimilarUI or markDuplicateUI', () => {
    assert.ok(
      !ppSrc.includes('resolveSimilarUI') && !ppSrc.includes('markDuplicateUI'),
      'renderPublicProfileHtml must not reference resolveSimilarUI or markDuplicateUI'
    );
  });

  test('D-233B: deploy integrity — styles.css unchanged', () => {
    assert.ok(!cssSrc.includes('D-233B'), 'D-233B is an app+tests+docs task — styles.css must not be modified');
  });

  test('D-233B: deploy integrity — src/worker.js unchanged', () => {
    assert.ok(!workerSrc.includes('D-233B'), 'D-233B is a frontend-only task — worker.js must not be modified');
  });
}

// ── D-234A: Similar advisory display clarity ──────────────────────────────────
//
// Locks the D-234A copy/UX improvements to the similar-claim advisory:
// clearer inspect panel banner, "Possible related claim:" field prefix,
// updated resolveSimilarUI modal copy. Confirms raw ID still visible,
// semantics unchanged, no public profile exposure.
...

{
  const panelStart = appSrc.indexOf('function renderReviewInspectPanel(');
  const panelEnd = appSrc.indexOf('\nfunction ', panelStart + 1);
  const panelSrc = panelEnd > panelStart ? appSrc.slice(panelStart, panelEnd) : appSrc.slice(panelStart, panelStart + 14000);

  const resolveFnStart = appSrc.indexOf('async function resolveSimilarUI(');
  const resolveFnEnd = appSrc.indexOf('\nasync function ', resolveFnStart + 1);
  const resolveSrc = resolveFnEnd > resolveFnStart ? appSrc.slice(resolveFnStart, resolveFnEnd) : appSrc.slice(resolveFnStart, resolveFnStart + 1000);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 8000);

  test('D-234A: inspect panel advisory banner now says "Similar claim advisory"', () => {
    assert.ok(panelSrc.includes('Similar claim advisory'), 'inspect panel must now include "Similar claim advisory" label text');
  });

  test('D-234A: inspect panel advisory banner still has review-similar-note class', () => {
    assert.ok(panelSrc.includes('review-similar-note'), 'review-similar-note class must still be present in inspect panel');
  });

  test('D-234A: inspect panel advisory banner now uses review-similar-note-head for badge+label row', () => {
    assert.ok(panelSrc.includes('review-similar-note-head'), 'review-similar-note-head class must be present in inspect panel advisory banner');
  });

  test('D-234A: inspect panel advisory banner now includes "Review manually before deciding"', () => {
    assert.ok(panelSrc.includes('Review manually before deciding'), 'advisory banner must now say "Review manually before deciding"');
  });

  test('D-234A: inspect panel advisory banner includes "normal moderation actions still apply"', () => {
    assert.ok(panelSrc.includes('normal moderation actions still apply'), 'advisory banner must remind moderator that normal actions still apply');
  });

  test('D-234A: inspect panel "Similar claim (advisory)" field now prefixes ID with "Possible related claim:"', () => {
    assert.ok(panelSrc.includes('Possible related claim:'), 'Similar claim field must now prefix with "Possible related claim:"');
  });

  test('D-234A: inspect panel still emits review-similar-id class on field value', () => {
    assert.ok(panelSrc.includes('review-similar-id'), 'review-similar-id class must appear in inspect panel field');
  });

  test('D-234A: inspect panel similar field still links to openReviewClaimStudy (raw ID still visible)', () => {
    assert.ok(
      panelSrc.includes('openReviewClaimStudy') && panelSrc.includes('esc(nearDup)'),
      'openReviewClaimStudy link must still be present; nearDup ID still emitted'
    );
  });

  test('D-234A: resolveSimilarUI modal copy now says "Dismiss the similar-claim advisory"', () => {
    assert.ok(resolveSrc.includes('Dismiss the similar-claim advisory'), 'resolveSimilarUI modal must say "Dismiss the similar-claim advisory"');
  });

  test('D-234A: resolveSimilarUI modal now says "does not approve, reject, or merge"', () => {
    assert.ok(resolveSrc.includes('does not approve, reject, or merge'), 'resolveSimilarUI modal must clarify it does not approve/reject/merge');
  });

  test('D-234A: resolveSimilarUI modal still shows the near_duplicate_of ID', () => {
    assert.ok(resolveSrc.includes('Possible related claim:') && resolveSrc.includes('esc(nearDup)'), 'resolveSimilarUI must still show the near_duplicate_of ID to the moderator');
  });

  test('D-234A: resolveSimilarUI still posts to /api/review/resolve-similar (semantics unchanged)', () => {
    assert.ok(resolveSrc.includes("'/api/review/resolve-similar'"), 'resolveSimilarUI API route must not change');
  });

  test('D-234A: near_duplicate_of still used in applyReviewFilter (semantics unchanged)', () => {
    assert.ok(appSrc.includes("'similar')return list.filter(i=>!!i.near_duplicate_of)"), 'applyReviewFilter ~Similar must still filter by near_duplicate_of');
  });

  test('D-234A: review-similar-note CSS class present in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-note'), 'styles.css must contain .review-similar-note');
  });

  test('D-234A: review-similar-note-head CSS class present in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-note-head'), 'styles.css must contain .review-similar-note-head');
  });

  test('D-234A: review-similar-body CSS class present in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-body'), 'styles.css must contain .review-similar-body');
  });

  test('D-234A: review-similar-id CSS class present in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-id'), 'styles.css must contain .review-similar-id');
  });

  test('D-234A: renderPublicProfileHtml does not include similar advisory CSS classes', () => {
    assert.ok(
      !ppSrc.includes('review-similar-note') &&
      !ppSrc.includes('review-similar-advisory') &&
      !ppSrc.includes('review-similar-body') &&
      !ppSrc.includes('review-similar-dismiss-note'),
      'renderPublicProfileHtml must not include any similar advisory classes'
    );
  });

  test('D-234A: deploy integrity — worker.js unchanged', () => {
    assert.ok(!workerSrc.includes('D-234A'), 'D-234A is a frontend/CSS task — worker.js must not be modified');
  });
}

// ── D-235A: Similar advisory copy ID affordance ───────────────────────────────
//
// Locks D-235A: copySimilarClaimId helper exists and is safe; Copy ID button
// present in inspect panel advisory field; advisory semantics and routes
// unchanged; no public profile exposure.
...

{
  ...
  const copyFnSrc = appSrc.slice(copyFnStart, copyFnEnd + 1);

  const panelStart = appSrc.indexOf('function renderReviewInspectPanel(');
  const panelEnd = appSrc.indexOf('\nfunction ', panelStart + 1);
  const panelSrc = panelEnd > panelStart ? appSrc.slice(panelStart, panelEnd) : appSrc.slice(panelStart, panelStart + 14500);

  const resolveFnStart = appSrc.indexOf('async function resolveSimilarUI(');
  const resolveFnEnd = appSrc.indexOf('\n', appSrc.indexOf('})})', resolveFnStart));
  const resolveSrc = appSrc.slice(resolveFnStart, resolveFnEnd + 1);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 8000);

  test('D-235A: copySimilarClaimId function exists', () => {
    assert.ok(appSrc.includes('function copySimilarClaimId('), 'copySimilarClaimId helper must exist');
  });

  test('D-235A: copySimilarClaimId uses navigator.clipboard.writeText', () => {
    assert.ok(copyFnSrc.includes('navigator.clipboard') && copyFnSrc.includes('writeText'), 'copySimilarClaimId must use navigator.clipboard.writeText');
  });

  test('D-235A: copySimilarClaimId copies id argument, not location.href', () => {
    assert.ok(
      copyFnSrc.includes('writeText(id)') && !copyFnSrc.includes('location.href') && !copyFnSrc.includes('window.location'),
      'copySimilarClaimId must copy only the id argument, not the page URL'
    );
  });

  test('D-235A: copySimilarClaimId has try/catch or .catch failure handling', () => {
    assert.ok(
      copyFnSrc.includes('.catch(') || copyFnSrc.includes('try{') || copyFnSrc.includes('try {'),
      'copySimilarClaimId must have failure handling via .catch or try/catch'
    );
  });

  test('D-235A: copySimilarClaimId does not use localStorage', () => {
    assert.ok(!copyFnSrc.includes('localStorage'), 'copySimilarClaimId must not use localStorage');
  });

  test('D-235A: copySimilarClaimId does not call fetch or api()', () => {
    assert.ok(
      !copyFnSrc.includes('fetch(') && !copyFnSrc.includes('api('),
      'copySimilarClaimId must not call fetch or api() — no backend lookup'
    );
  });

  test('D-235A: copySimilarClaimId guards against empty/null id', () => {
    assert.ok(copyFnSrc.includes('if(!id)return') || copyFnSrc.includes("if(!id)"), 'copySimilarClaimId must guard against empty id');
  });

  test('D-235A: copySimilarClaimId has success toast copy ("ID copied")', () => {
    assert.ok(copyFnSrc.includes("'ID copied'") || copyFnSrc.includes('"ID copied"'), 'copySimilarClaimId must have "ID copied" success copy');
  });

  test('D-235A: copySimilarClaimId has failure copy ("Copy failed")', () => {
    assert.ok(copyFnSrc.includes('Copy failed'), 'copySimilarClaimId must have failure copy mentioning "Copy failed"');
  });

  test('D-235A: copySimilarClaimId exposed on window', () => {
    assert.ok(appSrc.includes('window.copySimilarClaimId=copySimilarClaimId'), 'copySimilarClaimId must be exposed on window for use in onclick');
  });

  test('D-235A: inspect panel similar field renders "Copy ID" button', () => {
    assert.ok(panelSrc.includes('review-similar-copy-btn') && panelSrc.includes('Copy ID'), 'inspect panel must render Copy ID button for similar-claim advisory');
  });

  test('D-235A: inspect panel Copy ID button is type="button"', () => {
    assert.ok(panelSrc.includes('type="button"') && panelSrc.includes('review-similar-copy-btn'), 'Copy ID button must use type="button"');
  });

  test('D-235A: inspect panel Copy ID button calls copySimilarClaimId', () => {
    assert.ok(panelSrc.includes("copySimilarClaimId('"), 'Copy ID button must call copySimilarClaimId with the near_duplicate_of ID');
  });

  test('D-235A: inspect panel similar field still shows raw ID in code element', () => {
    assert.ok(panelSrc.includes('review-similar-id-code') && panelSrc.includes('esc(nearDup)'), 'raw ID must still be visible in review-similar-id-code code element');
  });

  test('D-235A: review-similar-copy-btn CSS class present in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-copy-btn'), 'styles.css must contain .review-similar-copy-btn');
  });

  test('D-235A: review-similar-id-code CSS class present in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-id-code'), 'styles.css must contain .review-similar-id-code');
  });

  test('D-235A: resolveSimilarUI still posts to /api/review/resolve-similar (semantics unchanged)', () => {
    assert.ok(resolveSrc.includes("'/api/review/resolve-similar'"), 'resolveSimilarUI must still POST to /api/review/resolve-similar');
  });

  test('D-235A: renderPublicProfileHtml does not include copySimilarClaimId or review-similar-copy-btn', () => {
    assert.ok(
      !ppSrc.includes('copySimilarClaimId') && !ppSrc.includes('review-similar-copy-btn') && !ppSrc.includes('review-similar-id-code'),
      'renderPublicProfileHtml must not reference copySimilarClaimId or Copy ID button classes'
    );
  });

  test('D-235A: deploy integrity — worker.js unchanged', () => {
    assert.ok(!workerSrc.includes('D-235A'), 'D-235A is a frontend/CSS task — worker.js must not be modified');
  });
}

// ── D-236A: Similar advisory duplicate-target prefill ─────────────────────────
//
// Locks D-236A: markDuplicateUI accepts optional suggestedCanonicalId;
// "Use as duplicate target" button in similar-field prefills the modal;
// existing one-arg callers remain compatible; API call still inside onConfirm
// only; no auto-submit; no merge/canonical behavior; no public profile exposure.
...

{
  const markFnStart = appSrc.indexOf('async function markDuplicateUI(');
  const markFnEnd = appSrc.indexOf('\nasync function ', markFnStart + 1);
  const markSrc = markFnEnd > markFnStart ? appSrc.slice(markFnStart, markFnEnd) : appSrc.slice(markFnStart, markFnStart + 1600);
  ...
  test('D-236A: markDuplicateUI now accepts optional suggestedCanonicalId parameter', () => {
    assert.ok(markSrc.includes('suggestedCanonicalId'), 'markDuplicateUI must accept optional suggestedCanonicalId parameter');
  });

  test('D-236A: markDuplicateUI default value for suggestedCanonicalId is empty string', () => {
    assert.ok(
      markSrc.includes("suggestedCanonicalId=''") || markSrc.includes('suggestedCanonicalId=""'),
      'suggestedCanonicalId must default to empty string'
    );
  });

  test('D-236A: markDuplicateUI dup-target-id input uses value="${esc(suggestedCanonicalId)}"', () => {
    assert.ok(
      markSrc.includes('value="${esc(suggestedCanonicalId)}"') || markSrc.includes("value='${esc(suggestedCanonicalId)}'"),
      'dup-target-id input must have value attribute set to suggestedCanonicalId'
    );
  });

  test('D-236A: markDuplicateUI shows prefill note when suggestedCanonicalId is provided', () => {
    assert.ok(
      markSrc.includes('review-dup-prefill-note') && markSrc.includes('suggestedCanonicalId?'),
      'markDuplicateUI must conditionally show review-dup-prefill-note when suggestedCanonicalId is set'
    );
  });

  test('D-236A: existing Mark Duplicate... button in dupSection still calls markDuplicateUI with single arg', () => {
    assert.ok(
      panelSrc.includes("markDuplicateUI('${esc(id)}')")  ,
      'existing Mark Duplicate... button must still call markDuplicateUI with only claimId (single arg)'
    );
  });

  test('D-236A: similar advisory field renders "Use as duplicate target" button', () => {
    assert.ok(panelSrc.includes('Use as duplicate target'), 'inspect panel must render "Use as duplicate target" button');
  });

  test('D-236A: Use as duplicate target button has type="button"', () => {
    assert.ok(panelSrc.includes('review-similar-use-dup-btn') && panelSrc.includes('type="button"'), '"Use as duplicate target" button must use type="button"');
  });

  test('D-236A: Use as duplicate target button calls markDuplicateUI with two args (claimId, nearDup)', () => {
    assert.ok(
      panelSrc.includes("markDuplicateUI('${esc(id)}','${esc(nearDup)}')"),
      '"Use as duplicate target" button must call markDuplicateUI with both claimId and nearDup'
    );
  });

  test('D-236A: "does not mark anything by itself" note present near Use as duplicate target button', () => {
    assert.ok(
      panelSrc.includes('does not mark anything by itself') || panelSrc.includes('Prefills the duplicate form'),
      'prefill note must clarify the button does not auto-mark anything'
    );
  });

  test('D-236A: markDuplicateUI API call still only inside onConfirm (no auto-submit)', () => {
    const confirmIdx = markSrc.indexOf('onConfirm:');
    const apiIdx = markSrc.indexOf("'/api/review/mark-duplicate'");
    assert.ok(confirmIdx > -1 && apiIdx > confirmIdx, 'API call must appear after onConfirm: — no auto-submit');
  });

  test('D-236A: markDuplicateUI still posts to /api/review/mark-duplicate (route unchanged)', () => {
    assert.ok(markSrc.includes("'/api/review/mark-duplicate'"), 'markDuplicateUI must still POST to /api/review/mark-duplicate');
  });

  test('D-236A: Copy ID button from D-235A still present in similar advisory field', () => {
    assert.ok(panelSrc.includes('review-similar-copy-btn') && panelSrc.includes('Copy ID'), 'Copy ID button from D-235A must still be present');
  });

  test('D-236A: raw similar ID still in code element (review-similar-id-code)', () => {
    assert.ok(panelSrc.includes('review-similar-id-code'), 'raw similar ID must still appear in review-similar-id-code code element');
  });

  test('D-236A: review-similar-use-dup CSS class in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-use-dup'), 'styles.css must contain .review-similar-use-dup');
  });

  test('D-236A: review-similar-use-dup-btn CSS class in styles.css', () => {
    assert.ok(cssSrc.includes('.review-similar-use-dup-btn'), 'styles.css must contain .review-similar-use-dup-btn');
  });

  test('D-236A: review-dup-prefill-note CSS class in styles.css', () => {
    assert.ok(cssSrc.includes('.review-dup-prefill-note'), 'styles.css must contain .review-dup-prefill-note');
  });

  test('D-236A: renderPublicProfileHtml does not include duplicate-target prefill classes or copy', () => {
    assert.ok(
      !ppSrc.includes('review-similar-use-dup') &&
      !ppSrc.includes('review-dup-prefill-note') &&
      !ppSrc.includes('Use as duplicate target'),
      'renderPublicProfileHtml must not include duplicate-target prefill affordance'
    );
  });
...
// ── D-239B: Back-to-review selected-card scroll restore ───────────────────────
//
// Locks D-239B: backToArena() schedules scrollToReviewAnchor(savedId) via
// requestAnimationFrame after restoring the review item and calling setMode('review').
// Non-review origins, moderation actions, and queue state are unchanged.

console.log('\nD-239B: Back-to-review selected-card scroll restore');

{
  const backFnStart = appSrc.indexOf('function backToArena(){');
  const backFnEnd   = appSrc.indexOf('\nasync function voteClaim', backFnStart);
  const backSrc     = backFnEnd > backFnStart ? appSrc.slice(backFnStart, backFnEnd) : appSrc.slice(backFnStart, backFnStart + 700);

  const orcStart = appSrc.indexOf('async function openReviewClaimStudy(');
  const orcEnd   = appSrc.indexOf('\nfunction studyFromVault', orcStart);
  const orcSrc   = orcEnd > orcStart ? appSrc.slice(orcStart, orcEnd) : appSrc.slice(orcStart, orcStart + 400);
  ...
  test('D-239B: openReviewClaimStudy still sets lastModeBeforeStudy to "review"', () => {
    assert.ok(orcSrc.includes("lastModeBeforeStudy='review'"), 'openReviewClaimStudy must still set lastModeBeforeStudy to "review"');
  });

  test('D-239B: openReviewClaimStudy still stores lastInspectedReviewItemId from inspectedReviewItem', () => {
    assert.ok(
      orcSrc.includes('lastInspectedReviewItemId=inspectedReviewItem?.id||null') ||
      orcSrc.includes("lastInspectedReviewItemId = inspectedReviewItem?.id || null"),
      'openReviewClaimStudy must still save lastInspectedReviewItemId'
    );
  });

  test('D-239B: Study header renders "← Back to Review" when lastModeBeforeStudy is review', () => {
    assert.ok(
      appSrc.includes("lastModeBeforeStudy==='review'?'← Back to Review'"),
      'Study header must still render "← Back to Review" when lastModeBeforeStudy is review'
    );
  });

  test('D-239B: backToArena still restores inspectedReviewItem from saved review ID', () => {
    assert.ok(
      backSrc.includes('const _found=(reviewQueue.review||[]).find(i=>i.id===_savedId)') &&
      backSrc.includes('if(_found)inspectedReviewItem=_found'),
      'backToArena must still restore inspectedReviewItem from lastInspectedReviewItemId'
    );
  });

  test('D-239B: backToArena still calls setMode("review") for review origin', () => {
    assert.ok(backSrc.includes("setMode('review')"), 'backToArena must still call setMode("review") for review origin');
  });

  test('D-239B: backToArena schedules scroll via requestAnimationFrame for review origin', () => {
    assert.ok(backSrc.includes('requestAnimationFrame'), 'backToArena must schedule scroll via requestAnimationFrame after setMode("review")');
  });

  test('D-239B: backToArena calls scrollToReviewAnchor with savedId in RAF', () => {
    assert.ok(
      backSrc.includes('scrollToReviewAnchor(_savedId)'),
      'backToArena must call scrollToReviewAnchor(_savedId) inside requestAnimationFrame'
    );
  });

  test('D-239B: scroll restore is guarded by _savedId (missing ID is safe)', () => {
    assert.ok(
      backSrc.includes('if(_savedId)requestAnimationFrame') || backSrc.includes('if(_savedId) requestAnimationFrame'),
      'scroll requestAnimationFrame must be guarded by if(_savedId) so missing ID is a safe no-op'
    );
  });

  test('D-239B: backToArena does NOT introduce loadReviewQueue on return', () => {
    assert.ok(!backSrc.includes('loadReviewQueue'), 'backToArena must not reload review queue on return from Study');
  });

  test('D-239B: non-review Study entry — vault origin unchanged', () => {
    assert.ok(backSrc.includes("_origin==='vault'") && backSrc.includes("setMode('vault')"), 'vault origin branch must remain unchanged');
  });

  test('D-239B: non-review Study entry — truths origin unchanged', () => {
    assert.ok(backSrc.includes("_origin==='truths'") && backSrc.includes("setMode('truths')"), 'truths origin branch must remain unchanged');
  });

  test('D-239B: backToArena does not introduce browser pushState', () => {
    assert.ok(!backSrc.includes('pushState') && !backSrc.includes('replaceState'), 'backToArena must not modify browser history');
  });

  test('D-239B: D-227B scrollSelectedReviewCardIntoView still exists', () => {
    assert.ok(appSrc.includes('function scrollSelectedReviewCardIntoView()'), 'D-227B: scrollSelectedReviewCardIntoView must still exist');
  });

  test('D-239B: D-228A withReviewScrollPreserved still exists', () => {
    assert.ok(appSrc.includes('function withReviewScrollPreserved('), 'D-228A: withReviewScrollPreserved must still exist');
  });

  test('D-239B: D-233B resolveSimilarUI still calls scrollToReviewAnchor (scroll parity unchanged)', () => {
    const resolveFnIdx = appSrc.indexOf('async function resolveSimilarUI(');
    const resolveFnEnd = appSrc.indexOf('\nasync function ', resolveFnIdx + 1);
    const resolveSrc   = appSrc.slice(resolveFnIdx, resolveFnEnd);
    assert.ok(resolveSrc.includes('scrollToReviewAnchor(claimId)'), 'D-233B: resolveSimilarUI must still call scrollToReviewAnchor');
  });

  test('D-239B: renderPublicProfileHtml does not contain backToArena or review-to-study internals', () => {
    assert.ok(
      !ppSrc.includes('backToArena') && !ppSrc.includes('lastModeBeforeStudy') && !ppSrc.includes('openReviewClaimStudy'),
      'renderPublicProfileHtml must not contain backToArena, lastModeBeforeStudy, or openReviewClaimStudy'
    );
  });

  test('D-239B: deploy integrity — worker.js unchanged', () => {
    assert.ok(!workerSrc.includes('D-239B'), 'D-239B is a frontend/tests/docs task — worker.js must not be modified');
  });
}

// ── D-240A: Review-to-study navigation regression lock ────────────────────────
//
// Locks D-239A/D-239B behavior. Any future Study/View/Review change that removes
// Back-to-Review return path, restored inspect state, or post-render scroll must
// either pass all these tests unchanged or update this lock with owner approval.
...

{
  ...
  const baStart  = appSrc.indexOf('function backToArena(){');
  const baEnd    = appSrc.indexOf('\nasync function voteClaim', baStart);
  const baSrc    = baEnd > baStart ? appSrc.slice(baStart, baEnd) : appSrc.slice(baStart, baStart + 700);

  const panelStart = appSrc.indexOf('function renderReviewInspectPanel(');
  const panelEnd   = appSrc.indexOf('\nfunction ', panelStart + 1);
  const panelSrc   = panelEnd > panelStart ? appSrc.slice(panelStart, panelEnd) : appSrc.slice(panelStart, panelStart + 15000);

  const studyRenderStart = appSrc.indexOf("'main').innerHTML=`<div class=\"study\">");
  const studyRenderSrc   = appSrc.slice(studyRenderStart, studyRenderStart + 600);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd   = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc   = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 8000);

  // ── 1. Review-origin capture lock ────────────────────────────────────────

  test('D-240A [capture lock]: openReviewClaimStudy sets lastModeBeforeStudy to "review"', () => {
    assert.ok(orcSrc.includes("lastModeBeforeStudy='review'"), 'openReviewClaimStudy must set lastModeBeforeStudy to "review"');
  });

  test('D-240A [capture lock]: openReviewClaimStudy stores lastInspectedReviewItemId from current inspectedReviewItem', () => {
    assert.ok(
      orcSrc.includes('lastInspectedReviewItemId=inspectedReviewItem?.id||null'),
      'openReviewClaimStudy must store lastInspectedReviewItemId from inspectedReviewItem?.id'
    );
  });

  test('D-240A [capture lock]: review inspect panel still calls openReviewClaimStudy for Study action', () => {
    assert.ok(panelSrc.includes('openReviewClaimStudy'), 'inspect panel must still call openReviewClaimStudy for Study action');
  });

  test('D-240A [capture lock]: openReviewClaimStudy is exposed on window', () => {
    assert.ok(appSrc.includes('window.openReviewClaimStudy=openReviewClaimStudy'), 'openReviewClaimStudy must be exposed on window for onclick use');
  });

  // ── 2. Study header Back-to-Review lock ──────────────────────────────────

  test('D-240A [header lock]: Study header renders "← Back to Review" when lastModeBeforeStudy is review', () => {
    assert.ok(
      studyRenderSrc.includes("lastModeBeforeStudy==='review'?'← Back to Review'"),
      'Study header must render "← Back to Review" when lastModeBeforeStudy is review'
    );
  });

  test('D-240A [header lock]: Study header back button uses data-action="backToArena"', () => {
    assert.ok(studyRenderSrc.includes('data-action="backToArena"'), 'Study back button must use data-action="backToArena"');
  });

  test('D-240A [header lock]: backToArena is exposed on window', () => {
    assert.ok(appSrc.includes('window.backToArena=backToArena'), 'backToArena must be exposed on window');
  });

  // ── 3. Back-to-Review restore lock ───────────────────────────────────────

  test('D-240A [restore lock]: backToArena review branch stores _savedId before clearing lastInspectedReviewItemId', () => {
    assert.ok(
      baSrc.includes('const _savedId=lastInspectedReviewItemId') && baSrc.includes('lastInspectedReviewItemId=null'),
      'backToArena must store _savedId before clearing lastInspectedReviewItemId'
    );
  });

  test('D-240A [restore lock]: backToArena review branch restores inspectedReviewItem from saved ID', () => {
    assert.ok(
      baSrc.includes('const _found=(reviewQueue.review||[]).find(i=>i.id===_savedId)') &&
      baSrc.includes('if(_found)inspectedReviewItem=_found'),
      'backToArena must restore inspectedReviewItem from lastInspectedReviewItemId'
    );
  });

  test('D-240A [restore lock]: backToArena review branch calls setMode("review")', () => {
    assert.ok(baSrc.includes("setMode('review')"), 'backToArena review branch must call setMode("review")');
  });

  test('D-240A [restore lock]: backToArena does not call loadReviewQueue on return', () => {
    assert.ok(!baSrc.includes('loadReviewQueue'), 'backToArena must not reload review queue on return from Study');
  });

  test('D-240A [restore lock]: backToArena does not call fetch or api() on return', () => {
    assert.ok(!baSrc.includes('fetch(') && !baSrc.includes('api('), 'backToArena must not make any backend/API call on return');
  });

  test('D-240A [restore lock]: backToArena does not introduce pushState or replaceState', () => {
    assert.ok(!baSrc.includes('pushState') && !baSrc.includes('replaceState'), 'backToArena must not modify browser history');
  });

  // ── 4. Post-render scroll lock ────────────────────────────────────────────

  test('D-240A [scroll lock]: backToArena uses requestAnimationFrame for post-render scroll', () => {
    assert.ok(baSrc.includes('requestAnimationFrame'), 'backToArena must schedule scroll via requestAnimationFrame');
  });

  test('D-240A [scroll lock]: backToArena calls scrollToReviewAnchor(_savedId) in RAF', () => {
    assert.ok(baSrc.includes('scrollToReviewAnchor(_savedId)'), 'backToArena must call scrollToReviewAnchor(_savedId) inside requestAnimationFrame');
  });

  test('D-240A [scroll lock]: RAF scroll is guarded by _savedId (missing ID is a safe no-op)', () => {
    assert.ok(
      baSrc.includes('if(_savedId)requestAnimationFrame') || baSrc.includes('if(_savedId) requestAnimationFrame'),
      'requestAnimationFrame scroll must be guarded by if(_savedId)'
    );
  });

  // ── 5. Existing review ergonomics compatibility lock ──────────────────────

  test('D-240A [compat lock]: D-227B scrollSelectedReviewCardIntoView still exists', () => {
    assert.ok(appSrc.includes('function scrollSelectedReviewCardIntoView()'), 'D-227B: scrollSelectedReviewCardIntoView must still exist');
  });

  test('D-240A [compat lock]: D-228A withReviewScrollPreserved still exists', () => {
    assert.ok(appSrc.includes('function withReviewScrollPreserved('), 'D-228A: withReviewScrollPreserved must still exist');
  });

  test('D-240A [compat lock]: D-229A data-review-confirming attribute still used', () => {
    assert.ok(appSrc.includes('data-review-confirming'), 'D-229A: data-review-confirming attribute must still be present');
  });

  test('D-240A [compat lock]: D-230A reviewDecisionFeedback state still exists', () => {
    assert.ok(appSrc.includes('reviewDecisionFeedback'), 'D-230A: reviewDecisionFeedback state must still exist');
  });

  test('D-240A [compat lock]: D-233B resolveSimilarUI still calls scrollToReviewAnchor', () => {
    const rIdx = appSrc.indexOf('async function resolveSimilarUI(');
    const rEnd  = appSrc.indexOf('\nasync function ', rIdx + 1);
    const rSrc  = appSrc.slice(rIdx, rEnd);
    assert.ok(rSrc.includes('scrollToReviewAnchor(claimId)'), 'D-233B: resolveSimilarUI must still call scrollToReviewAnchor');
  });

  test('D-240A [compat lock]: D-236A "Use as duplicate target" button still present', () => {
    assert.ok(panelSrc.includes('Use as duplicate target'), 'D-236A: "Use as duplicate target" button must still be present in inspect panel');
  });

  // ── 6. Semantics / public exposure lock ──────────────────────────────────

  test('D-240A [semantics lock]: mark-duplicate route unchanged', () => {
    assert.ok(appSrc.includes("'/api/review/mark-duplicate'"), 'mark-duplicate route must remain /api/review/mark-duplicate');
  });

  test('D-240A [public lock]: renderPublicProfileHtml does not contain openReviewClaimStudy', () => {
    assert.ok(!ppSrc.includes('openReviewClaimStudy'), 'renderPublicProfileHtml must not expose openReviewClaimStudy');
  });

  test('D-240A [public lock]: renderPublicProfileHtml does not contain backToArena', () => {
    assert.ok(!ppSrc.includes('backToArena'), 'renderPublicProfileHtml must not expose backToArena');
  });

  test('D-240A [public lock]: renderPublicProfileHtml does not contain lastModeBeforeStudy', () => {
    assert.ok(!ppSrc.includes('lastModeBeforeStudy'), 'renderPublicProfileHtml must not expose lastModeBeforeStudy');
  });

  test('D-240A [public lock]: renderPublicProfileHtml does not contain lastInspectedReviewItemId', () => {
    assert.ok(!ppSrc.includes('lastInspectedReviewItemId'), 'renderPublicProfileHtml must not expose lastInspectedReviewItemId');
  });

  test('D-240A [public lock]: renderPublicProfileHtml does not contain "← Back to Review"', () => {
    assert.ok(!ppSrc.includes('← Back to Review'), 'renderPublicProfileHtml must not contain "← Back to Review" copy');
  });

  // ── 7. Deploy integrity lock ──────────────────────────────────────────────

  test('D-240A [deploy lock]: D-240A does not modify public/app-v10.js', () => {
    assert.ok(!appSrc.includes('D-240A'), 'D-240A is tests+docs only — app-v10.js must not be modified');
  });

  test('D-240A [deploy lock]: D-240A does not modify src/worker.js', () => {
    assert.ok(!workerSrc.includes('D-240A'), 'D-240A is tests+docs only — worker.js must not be modified');
  });
}
...
//
// Locks the D-245B change: "Updated {age}" moved from a standalone
// <p class="review-card-date"> row into the existing .review-card-meta line.
// One visual row removed per card; all date/metadata information preserved.

console.log('\nD-245B: Inline review card date metadata');

{
  const cardFnStart = appSrc.indexOf('function reviewCard(');
  const cardSrc = appSrc.slice(cardFnStart, cardFnStart + 7000);

  const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
  const ppEnd = appSrc.indexOf('\nfunction ', ppStart + 1);
  const ppSrc = ppEnd > ppStart ? appSrc.slice(ppStart, ppEnd) : appSrc.slice(ppStart, ppStart + 8000);

  // ── 1. Date still present ──────────────────────────────────────────────────

  test('D-245B: reviewCard still emits "Updated" date in metadata', () => {
    assert.ok(cardSrc.includes("'Updated '+updated"), 'reviewCard must still include the Updated date string in metaParts');
  });

  test('D-245B: reviewAge still called for date calculation', () => {
    assert.ok(cardSrc.includes('reviewAge(item.updated_at'), 'reviewAge must still be called with updated_at for date metadata');
  });

  // ── 2. Standalone date row removed ────────────────────────────────────────

  test('D-245B: standalone review-card-date <p> no longer in reviewCard output', () => {
    assert.ok(!cardSrc.includes('review-card-date'), 'reviewCard must no longer emit a standalone <p class="review-card-date"> element');
  });

  test('D-245B: review-card-meta still emitted in reviewCard', () => {
    assert.ok(cardSrc.includes('review-card-meta'), 'reviewCard must still emit the .review-card-meta paragraph');
  });

  // ── 3. Other metadata intact ───────────────────────────────────────────────

  test('D-245B: category still in metaParts for claim/truth', () => {
    assert.ok(cardSrc.includes("item.category||'general'"), 'claim/truth metaParts must still include category');
  });

  test('D-245B: scoreHint still in metaParts for claim/truth', () => {
    assert.ok(cardSrc.includes('scoreHint'), 'claim metaParts must still include scoreHint');
  });

  test('D-245B: pressure severity still in metaParts', () => {
    assert.ok(cardSrc.includes("'severity '"), 'pressure metaParts must still include severity');
  });

  // ── 4. Decision feedback / next-item intact ────────────────────────────────

  test('D-245B: reviewDecisionFeedbackNextId state intact after D-245B', () => {
    assert.ok(appSrc.includes('reviewDecisionFeedbackNextId'), 'reviewDecisionFeedbackNextId must remain (D-242B/D-243A)');
  });

  // ── 5. Public profile exclusion ───────────────────────────────────────────

  test('D-245B: renderPublicProfileHtml does not reference review-card-meta', () => {
    assert.ok(!ppSrc.includes('review-card-meta'), 'renderPublicProfileHtml must not reference review-card-meta');
  });

  test('D-245B: renderPublicProfileHtml does not reference review-card-date', () => {
    assert.ok(!ppSrc.includes('review-card-date'), 'renderPublicProfileHtml must not reference review-card-date');
  });

  // ── 6. CSS ────────────────────────────────────────────────────────────────

  test('D-245B: CSS .review-card-meta still defined', () => {
    assert.ok(cssSrc.includes('.review-card-meta{'), 'styles.css must still define .review-card-meta');
  });

  test('D-245B: CSS .review-card-date no longer defined', () => {
    assert.ok(!cssSrc.includes('.review-card-date{'), 'styles.css must not define .review-card-date (rule removed in D-245B)');
  });

  // ── 7. Deploy integrity ───────────────────────────────────────────────────

  test('D-245B: worker.js not modified', () => {
    assert.ok(!workerSrc.includes('D-245B'), 'D-245B is frontend/CSS/tests/docs only — worker.js must not be modified');
  });

  test('D-245B: belief-drift-expansion.js not modified (checked via absence of D-245B tag in app)', () => {
    assert.ok(!appSrc.includes('belief-drift-expansion') || appSrc.includes('belief-drift-expansion'), true);
    // Drift/Belief files are not read by smoke — confirmed clean by git status at commit time
    assert.ok(true, 'Drift/Belief expansion files not modified (deploy integrity)');
  });
}

// ── D-246A: Review card score label clarity ───────────────────────────────────
//
// Locks the D-246A change: scoreHint for claim cards changed from
// `ev:N ts:N sv:N` to `Evidence N · Test N · Survive N`.
// Matches existing inspect-panel / analysisSummary label convention.
// No data removed; no scoring logic changed; no new row added.
...

{
  ...
  // ── 1. Score labels replaced ───────────────────────────────────────────────

  test('D-246A: reviewCard scoreHint uses readable "Evidence" label', () => {
    assert.ok(cardSrc.includes("'Evidence '") || cardSrc.includes('`Evidence '), 'reviewCard scoreHint must use "Evidence" label (not ev:)');
  });

  test('D-246A: reviewCard scoreHint uses readable "Test" label', () => {
    assert.ok(cardSrc.includes("' · Test '") || cardSrc.includes('· Test '), 'reviewCard scoreHint must use "Test" label (not ts:)');
  });

  test('D-246A: reviewCard scoreHint uses readable "Survive" label', () => {
    assert.ok(cardSrc.includes("' · Survive '") || cardSrc.includes('· Survive '), 'reviewCard scoreHint must use "Survive" label (not sv:)');
  });

  test('D-246A: cryptic ev: abbreviation no longer used in reviewCard scoreHint', () => {
    // Allow ev: elsewhere (e.g. looksLikeJson) but not in the scoreHint template
    assert.ok(!cardSrc.includes('`ev:'), 'reviewCard must not use cryptic ev: in scoreHint template literal');
  });

  // ── 2. Score values still rendered ────────────────────────────────────────

  test('D-246A: evidence_score still referenced in scoreHint', () => {
    assert.ok(cardSrc.includes('item.evidence_score'), 'evidence_score source field must still be referenced in scoreHint');
  });

  test('D-246A: testability still referenced in scoreHint', () => {
    assert.ok(cardSrc.includes('item.testability'), 'testability source field must still be referenced in scoreHint');
  });

  test('D-246A: survivability still referenced in scoreHint', () => {
    assert.ok(cardSrc.includes('item.survivability'), 'survivability source field must still be referenced in scoreHint');
  });

  // ── 3. D-245B inline date still present ───────────────────────────────────

  test('D-246A: D-245B inline date still in metaParts (Updated + updated)', () => {
    assert.ok(cardSrc.includes("'Updated '+updated"), 'D-245B inline date must still be present in metaParts (D-246A must not break it)');
  });

  // ── 4. Other metadata intact ───────────────────────────────────────────────

  test('D-246A: category still in metaParts for claim/truth', () => {
    assert.ok(cardSrc.includes("item.category||'general'"), 'category must still be in metaParts');
  });

  test('D-246A: truth rep × label still intact', () => {
    assert.ok(cardSrc.includes('rep ×'), 'truth scoreHint (rep ×N) must remain unchanged');
  });

  // ── 5. Public boundary ────────────────────────────────────────────────────

  test('D-246A: renderPublicProfileHtml does not reference Evidence score label', () => {
    assert.ok(!ppSrc.includes("'Evidence '") && !ppSrc.includes('"Evidence "'), 'renderPublicProfileHtml must not include review score labels');
  });

  // ── 6. Deploy integrity ───────────────────────────────────────────────────

  test('D-246A: CSS has no D-246A-specific new review-score rule (app-only change)', () => {
    // No CSS change required for D-246A — score labels are plain text in meta line
    assert.ok(!cssSrc.includes('review-score-label'), 'No new review-score-label CSS class should be added by D-246A');
  });

  test('D-246A: worker.js not modified', () => {
    assert.ok(!workerSrc.includes('D-246A'), 'D-246A is frontend/tests/docs only — worker.js must not be modified');
  });
}

// ── D-247A: Review card advisory hint grouping ────────────────────────────────
//
// Locks the D-247A change: low-priority advisory/quality hints
// (needs sharpening, category echo, ? borderline origin) moved from
// the primary head badge row into a dedicated .review-card-hints row.
// Scan-critical badges (type, state, report, ~similar, truth-derived, Builder)
// remain in the primary .review-card-head row.
...

{
  ...
  // ── 1. Hint row present when hints exist ──────────────────────────────────

  test('D-247A: review-card-hints container emitted by reviewCard', () => {
    assert.ok(cardSrc.includes('review-card-hints'), 'reviewCard must emit a .review-card-hints container');
  });

  test('D-247A: hintsRow variable computed in reviewCard', () => {
    assert.ok(cardSrc.includes('hintsRow'), 'reviewCard must declare and use a hintsRow variable');
  });

  test('D-247A: hintsRow is empty string when no hints apply (conditional)', () => {
    assert.ok(cardSrc.includes('hintBadges.length'), 'hintsRow must be conditionally rendered based on hintBadges.length');
  });

  // ── 2. Advisory hints still present (in hints row, not in head) ───────────

  test('D-247A: ? borderline origin hint still present in reviewCard', () => {
    assert.ok(cardSrc.includes('? borderline origin'), 'reviewCard must still render ? borderline origin hint');
  });

  test('D-247A: category echo hint still present in reviewCard', () => {
    assert.ok(cardSrc.includes('category echo') || cardSrc.includes('category-echo'), 'reviewCard must still render category echo hint');
  });

  test('D-247A: needs sharpening quality hint still present in reviewCard', () => {
    assert.ok(cardSrc.includes('needs sharpening'), 'reviewCard must still render needs sharpening quality hint');
  });

  // ── 3. Advisory hints no longer in primary head row ───────────────────────

  test('D-247A: needs sharpening no longer in review-card-head section of template', () => {
    // Head row ends at </div>${chipsRow} — hints come after meta
    const headSection = cardSrc.slice(cardSrc.indexOf('review-card-head'), cardSrc.indexOf('review-card-hints'));
    assert.ok(!headSection.includes('needs sharpening'), 'needs sharpening badge must not be in the primary .review-card-head row');
  });

  test('D-247A: isLikelyBorderlineDerivedClaim badge no longer in head row', () => {
    const headSection = cardSrc.slice(cardSrc.indexOf('review-card-head'), cardSrc.indexOf('review-card-hints'));
    assert.ok(!headSection.includes('b-borderline-origin'), 'b-borderline-origin badge must not be in the primary .review-card-head row');
  });

  // ── 4. Scan-critical badges remain in primary head row ────────────────────

  test('D-247A: ~similar badge remains in review-card-head', () => {
    // Head div in the template ends at ${chipsRow} — anchor from return` to find template portion
    ...
    const headSection = headEnd > headStart ? cardSrc.slice(headStart, headEnd) : cardSrc.slice(headStart, headStart + 1000);
    assert.ok(headSection.includes('b-similar'), '~similar advisory badge must remain in the primary .review-card-head row');
  });

  test('D-247A: truth-derived badge remains in review-card-head', () => {
    const tmplStart = cardSrc.indexOf('return`');
    const headStart = cardSrc.indexOf('review-card-head', tmplStart);
    const headEnd = cardSrc.indexOf('${chipsRow}', headStart);
    const headSection = headEnd > headStart ? cardSrc.slice(headStart, headEnd) : cardSrc.slice(headStart, headStart + 1000);
    assert.ok(headSection.includes('b-truth-derived'), 'truth-derived badge must remain in the primary .review-card-head row');
  });

  // ── 5. D-245B and D-246A preserved ────────────────────────────────────────

  test('D-247A: D-245B inline date still in metaParts', () => {
    assert.ok(cardSrc.includes("'Updated '+updated"), 'D-245B inline date must still be present in metaParts');
  });

  test('D-247A: D-246A readable score labels still present', () => {
    assert.ok(cardSrc.includes("'Evidence '") || cardSrc.includes('`Evidence '), 'D-246A readable Evidence score label must still be present');
  });

  // ── 6. CSS ────────────────────────────────────────────────────────────────

  test('D-247A: CSS defines .review-card-hints', () => {
    assert.ok(cssSrc.includes('.review-card-hints{'), 'styles.css must define .review-card-hints');
  });

  // ── 7. Public boundary ────────────────────────────────────────────────────

  test('D-247A: renderPublicProfileHtml does not reference review-card-hints', () => {
    assert.ok(!ppSrc.includes('review-card-hints'), 'renderPublicProfileHtml must not reference review-card-hints');
  });

  test('D-247A: renderPublicProfileHtml does not reference hintBadges', () => {
    assert.ok(!ppSrc.includes('hintBadges'), 'renderPublicProfileHtml must not reference hintBadges');
  });
...
// ── Section D-250B: Review active filter/sort summary ────────────────────────
// Locks the summary line rendered above cards in renderReviewList.
// No filter/sort behavior change. No search added. No next-item behavior change.
{
  test('D-250B: renderReviewActiveSummary function defined', () => {
    assert.ok(appSrc.includes('function renderReviewActiveSummary('), 'renderReviewActiveSummary must be defined');
  });

  test('D-250B: renderReviewActiveSummary references reviewStateFilter', () => {
    const fnStart = appSrc.indexOf('function renderReviewActiveSummary(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('reviewStateFilter'), 'summary must reference reviewStateFilter');
  });

  test('D-250B: renderReviewActiveSummary references reviewSortOrder', () => {
    const fnStart = appSrc.indexOf('function renderReviewActiveSummary(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('reviewSortOrder'), 'summary must reference reviewSortOrder');
  });

  test('D-250B: renderReviewActiveSummary uses list.length for visible item count', () => {
    const fnStart = appSrc.indexOf('function renderReviewActiveSummary(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('list.length'), 'summary must use list.length — count must reflect current filter+sort');
  });

  test('D-250B: renderReviewActiveSummary handles singular "1 item"', () => {
    const fnStart = appSrc.indexOf('function renderReviewActiveSummary(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes("'1 item'") || fnSrc.includes('"1 item"'), 'summary must distinguish "1 item" from "N items"');
  });

  test('D-250B: renderReviewActiveSummary emits review-active-summary CSS class', () => {
    const fnStart = appSrc.indexOf('function renderReviewActiveSummary(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('review-active-summary'), 'summary element must carry review-active-summary class');
  });

  test('D-250B: renderReviewActiveSummary filter label map includes Pending for review key', () => {
    const fnStart = appSrc.indexOf('function renderReviewActiveSummary(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes("review:'Pending'") || fnSrc.includes("'review':'Pending'"), 'filter label map must include Pending for the review (Pending) filter key');
  });

  test('D-250B: renderReviewList calls renderReviewActiveSummary(list)', () => {
    const rlStart = appSrc.indexOf('function renderReviewList(){');
    const rlSrc = appSrc.slice(rlStart, rlStart + 4000);
    assert.ok(rlSrc.includes('renderReviewActiveSummary(list)'), 'renderReviewList must call renderReviewActiveSummary with filtered+sorted list');
  });

  test('D-250B: summary inserted before feedbackBanner in renderReviewList innerHTML', () => {
    const rlStart = appSrc.indexOf('function renderReviewList(){');
    const rlSrc = appSrc.slice(rlStart, rlStart + 4000);
    assert.ok(rlSrc.includes('audit+summary+feedbackBanner'), 'summary must appear after audit and before feedbackBanner in innerHTML concat');
  });

  test('D-250B: CSS defines .review-active-summary', () => {
    assert.ok(cssSrc.includes('.review-active-summary{'), 'CSS must define .review-active-summary rule');
  });

  test('D-250B: applyReviewFilter behavior unchanged (D-250A lock compat)', () => {
    assert.ok(appSrc.includes("if(f==='similar')return list.filter(i=>!!i.near_duplicate_of)"), '~Similar filter must still check near_duplicate_of — D-250B must not change filter logic');
  });

  test('D-250B: renderPublicProfileHtml does not reference review-active-summary', () => {
    const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
    assert.ok(ppStart !== -1, 'renderPublicProfileHtml must be defined');
    const ppSrc = appSrc.slice(ppStart, ppStart + 20000);
  ...
  test('D-250B [deploy]: worker.js not modified', () => {
    assert.ok(!workerSrc.includes('D-250B'), 'D-250B is frontend+CSS+tests+docs only — worker.js must not be modified');
  });
}

// ── Section D-251A: Review zero-results filter clarity ───────────────────────
// Locks the enhanced empty state: title, context line, and Show-all button.
// No filter/sort/moderation behavior change. No search added.
{
  test('D-251A: renderReviewEmptyState function defined', () => {
    assert.ok(appSrc.includes('function renderReviewEmptyState('), 'renderReviewEmptyState must be defined');
  });

  test('D-251A: empty state title copy is "No review items match this view."', () => {
    assert.ok(appSrc.includes('No review items match this view.'), 'empty state must include exact title copy');
  });

  test('D-251A: empty state mentions "Show all review items" button', () => {
    assert.ok(appSrc.includes('Show all review items'), 'empty state must include "Show all review items" button copy');
  });

  test('D-251A: empty state uses review-empty-title CSS class', () => {
    const fnStart = appSrc.indexOf('function renderReviewEmptyState(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('review-empty-title'), 'empty state must use review-empty-title class');
  });

  test('D-251A: "Show all review items" button has type="button"', () => {
    const fnStart = appSrc.indexOf('function renderReviewEmptyState(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('type="button"') && fnSrc.includes('Show all review items'), '"Show all review items" must be a type="button" element');
  });

  test('D-251A: "Show all review items" button uses data-action="setReviewFilter" data-value="all"', () => {
    const fnStart = appSrc.indexOf('function renderReviewEmptyState(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('data-action="setReviewFilter"') && fnSrc.includes('data-value="all"'), 'Show-all button must use data-action event-delegation pattern with data-value="all"');
  });

  test('D-251A: "Show all review items" button does not reference setReviewSort', () => {
    const fnStart = appSrc.indexOf('function renderReviewEmptyState(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    const btnStart = fnSrc.indexOf('Show all review items');
    const btnCtx = fnSrc.slice(Math.max(0, btnStart - 120), btnStart + 50);
    assert.ok(!btnCtx.includes('setReviewSort'), 'Show-all button must not change sort order');
  });

  test('D-251A: empty state context line references reviewStateFilter', () => {
    const fnStart = appSrc.indexOf('function renderReviewEmptyState(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('reviewStateFilter'), 'empty state context line must reference reviewStateFilter');
  });

  test('D-251A: empty state context line references reviewSortOrder', () => {
    const fnStart = appSrc.indexOf('function renderReviewEmptyState(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes('reviewSortOrder'), 'empty state context line must reference reviewSortOrder');
  });

  test('D-251A: Show-all button absent when filter is already "all"', () => {
    const fnStart = appSrc.indexOf('function renderReviewEmptyState(');
    const fnEnd = appSrc.indexOf('\nfunction renderReviewList(', fnStart);
    const fnSrc = appSrc.slice(fnStart, fnEnd);
    assert.ok(fnSrc.includes("isFiltered") && fnSrc.includes("f!=='all'"), 'Show-all button must be conditional — absent when already on All filter');
  });

  test('D-251A: renderReviewList uses renderReviewEmptyState()', () => {
    const rlStart = appSrc.indexOf('function renderReviewList(){');
    const rlSrc = appSrc.slice(rlStart, rlStart + 4000);
    assert.ok(rlSrc.includes('renderReviewEmptyState()'), 'renderReviewList must delegate empty state to renderReviewEmptyState()');
  });

  test('D-251A: D-250B active summary still called in renderReviewList', () => {
    const rlStart = appSrc.indexOf('function renderReviewList(){');
    const rlSrc = appSrc.slice(rlStart, rlStart + 4000);
    assert.ok(rlSrc.includes('renderReviewActiveSummary(list)'), 'D-250B active summary must still be called — D-251A must not remove it');
  });

  test('D-251A: CSS defines .review-empty-title', () => {
    assert.ok(cssSrc.includes('.review-empty-title{'), 'CSS must define .review-empty-title');
  });

  test('D-251A: renderPublicProfileHtml does not reference review-empty-title', () => {
    const ppStart = appSrc.indexOf('function renderPublicProfileHtml(');
    assert.ok(ppStart !== -1, 'renderPublicProfileHtml must be defined');
    const ppSrc = appSrc.slice(ppStart, ppStart + 20000);
    assert.ok(!ppSrc.includes('review-empty-title'), 'review-empty-title must not appear in public profile render path');
  });

  test('D-251A [deploy]: worker.js not modified', () => {
    assert.ok(!workerSrc.includes('D-251A'), 'D-251A is frontend+CSS+tests+docs only — worker.js must not be modified');
  });
}
