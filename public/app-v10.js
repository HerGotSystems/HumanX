...
function helperText() {
  ...
  if (mode === 'export') return selected ? `<p class="small">Selected for RunPack:<br><b>${esc(selected.claim)}</b></p><p class="small">Legacy AIP compatible.</p>` : '<p class="small">No selected claim. Open a claim first, then generate a RunPack packet. Legacy AIP compatible.</p>';
  if (mode === 'submit') return '<p class="small">Submit one clear claim. Add evidence later in study mode.</p>';
  return '<p class="small">Claims: browse, open Study Claim, inspect evidence, pressure, tests, votes and RunPack.</p>' + graphBox();
}
...
function renderExport(){document.body.classList.remove('study-mode');document.getElementById('casefile').innerHTML=helperText();document.getElementById('side-tools').style.display='none';document.getElementById('main').innerHTML=`<div class="panel form-panel"><h2>RunPack Export</h2><p class="small">Open a claim first, generate packet, then copy into any AI. Public users do not use owner API credits. Legacy AIP compatible.</p><div class="actions"><button class="primary" onclick="generateRunPack()">Generate selected claim packet</button><button onclick="downloadJSON()">Download visible data JSON</button></div><pre class="output">${esc(selected?JSON.stringify({selected},null,2):'No selected claim. Open a claim first.')}</pre></div>`}
async function selectClaim(id){try{const data=await api('/api/claims/'+id);selected=data.claim;selected.evidence=data.evidence||[];selected.pressure=data.pressure||[];selected.tests=data.tests||data.home_tests||[];selected.analyses=data.analyses||[];selected.lineage=data.lineage||null;renderStudy();toast('Study loaded')}catch(e){selected=claims.find(c=>c.id===id)||selected;selected.analyses=selected.analyses||[];selected.lineage=selected.lineage||null;renderStudy();toast(e.message)}}
function renderStudy(){if(!selected)return;document.body.classList.add('study-mode');document.getElementById('side-tools').style.display='block';document.getElementById('main').innerHTML=`<div class="study"><div><button onclick="backToArena()">← Back</button><span class="badge ${cls(selected.status)}">${esc(selected.status)}</span></div><h2>${esc(selected.claim)}</h2><p class="small">${esc(selected.category)} · ${esc(selected.type)} · by ${esc(selected.handle||'anon')}</p><div class="meters wide">${meter('Evidence',selected.evidenceScore)}${meter('Testability',selected.testability)}${meter('Survivability',selected.survivability)}</div><div class="actions"><button onclick="voteClaim('believe')">I believe</button><button onclick="voteClaim('reject')">I reject</button><button onclick="voteClaim('unsure')">Uncertain</button><button class="primary" onclick="generateRunPack()">Generate RunPack</button></div>${sectionLineage()}<div class="study-grid"><section class="panel evidence-panel">${sectionEvidence()}</section><section class="panel pressure-panel">${sectionPressure()}</section><section class="panel tests-panel">${sectionTests()}</section><section class="panel analysis-panel">${sectionAnalyses()}</section></div></div>`;renderCaseMini()}
function renderCaseMini(){const l=selected?.lineage||{};document.getElementById('casefile').innerHTML=`<p class="small"><b>Selected claim</b><br>${esc(selected.claim)}</p><p class="small">Lineage: ${esc((l.truths||[]).length)} truth · ${esc((l.evidenceLinks||[]).length)} evidence · ${esc(l.pressureCount||0)} pressure · ${esc(l.analysisCount||0)} analysis.</p><p class="small">Use this panel to add evidence, pressure, generate RunPack, save analysis, or report. Legacy AIP compatible.</p>`}
...
function analysisItem(a){const raw=a.raw||a;return`<div class="row analysis-card"><span class="pill">ANALYSIS RESULT · ${esc(a.source||'runpack-user')}</span><span class="badge ${cls(a.verdict||raw.verdict)}">${esc(a.verdict||raw.verdict||'analysis')}</span><div class="meters">${meter('Evidence',a.evidenceScore||raw.evidence_score||0)}${meter('Test',a.testability||raw.testability||0)}${meter('Survive',a.survivability||raw.survivability||0)}</div><p class="small">${esc(a.plainLanguageSummary||raw.plain_language_summary||'Stored analysis result.')}</p><details><summary>Support / pressure / missing tests</summary><h4>Support</h4>${listBits(a.strongestSupport||raw.strongest_support)}<h4>Pressure</h4>${listBits(a.strongestPressure||raw.strongest_pressure)}<h4>Missing tests</h4>${listBits(a.missingTests||raw.missing_tests)}</details></div>`}
function sectionLineage(){const l=selected?.lineage||{};const truths=l.truths||[];const evidenceLinks=l.evidenceLinks||[];return`<section class="panel lineage-panel"><div class="section-head"><h3>Lineage</h3><span class="badge b-blue">Belief → Truth → Claim → Evidence</span></div><div class="graph-status"><span><b>${esc((l.beliefs||[]).length)}</b><small>Beliefs</small></span><span><b>${esc(truths.length)}</b><small>Truths</small></span><span><b>1</b><small>Claim</small></span><span><b>${esc(evidenceLinks.length)}</b><small>Evidence</small></span><span><b>${esc(l.pressureCount||0)}</b><small>Pressure</small></span><span><b>${esc(l.testCount||0)}</b><small>Tests</small></span><span><b>${esc(l.analysisCount||0)}</b><small>Analysis</small></span></div>${truths.length?`<div class="row"><span class="pill">upstream truth</span>${truths.slice(0,3).map(t=>`<p class="small"><b>${esc(t.statement||'Truth')}</b><br>${esc(t.bridge_note||t.origin||'Linked truth')}</p>`).join('')}</div>`:'<p class="small">No upstream truth linked yet. This claim may have been submitted directly or promoted from a belief without a formal truth bridge.</p>'}</section>`}
function sectionEvidence(){return`<div class="section-head"><h3>Evidence</h3><span class="badge b-blue">support / reused sources</span></div>${(selected.evidence||[]).map(evidenceItem).join('')||'<p class="small">No evidence yet.</p>'}`}
function sectionPressure(){return`<div class="section-head"><h3>Pressure</h3><span class="badge b-yellow">contradictions / attacks</span></div>${(selected.pressure||[]).map(p=>{const a=parseAnalysis(p.body||p.note);return a?`<div class="row analysis-card"><span class="pill">LEGACY ANALYSIS PRESSURE · ${esc(a.verdict||'analysis')}</span><b>${esc(p.title)}</b><p class="small">${esc(a.plain_language_summary||'Structured analysis pressure result.')}</p>${listBits(a.strongest_pressure)}</div>`:`<div class="row"><span class="pill">severity ${esc(p.severity||1)}</span><b>${esc(p.title)}</b><p class="small">${esc(shortText(p.body||p.note,620))}</p></div>`}).join('')||'<p class="small">No pressure yet.</p>'}`}
function sectionTests(){return`<div class="section-head"><h3>Tests</h3><span class="badge b-green">ways to verify</span></div>${(selected.tests||[]).map(t=>`<div class="row"><span class="pill">${esc(t.difficulty||'test')} · ${esc(t.safety_level||t.safetyLevel||'normal')}</span><b>${esc(t.title)}</b><p class="small">${esc(t.instructions)}</p></div>`).join('')||'<p class="small">No tests yet.</p>'}`}
function sectionAnalyses(){return`<div class="section-head"><h3>Analysis</h3><span class="badge b-blue">RunPack results</span></div><div class="row analysis-save"><p class="small">Paste the JSON result from your AI here. It saves into analysis_results, not evidence.</p><textarea id="analysisPaste" placeholder="Paste RunPack output JSON here"></textarea><div class="actions"><button class="primary" onclick="saveAnalysisResult()">Save Analysis Result</button></div></div>${(selected.analyses||[]).map(analysisItem).join('')||'<p class="small">No saved analysis results yet.</p>'}`}
...
window.setMode=setMode;window.searchCurrent=searchCurrent;window.loadClaims=loadClaims;window.selectClaim=selectClaim;window.saveClaim=saveClaim;window.addCaseItem=addCaseItem;window.reportSelected=reportSelected;window.generateAIP=generateAIP;window.generateRunPack=generateRunPack;window.copyAIP=copyAIP;window.downloadJSON=downloadJSON;window.voteClaim=voteClaim;window.backToArena=backToArena;window.submitTruth=submitTruth;window.convertTruth=convertTruth;window.attachEvidencePrompt=attachEvidencePrompt;window.saveBeliefMirror=saveBeliefMirror;window.beliefPreview=beliefPreview;window.promoteBelief=promoteBelief;window.saveAnalysisResult=saveAnalysisResult;
boot();