...
**Project state checkpoint:** [`docs/PROJECT_STATE.md`](PROJECT_STATE.md) — updated D-255A (2026-07-01). Covers D-210→D-218 hardening arc + D-220→D-225 public profile polish arc + D-227→D-231 review ergonomics arc + D-233→D-237 duplicate advisory arc + D-239→D-240 review-to-study navigation arc + D-242→D-243 review next-item flow arc + D-245→D-248 review card metadata density arc + D-250→D-254 review search/filter clarity arc, current baseline 2877/0/24/57, privacy boundary state, Drift/Belief expansion state, deployment state, safe next-work rules 1–43. D-256B in progress: baseline now 2903/0/24/57.

### `D256B_REVIEW_DUPLICATE_SIMILAR_FILTER_LABEL_CLARITY.md` ⭐ CURRENT — D-256B REVIEW DUPLICATE/SIMILAR FILTER LABEL CLARITY — DEPLOY NEEDED

Copy-only rename: `Dupes` filter button label → `Dupes + Similar` in all visible surfaces. Predicate unchanged (`duplicate_of || near_duplicate_of`). Filter state key `duplicate` unchanged. Changes: `renderReviewFilterBar` defs, `renderReviewActiveSummary` filterLabels, `renderReviewEmptyState` filterLabels, `renderReviewFilterHelper` helper copy (`Dupes + Similar includes confirmed duplicates and near-duplicate advisories.`), `reviewEmptyText` duplicate entry (now mentions confirmed duplicates and similar advisories with context note), `renderReviewAuditSummary` stat label. `~Similar` chip/predicate/card badges/inspect panel fields/action buttons unchanged. 26 new D-256B tests. Baseline: 2903/0/24/57. Deploy needed.
