...
| POST | `/api/report` | Public — requires `x-humanx-user` | `reports`, `claims` or `evidence` (auto-escalation), `users`, `rate_limits`, `claims` (score recalc on evidence escalation) | Fail-closed rate limit: 20/hr per IP | D-42B: supports `targetType='evidence'` — same report-bombing risk as claims. Evidence auto-escalates to `review_state='review'` at 2+ reports, hiding it from Study and Vault. Rate limit (20/hr per IP) is the only defence. D-50: score recalc now fires on parent claim when evidence is first escalated (report 2); escalating evidence out of public visibility now immediately removes it from the claim score | Yes — double report by same user, report count threshold, rate limit; confirm evidence auto-escalation fires at threshold 2 |
