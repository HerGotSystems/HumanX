...
Deployed D-209H worker (commit `3dbed11`) to Cloudflare Workers production.
...
| Toggle ON → preview updates locally (no backend call) | PASS |
| Public profile unchanged before Save | PASS |
| Save → public profile shows label + contradictionCount + date + Reflection scores | PASS |
| Public profile does not show: dominant pattern, top alignment, alignment labels, religion/ideology/worldview labels, private reflection cards, pattern summary | PASS |
| Toggle OFF → Save → public profile hides Reflection scores | PASS |

Scores opt-in is confirmed reversible.
