...
Last updated: 2026-06-06 after D-40 evidence moderation Phase 2 plan.
...
D-38 merged (PR #97, `c03e5eb`), D-39 validated (all live checks green). D-40 evidence moderation Phase 2 plan complete — see `docs/D40_EVIDENCE_MODERATION_PHASE2_PLAN.md`. Static baseline 103/24/39.

1. **D-41 — Evidence schema migration proposal (docs-only, direct main)** — create `migrations/0007_evidence_review_state.sql` and update `0003_full_schema.sql` to include `review_state` and `report_count` on `evidence`. Do not apply to production without explicit per-session approval and `PRAGMA table_info(evidence)` confirmation.
2. **D-42 — Backend evidence moderation (branch + PR)** — Worker/module changes to `addEvidence`, `listEvidenceVault`, `claimDetail`, `getClaim`, `reportTarget`, `reviewQueue`, `reviewDecision`. Prerequisite: D-41 reviewed and production migration approved.
3. **D-43 — Frontend review UI for evidence (branch or direct main)** — `reviewCard` and `renderReviewInspectPanel` handle `target_type='evidence'`. Prerequisite: D-42 merged.
4. **D-44 — Validation record (docs-only, direct main)** — record static/live/manual results after D-42+D-43. Expected baseline 110/24/39.
...
6. **AI-return packet linkage (optional, Approach A)** — implement only if operational need emerges. See D-31 section 7.
7. **No live write smoke** without explicit per-session approval.
8. **No production migration** (0006 or 0007) without explicit per-session approval and PRAGMA confirmation.
9. **No evidence-level moderation code** before D-41 migration proposal is reviewed and D-42 branch is approved.
