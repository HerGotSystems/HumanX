...
Last updated: 2026-06-06 after D-39 post-merge validation record.
...
| D-39 | docs-only (direct main) | Post-merge validation record for D-38 — static baseline confirmed 103/24/39 on merged main (`c03e5eb`); live validation (Cloudflare deploy, Read Smoke, manual UI sanity) recorded as pending user confirmation; what D-38 proves and does not prove documented; next safe work captured; full record in `docs/D39_D38_POSTMERGE_VALIDATION.md` |
...
D-38 merged (PR #97, commit `c03e5eb`). Static baseline confirmed 103/24/39 on merged main. Live validation (Cloudflare deploy, Read Smoke, manual UI sanity) is pending user confirmation — see `docs/D39_D38_POSTMERGE_VALIDATION.md`.

1. **Confirm HumanX Read Smoke on main post D-38** — trigger `HumanX Read Smoke` workflow manually on `main` if it did not fire for PR #97. Confirm 8 endpoint groups pass (same baseline as D-33).
2. **Manual UI sanity** — confirm Home, Claims list, public claim Study, Evidence Vault, and RunPack on a public claim all load correctly after Cloudflare deploys D-38. See pending checks in `docs/D39_D38_POSTMERGE_VALIDATION.md`.
3. **Run D-26 manual test plan** — `docs/D26_MANUAL_LIVE_UI_TEST_PLAN.md` covers RunPack provenance (section 8), duplicate-resolution (section 6), Study continuity (section 7), and more. Execute when the user is ready to open a browser session against `humanx.rinkimirikata.com`. Use `HX_TEST_D26_` prefix for all test claims. Requires explicit per-session approval for any **[WRITE]** steps.
4. **D-40 evidence moderation Phase 2 plan (docs-only)** — plan the schema change that would add per-evidence `review_state` (deferred from D-38). Only after manual validation in items 1–2 above passes. Any Worker/D1 change requires its own branch + PR.
5. **AI-return packet linkage (optional, Approach A)** — if linkage ever becomes operationally needed, implement in the safe sequence from D-31 section 7. Do not implement speculatively.
6. **No live write smoke** without explicit per-session approval.
7. **No production migration 0006 apply** unless `PRAGMA table_info(claims)` first confirms `near_duplicate_of` is absent.
8. **No evidence-level moderation migration** (Phase 2) without an explicit D-40 plan doc reviewed first and per-session approval.
