...
Last updated: 2026-06-06 after D-33 read smoke CI result recorded.
...
**Pending (before June 16, 2026):** Update `node-version: '20'` to `node-version: '24'` in `.github/workflows/read-smoke.yml` — non-blocking today but GitHub will force Node 24 on June 16. One-line change; direct main acceptable.

---

## What is safe to do next

The D-23 planning chain and all implementation batches through D-33 are complete. The canonical RunPack provenance system (D-24B + D-28 + D-29) is fully operational. The D-24 moderation chain (D-24A → D-24G) is fully operational. D-32 added the read smoke CI workflow; D-33 confirmed the first successful run (all 8 endpoints green, 15s). Static checks hold at 100/24/39.

1. **Update workflow to Node 24 before June 16, 2026** — change `node-version: '20'` to `node-version: '24'` in `.github/workflows/read-smoke.yml`. One-line fix; no script changes; direct main acceptable. Non-blocking today but will be forced by GitHub on June 16.
