...
### `D100D_LIVE_DEPLOYMENT_VERIFICATION.md`
Deployment record for D-100B (direct commit `08fa3cb` on main) — Worker version `d3ae3a01`, Wrangler 4.99.0, uploaded asset `/app-v10.js`. Documents what went live (Study/Claim verdict & score clarity: Study header qualifier "Verdict is a pressure-test label, not an automatic truth ruling. Scores reflect the current submitted packet, not absolute certainty."; Evidence/Testability/Survivability meter tooltips; arena/study helperText "Verdicts are pressure-test labels, not automatic truth rulings."; verdict colours/names and score logic unchanged), user-confirmed live verification, safety confirmations (no D1/schema/admin mutations), static checks 324/24/39, and the standing Wrangler/D1 explicit-approval rule.
**Read when:** verifying what is currently deployed, or confirming the Study verdict/score clarity changes are live.

### `D101D_LIVE_DEPLOYMENT_VERIFICATION.md`
Deployment record for D-101B (PR #121) — main HEAD `1d3f8f6`, feature commit `7a3b93b`, Worker version `5a73a625`, Wrangler 4.99.0, uploaded asset `/app-v10.js`. Documents what went live (public journey minor polish: `.commandbar` flex/wrap header layout so brand/tabs/statusline no longer stack; `renderError` "← Back to Home" recovery button calling existing `setMode('home')`; Retry deliberately skipped; no API behavior change), user-confirmed live verification, safety confirmations (no D1/schema/admin mutations), static checks 328/24/39, and the standing Wrangler/D1 explicit-approval rule. Final deployment in the D-93→D-101 public safety/clarity run.
**Read when:** verifying what is currently deployed, or confirming the commandbar layout + error-recovery changes are live.

### `D103D_LIVE_DEPLOYMENT_VERIFICATION.md`
Deployment record for D-103B (PR #122) — main HEAD `263c7c4`, feature commit `c04c79e`, Worker version `f9895422`, Wrangler 4.100.0, uploaded assets `/styles.css` + `/app-v10.js`. Documents what went live (evidence quality & source clarity: `vibes`→"weak argument" label map, tiered quality pill styling strong/mid/weak/neutral, muted "no source provided" indicator for missing citations, reused compact rows inherit the new display; no "verified/trusted" wording; score/verdict logic unchanged), user-confirmed live verification, safety confirmations (no D1/schema/admin mutations), static checks 340/24/39, and the standing Wrangler/D1 explicit-approval rule.
**Read when:** verifying what is currently deployed, or confirming the evidence quality/source display changes are live.

### `D104D_LIVE_DEPLOYMENT_VERIFICATION.md`
Deployment record for D-104B (PR #123) — main HEAD `cb04a87`, feature commit `eabebaa`, Worker version `eb055ee3`, Wrangler 4.100.0, uploaded asset `/app-v10.js`. **Security fix** — closes the D-104A HIGH-severity stored-XSS vector: `safeHttpUrl()` whitelists `http:`/`https:`, `sourceLink` renders non-web/unsafe source values (`javascript:`/`data:`/`vbscript:`/`blob:`/`file:`/protocol-relative/scheme-less/malformed) as escaped non-clickable text ("not clickable — not a valid web address") instead of an anchor href; valid links keep `rel="noopener noreferrer"`; D-103B quality display intact; no source data mutated. User-confirmed live verification, static checks 353/24/39. **Open follow-up:** D-104C/BE-1 server-side `sourceUrl` validation (defense-in-depth, Worker change).
**Read when:** verifying what is currently deployed, confirming the source-URL XSS fix is live, or before touching evidence source rendering / `sourceLink`.

### `D104H_WORKER_SOURCE_URL_VALIDATION_DEPLOYMENT.md`
Deployment record for D-104F (PR #124) — main HEAD `14c1233`, feature commit `172f025`, Worker version `0bb54517`, Wrangler 4.100.0 (Worker-code change; no asset upload). **Backend defense-in-depth** for the D-104B source-URL XSS fix: `httpUrlOrNull()` validates `body.sourceUrl` on `POST /api/evidence` (`cleanText`→`new URL()`→http/https only, else null), coercing non-web/malformed source values to null before D1 storage while preserving the evidence submission. Pressure route unchanged; importer unchanged; no schema/migration; no cleanup of existing rows; no verification wording. With this both layers (render `eb055ee3` + storage `0bb54517`) enforce http/https. User-confirmed live verification, static checks 357/24/48.
**Read when:** confirming the backend source-URL validator is live, or before changing `/api/evidence` source handling / `httpUrlOrNull`.

### `D106F_ADMIN_HARDENING_DEPLOYMENT_VERIFICATION.md`
Deployment record for D-106B (PR #125, `cba4886`) + D-106D (PR #126, `bc9f8b6`) — main HEAD `1e46615`, Worker version `019b929f`, Wrangler 4.100.0 (Worker-code change; no asset upload; first attempt failed on a transient Cloudflare connectivity issue, retry succeeded). **Security hardening** from the D-106A admin-token/secret audit: `/api/debug` now requires `requireAdmin` (unauthorised → `403 ADMIN_REQUIRED`, authorised shape unchanged); `requireAdmin` uses dependency-free timing-safe-ish `safeEqual` with the raw `!==` removed and fail-closed-on-missing-env preserved; new `.gitignore` protects local env/secret files; no token value committed; D-106D made the `.gitignore` static check CRLF-tolerant. User-confirmed live verification, static checks 362/24/56. **Token rotation NOT performed** — recommended operator follow-up (token material appeared in earlier chats); procedure in `D106B_ADMIN_SECRET_HYGIENE_DEBUG_HARDENING.md`.
**Read when:** confirming the admin/debug hardening is live, before touching `requireAdmin` / `safeEqual` / `/api/debug`, or planning the `HUMANX_ADMIN_TOKEN` rotation.

### `D107D_REVIEW_INSPECT_SOURCE_SANITISATION_DEPLOYMENT.md`
Deployment record for D-107B (PR #127, `f7ea706`) — main HEAD `98f1b0f`, Worker version `d10c6a67`, Wrangler 4.100.0, uploaded asset `/app-v10.js`. **Security fix** closing the last source-render path missed by the D-104 arc: the admin Review inspect panel previously rendered evidence source URLs via its own inline `href="${esc(item.source_url)}"` (admin-targeting XSS for legacy unsafe rows); now routes through the shared `safeHttpUrl`-guarded `sourceLink`, with Review inspect Quality via `evidenceQualityLabel` (`vibes`→"weak argument"). With this, all three source paths (public Study `eb055ee3`, Worker storage `0bb54517`, admin Review `d10c6a67`) enforce http/https. User-confirmed live verification, static checks 372/24/56. D-96B/D-95B/advisory badges preserved; no token rotation.
**Read when:** confirming the Review inspect source fix is live, or before touching `renderReviewInspectPanel` source/quality rendering.

### `D-108A-final-source-safety-regression-audit.md`
Final source-safety regression audit (D-108A) — repo HEAD at audit `c516073`, deployed Worker `d10c6a67`, baseline 372/24/56. **Verdict: no D-108B code patch needed.** Confirms the source-safety chain is complete and bypass-free: `app-v10.js` contains exactly one `href=` (the sanitised `sourceLink` safe branch); all four evidence render paths (vault card, study row, reused compact, admin Review inspect) route through `sourceLink`→`safeHttpUrl`; `/api/evidence` validates via `httpUrlOrNull`; belief-bridge evidence INSERT binds `source_url` to literal `''`; pressure has no source column; RunPack emits source URLs as JSON data only (never a HumanX-rendered href). Only finding: orphaned `public/app-v5.js`–`app-v9.js` contain the old pre-D-104B pattern but are loaded by no HTML — LOW hygiene, not a live vuln. Optional non-urgent follow-ups: delete the orphan files, add a "no raw source href" guard, read-only legacy `source_url` D1 audit.
**Read when:** confirming source-URL handling is fully safe, or before any change to evidence source rendering/storage.

### `D109C_ORPHAN_LEGACY_FRONTEND_BUNDLE_CLEANUP_DEPLOYMENT.md`
Deployment record for D-109B (PR #128, `38dba24`) — main HEAD `8f48d22`, Worker version `adb94a83`, Wrangler 4.100.0 (asset set dropped 15→8 files; no upload — `app-v10.js` unchanged). Records removal of orphaned `public/app-v3.js`–`app-v9.js` from the deployed assets, `app-v10.js` remaining the sole served frontend bundle, the `PROJECT_INDEX.md` correction, and Section 50 guard tests. No runtime behavior change; source-safety chain intact; rollback via git history. User-confirmed live verification, static checks 375/24/56. No token rotation.
**Read when:** confirming which frontend bundles are deployed, or that the orphan cleanup is live.

### `D111D_SUBMIT_TRUST_FRAMING_DEPLOYMENT.md`
Deployment record for D-111B (PR #129, `e9dfce6`) — main HEAD `26acb56`, Worker version `f089e116`, Wrangler 4.100.0, uploaded assets `/styles.css` + `/app-v10.js`. Surfaces the submit-panel trust note "Scores reflect submitted evidence — not an automatic verdict." in the visible main column (previously only in the submit-mode helperText, hidden because the side dock is `display:none` in submit mode). Closes the one public surface (submit) that lacked the "does not decide truth" framing; home/arena/study/truths framing intact; submit form/API unchanged; no verification wording; no token rotation. User-confirmed live verification, static checks 383/24/56.
**Read when:** confirming the submit trust note is live, or reviewing where each public surface carries the "not an automatic verdict" framing.

### `D112D_MOBILE_TAB_NAV_AFFORDANCE_DEPLOYMENT.md`
Deployment record for D-112B (PR #130, `8468b27`) — main HEAD `b781873`, Worker version `afa1cb51`, Wrangler 4.100.0, uploaded assets `/styles.css` + `/app-v10.js`. Mobile-nav UX polish: the `.tabs` strip (≤900px) gains a right-edge `mask-image` fade cue signalling horizontal scroll, and `setMode` scrolls the active tab into view (guarded). Touch scroll + hidden scrollbar preserved; all 9 tabs kept; Beliefs redirect, D-111 submit trust note, and source/admin hardening intact. User-confirmed live verification, static checks 392/24/56. No backend/API behavior change, no token rotation.
**Read when:** confirming the mobile tab affordance is live, or before touching `.tabs` CSS / `setMode` nav behavior.

### `D113C_HOME_MOBILE_CARD_DENSITY_DEPLOYMENT.md`
Deployment record for D-113B (PR #131, `972adc8`) — main HEAD `5aaa132`, Worker version `c0cbe389`, Wrangler 4.100.0, uploaded asset `/styles.css` (CSS-only; `app-v10.js` unchanged). Mobile density polish: `@media(max-width:600px){.cc-card-when{display:none}}` hides the secondary "When:" line on Home action cards at phone widths; the text stays in source (desktop/tablet unchanged), all 7 cards kept, titles/descriptions intact. Truths form-above-list collapse (D-113A C.2) deliberately deferred. D-111 submit trust note + D-112 tab cues + source/admin hardening preserved. User-confirmed live verification, static checks 403/24/56. No backend/API change, no token rotation.
**Read when:** confirming the Home mobile density change is live, or before touching `.cc-card-when` / Home card CSS.

### `D114C_TRUTHS_MOBILE_FORM_DENSITY_DEPLOYMENT.md`
Deployment record for D-114B (PR #132, `d0a8a54`) — main HEAD `c93bd3b`, Worker version `3fe7ab7f`, Wrangler 4.100.0, uploaded `/styles.css` + `/app-v10.js`. Truths Add-a-Truth form now wrapped in `<details class="truth-add-details">`: collapsed on phones (≤600px) behind "Add a public Truth", force-expanded on desktop (≥601px, summary hidden) so the truth list browses sooner on mobile. Field IDs (`truthStatement`/`truthCategory`/`truthOrigin`/`truthType`) + `submitTruth()` unchanged; form stays above the list; "Public means visible, not proven" + D-111/D-112/D-113 features preserved. User-confirmed live verification, static checks 416/24/56. No backend/API change, no token rotation. Closes the D-111→D-114 public mobile-UX increment.
**Read when:** confirming the Truths mobile form collapse is live, or before touching `renderTruths` / `.truth-add-details`.

### `D99A_PRODUCTION_CHECKPOINT_REVIEW_TRUTH_ONBOARDING.md`
Production checkpoint after the D-93→D-98 Review/Truth/onboarding safety run — main HEAD `b8d69b4`, static baseline 312/24/39, deployed Worker `9f8c2821`. Summarizes the six completed/deployed passes (Truth-derived Review context + category-echo guard, Review inspect ergonomics, card-row Approve confirmation, Truth trust-signal clarity, onboarding terminology), current safe operating rules, known live moderation state (SMALL INDEFERENT TRUTH still visible/borderline; its claim still pending; personal-belief policy deferred), full test-coverage map (Sections 34/38/39/40/41/42), and recommended next work. Recommends **D-100A — Study/Claim evidence display audit** as the next task.
**Read when:** orienting at the start of a new session, choosing the next task, or confirming the current safe baseline after the D-93→D-98 run.
