...
async function addEvidence(request, env) { const userId=await requireUser(request, env); await safeRateLimit(request,env,`evidence:${ip(request)}`,20,3600000); const body=await readJson(request); const claimId=cleanId(body.claimId); const title=cleanText(body.title || 'Evidence',120); const note=cleanText(body.body || body.note || '',1200); if (!claimId || note.length < 3) return json({ error:'BAD_EVIDENCE' },400); const evidenceClaimRow=await env.DB.prepare(`SELECT id FROM claims WHERE id=? LIMIT 1`).bind(claimId).first(); if (!evidenceClaimRow) return json({ error:'CLAIM_NOT_FOUND' },404); await ensureUser(env,userId); const item=await insertEvidence(env,claimId,userId,cleanText(body.stance || 'support',20),note,title,cleanText(body.quality || 'testimony',40),httpUrlOrNull(body.sourceUrl)); await recalcClaimScore(env,claimId); return json({ evidence:item, claim:await claimOnly(env,claimId) }); }
