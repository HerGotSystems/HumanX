...
Last updated: 2026-06-06 after D-24F near_duplicate_of migration proposal.
...
1. **Deferred from D-24A/B** — Claims list scroll restoration (`#main.scrollTop` save/restore on arena→study→back); RunPack Phase 2 backend provenance (worker-side `packet_id` generation, branch + PR). Both low risk.
