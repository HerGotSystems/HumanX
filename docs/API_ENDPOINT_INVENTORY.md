...
| POST | `/api/my-humanx/profile-settings` | Saves the caller's own public-profile settings (`profile_public`, `profile_slug`, `profile_bio`) (D-140B) | Public (requires `x-humanx-user`) | `users` | Always scoped to the requester's own `x-humanx-user` — never accepts a target user id. Slug required only when `profile_public=1`; validated (lowercase, `a-z0-9-`, 3-40 chars, no leading/trailing/double hyphen, reserved-word list), enforced unique via `idx_users_profile_slug` (`409 SLUG_TAKEN` on conflict). When `profile_public=0` the slug is always stored as NULL. Bio capped at 240 chars, plain text only. **No public read route exists yet** — settings are saved but nothing is exposed publicly by this endpoint (see docs/D140A audit) |
