...
Last updated: 2026-06-06 after D-48 evidence moderation platform checkpoint.
...
8. **Execute D-47 manual test plan** — only when user explicitly approves a live-write browser session. `HX_TEST_D47_` prefix. Plan: `docs/D47_EVIDENCE_MODERATION_MANUAL_TEST_PLAN.md`.
9. **D-49 — After D-47 execution** — docs-only record of test pass/fail, evidence IDs created, cleanup status.
10. **`recalcClaimScore` audit (optional, future)** — `recalcClaimScore` counts all evidence regardless of `review_state`; pending evidence inflates claim scores. Fix: add `COALESCE(review_state,'public')='public'` filter to score aggregation. Requires branch + PR (Worker change). No migration needed.
11. **Actions v5 upgrade (optional)** — upgrade to `actions/checkout@v5` / `actions/setup-node@v5` when available with native Node 24. CI-only, direct main.
12. **D-26 general manual test plan** — `docs/D26_MANUAL_LIVE_UI_TEST_PLAN.md`. Still available.
13. **No live write smoke** without explicit per-session approval.
14. **No further migrations** without explicit per-session approval and PRAGMA confirmation.
