...
### `D149E_OWNER_TOKEN_TELEMETRY_TIME_WINDOW_REPORTING_CHECKPOINT.md` ⭐ CURRENT — TIME-WINDOWED TELEMETRY REPORTING ADDED; NOT YET DEPLOYED — READY FOR NEXT FEATURE
D-149D's deliberate route-coverage pass permanently mixed 10 curated verification records into the all-time aggregate, with no way to look at only organic traffic going forward. D-149E adds an optional `window=all|1h|24h|7d` query param to `GET /api/debug/owner-token-telemetry` (default `all`, unchanged behavior; an unrecognized value silently normalizes to `all` rather than erroring) plus four new response fields (`sample_window`, `all_time`, `window_started_at`, `window_ended_at`) so a caller always knows exactly which window was applied. Non-`all` windows filter both the aggregate and `recent` queries by `created_at >= window_started_at`. All D-149B response-shape guarantees (zero-defaulted `status_counts`/`route_status_counts`, safe `valid_ratio`, `observed_routes`/`unobserved_owner_routes`) unchanged. Still admin-gated, still advisory-only, no enforcement, no soft warning (explicitly smoke-tested absent), no migration, no `wrangler.toml`/frontend change. **Live verification still pending** — ships code only. Baseline: 1016/24/57 (1 expected parameterised-route warning), up from 1006/24/57. Recommended next: deploy, live-verify each window value, then use `window=24h`/`7d` to review genuinely organic traffic — the actual prerequisite for any soft-warning or enforcement-readiness reconsideration.
